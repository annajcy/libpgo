...
def _mbs_from_2(a: np.ndarray, b: np.ndarray):
    c = (a + b) * 0.5
    return c, float(np.linalg.norm(b - a)) * 0.5


def _mbs_from_3(a: np.ndarray, b: np.ndarray, c: np.ndarray):
    ab, ac = b - a, c - a
    n = np.cross(ab, ac)
    denom = 2.0 * float(n @ n)
    if denom < 1e-30:
...
def _mbs_from_4(a: np.ndarray, b: np.ndarray, c: np.ndarray, d: np.ndarray):
...
def _mbs_from_boundary(boundary: list[np.ndarray]):
    if not boundary:
