...
:class:`pypgo.solver.NewtonOptimizer` via ``sparse_solver=...``::
    ...
    ps.NewtonOptimizer(sparse_solver=ps.OrigPardiso())
...
* ``ps.Auto()`` tries MKLPardiso first, then EigenLDLT.
* ``ps.EigenLDLT()`` Eigen's built-in SimplicialLDLT (always available).
* ``ps.MKLPardiso()`` Intel MKL PARDISO via Eigen wrapper.
* ``ps.OrigPardiso()`` Orig PARDISO (requires orig_pardiso.lic).
"""
...
def OrigPardiso() -> SparseSolver:
    """Orig PARDISO backend (requires separate license)."""
