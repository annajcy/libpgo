...
| Newton solver | `NewtonSolver` through `solver/service/optimizationService.h` | `pypgo.solver.solve_newton` | M3 | `tests/pypgo/test_solver.py::test_newton_solves_quadratic__warm_start` | needs facade | C++ 长期边界是 `minimize(problem, x0, NewtonOptions)`；Python 只暴露 `solve_newton`，不暴露 stateful `NewtonSolver` / `SolverParam` |
