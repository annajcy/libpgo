...
class CMakeExtension(Extension):
    """Placeholder extension whose native module is produced by CMake."""
...
class CMakeBuild(build_ext):
    """Build pypgo's native extension through the CMake pypgo-wheel preset."""

    preset = os.environ.get("PYPGO_CMAKE_PRESET", "pypgo-wheel")
    target = "pypgo_core"
    ...
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        subprocess.run(
            ["cmake", "--build", ".", "--target", self.target, *build_args],
            cwd=build_temp,
            check=True,
        )

        output_path = ext_fullpath.resolve()
        if not output_path.exists():
            candidates = sorted(extdir.glob("_core.*"))
            raise RuntimeError(
                "CMake build completed, but the expected extension "
                f"{output_path.name} was not found. Found: "
