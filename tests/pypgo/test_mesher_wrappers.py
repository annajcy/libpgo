...
def test_cubic_mesher_center_mode_avoids_conservative_boundary_voxels():
    surface = pgo.mesh.create_sphere(radius=1.0, axis_subdiv=16, height_subdiv=8)

    conservative = cubic_mesher(surface, resolution=4)
    center = cubic_mesher(surface, resolution=4, occupancy="center")

    assert 0 < center.num_elements < conservative.num_elements
