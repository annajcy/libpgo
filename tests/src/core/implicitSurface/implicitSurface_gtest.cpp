...
TEST(BooleanFieldTest, UnionSampleToGrid)
{
  ...
  a->fill(-1.0);
  b->fill(1.0);
  IS::GridField out = IS::makeUnion(a, b)->sampleToGrid(spec);
...
TEST(BooleanFieldTest, IntersectionSampleToGrid)
{
  ...
  a->fill(-1.0);
  b->fill(1.0);
  IS::GridField out = IS::makeIntersection(a, b)->sampleToGrid(spec);
  for (int i = 0; i < out.size(); ++i)
    EXPECT_DOUBLE_EQ(out[i], 1.0);
}
...
TEST(BooleanFieldTest, DifferenceSampleToGrid)
{
  ...
  a->fill(-1.0);
  b->fill(1.0);
  IS::GridField out = IS::makeDifference(a, b)->sampleToGrid(spec);
  for (int i = 0; i < out.size(); ++i)
    EXPECT_DOUBLE_EQ(out[i], -1.0);
}
...
TEST(OffsetFieldTest, SampledMeshDistanceOffsetProducesNegativeInside)
{
  ...
  IS::GridField outField = IS::OffsetField(distance, 1.0).sampleToGrid(spec);
...
TEST(SphereFieldTest, ThickenSphereShellProducesShellAroundSphere)
{
  ...
  IS::GridField radiusField = sphere->sampleToGrid(spec);
...
TEST(SphereFieldTest, EvaluateBallSDFIsNegativeInside)
{
  ...
  IS::SphereField sphere(ES::V3d(0.0, 0.0, 0.0), 1.0);
  IS::GridField outField = sphere.sampleToGrid(spec);
...
TEST(SphereFieldTest, TruncationViaBooleanIntersection)
{
  ...
  IS::GridField truncated = IS::makeIntersection(shell, sphere)->sampleToGrid(spec);
