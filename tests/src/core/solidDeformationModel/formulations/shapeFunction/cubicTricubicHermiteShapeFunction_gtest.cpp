...
namespace
{
// Corner parametric bits — must match CubicLinearShapeFunction / kVertexAlpha/Beta/Gamma.
...
TEST(CubicTricubicHermiteShapeFunctionGTest, ReproducesSeparableCubicValue)
{
...
TEST(CubicTricubicHermiteShapeFunctionGTest, ReproducesSeparableCubicGradient)
{
  CubicTricubicHermiteShapeFunction basis;
  auto dof = separableFieldDofs();
...
TEST(CubicTricubicHermiteShapeFunctionGTest, DerivativeMatchesFiniteDifference)
{
  CubicTricubicHermiteShapeFunction basis;
  const double h = 1e-6;
  for (const auto &pt : kTestPoints) {
...
TEST(CubicTricubicHermiteShapeFunctionGTest, ValueModeNodalInterpolation)
{
  CubicTricubicHermiteShapeFunction basis;
  for (int c = 0; c < 8; c++) {
    double N[64];
