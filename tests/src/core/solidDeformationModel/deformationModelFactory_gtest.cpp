...
// Mirror the tet manual builder for cubic meshes.
std::shared_ptr<DeformationModelEnergy> buildCubicEnergyManually(
  const pgo::VolumetricMeshes::CubicMesh &cubicMesh, ES::VXd &restPositionOut)
{
  std::unique_ptr<SimulationMesh> mesh = loadCubicMesh(&cubicMesh);
...
// and finite gradient.
// IMPORTANT: DeformationModelEnergy stores rest position internally and
// func(x) computes energy at restPosition + x. State x is displacement
// from rest, NOT absolute position.
TEST(DeformationModelFactoryGTest, TetZeroDisplacementBaseline)
{
  ...
  // Zero displacement => actual position = rest position => energy near zero.
  ...
  // Gradient at zero displacement must be finite.
...
// Baseline: cubic deformation energy at zero displacement has near-zero energy
// and finite gradient.
// State convention: func(x) computes energy at restPosition + x.
// x is displacement from rest, not absolute position.
TEST(DeformationModelFactoryGTest, CubicZeroDisplacementBaseline)
{
  ...
  DeformationModelBundle bundle = makeDeformationModel(
    cubicMesh, DeformationModelElasticMaterial::STABLE_NEO, DeformationModelPlasticMaterial::VOLUMETRIC_DOF6);
...
// Baseline: cubic makeDeformationModel facade matches the manual
// manager/assembler chain, mirroring the existing tet factory test.
TEST(DeformationModelFactoryGTest, CubicMakeDeformationModelMatchesManualChain)
{
  pgo::Logging::init();
  ...
  ES::VXd manualRest;
  auto manual = buildCubicEnergyManually(cubicMesh, manualRest);
