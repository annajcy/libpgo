...
TEST(MeshConstructionOpsTest, MakeMeshStorageBuildsValidatedStorageFromLoadedData) {
    pgo::VolumetricMeshes::storage::MeshStorage storage =
        pgo::VolumetricMeshes::ops::make_mesh_storage(make_loaded_tet_data());
    ...
    const auto* material = pgo::VolumetricMeshes::try_get_material<pgo::VolumetricMeshes::EnuMaterialData>(
        storage.material_catalog().element_material(0));
...
TEST(MeshConstructionOpsTest, AssignCommonLoadedDataReplacesTetGeometryAndMaterialCatalog) {
    ...
    pgo::VolumetricMeshes::ops::assign_common_loaded_data(mesh, make_loaded_tet_data());
