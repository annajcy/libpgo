function(_pgo_add_compile_option_for_language tgt platform mode domain language compiler_id flag)
  string(TOUPPER "${mode}" mode_upper)

  if(platform STREQUAL compiler_id)
    if(mode_upper STREQUAL "ALL")
      target_compile_options(${tgt} ${domain} "$<$<COMPILE_LANGUAGE:${language}>:${flag}>")
    else()
      target_compile_options(${tgt} ${domain} "$<$<AND:$<CONFIG:${mode_upper}>,$<COMPILE_LANGUAGE:${language}>>:${flag}>")
    endif()
  endif()
endfunction()

function(add_flag tgt platform mode domain flag)
  _pgo_add_compile_option_for_language(${tgt} ${platform} ${mode} ${domain} CXX "${CMAKE_CXX_COMPILER_ID}" ${flag})
  _pgo_add_compile_option_for_language(${tgt} ${platform} ${mode} ${domain} C "${CMAKE_C_COMPILER_ID}" ${flag})
endfunction()
...
function(add_flag_posix tgt domain flag)
  add_flag_poxis(${tgt} ${domain} ${flag})
endfunction()

function(add_flag_cuda tgt platform mode domain flag)
  _pgo_add_compile_option_for_language(${tgt} ${platform} ${mode} ${domain} CUDA "${CMAKE_C_COMPILER_ID}" ${flag})
endfunction()
