...
int main(int argc, char* argv[]) {
    ...
    cgal_smooth_cmd.add_description("Use CGAL angle_and_area_smoothing() to remesh surface");
    cgal_smooth_cmd.add_argument("-i", "--input-mesh").help("Input surface mesh filename").required().metavar("PATH");
    cgal_smooth_cmd.add_argument("-o", "--output-mesh").help("Output surface mesh filename").required().metavar("PATH");
    ...
    cgal_iso_cmd.add_description("Use CGAL isotropic_remeshing() to remesh surface");
    cgal_iso_cmd.add_argument("-i", "--input-mesh").help("Input surface mesh filename").required().metavar("PATH");
    cgal_iso_cmd.add_argument("-o", "--output-mesh").help("Output surface mesh filename").required().metavar("PATH");
    ...
    cgal_simplify_cmd.add_description("Use CGAL edge_collapse() to simplify surface");
    cgal_simplify_cmd.add_argument("-i", "--input-mesh").help("Input surface mesh filename").required().metavar("PATH");
    ...
    geogram_cmd.add_argument("-i", "--input-mesh").help("Input surface mesh filename").required().metavar("PATH");
    geogram_cmd.add_argument("-o", "--output-mesh").help("Output surface mesh filename").required().metavar("PATH");
    ...
    if (program.is_subcommand_used(cgal_smooth_cmd)) {
        ...
        pgo::Mesh::TriMeshGeo meshOut = pgo::CGALInterface::smoothMesh(inputMesh, 10, angleThreshold);
        ...
        for (int ti = 0; ti < inputMesh.numTriangles(); ti++) {
            for (int vi = 0; vi < 3; vi++) {
                pgo::EigenSupport::V3d diff = inputMesh.pos(ti, (vi + 1) % 3) - inputMesh.pos(ti, vi);
                avgLength += diff.norm();
            }
        }
        ...
        pgo::Mesh::TriMeshGeo meshOut = pgo::CGALInterface::simplifyMeshGH(inputMesh, "ptri", tgtRatio);
        ...
        pgo::Mesh::TriMeshGeo outputMesh      = pgo::GeogramInterface::remesh(
            geogram_cmd.get<std::string>("--input-mesh").c_str(), numTargetPoints, 0.8, -1.0);
        ...
        if constexpr (1) {
            ...
            for (int viewi = 0; viewi < 6; viewi++) {
                ...
                for (int vi = 0; vi < outputMesh.numVertices(); vi++) {
                    ...
                    if (outputMeshBVTree.hasLineSegmentIntersectionExact(outputMesh, segBeg, segEnd))
                        continue;
                ...
                std::cout << viewi << ":" << correctNormalCount << "/" << passThroughCount << std::endl;
