...
struct SurfaceSmoothnessAbsoluteMeanCurvatureBuf {
    SurfaceSmoothnessAbsoluteMeanCurvatureBuf(int numV) : locks(numV) {}
    std::vector<tbb::spin_mutex> locks;
};
}  // namespace PredefinedPotentialEnergies
}  // namespace pgo

SurfaceSmoothnessAbsoluteMeanCurvature::SurfaceSmoothnessAbsoluteMeanCurvature(const EigenSupport::VXd& restp,
    ...
    for (const auto& pr : edgeTriangles) {
        ...
        if (mesh.tri(pr.second[0])[vidxNext] == v1) {
            ...
            uint64_t vAll = (uint64_t)mesh.tri(pr.second[1])[0] ^ (uint64_t)mesh.tri(pr.second[1])[1] ^
                            (uint64_t)mesh.tri(pr.second[1])[2];
            ...
            // find the index for v3
            uint64_t vAll = (uint64_t)mesh.tri(pr.second[0])[0] ^ (uint64_t)mesh.tri(pr.second[0])[1] ^
                            (uint64_t)mesh.tri(pr.second[0])[2];
            v3 = (int)(vAll ^ (uint64_t)v0 ^ (uint64_t)v1);
        }
        if (!(v2 >= 0 && v3 >= 0)) {
            SPDLOG_LOGGER_ERROR(pgo::Logging::lgr(), "Input mesh is ill.");
...
// dH/dx = W
void SurfaceSmoothnessAbsoluteMeanCurvature::gradient(EigenSupport::ConstRefVecXd x,
