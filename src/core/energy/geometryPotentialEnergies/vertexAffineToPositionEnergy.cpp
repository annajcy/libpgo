...
VertexAffineToPositionEnergy::VertexAffineToPositionEnergy(int totalNumDOFs, const pgo::EigenSupport::VXd& restp,
                                                           std::shared_ptr<const PotentialEnergy> origE)
    : nAll(totalNumDOFs), restPosition(restp), originalEnergy(origE) {
    ...
    if (totalNumDOFs == (int)AToX.cols()) {
        ...
        std::set_difference(allDOFs.begin(), allDOFs.end(), positionDOFs.begin(), positionDOFs.end(),
                            std::back_inserter(restDOFs));
        ...
        for (ES::IDX rowi = 0; rowi < buf->hess_orig22.rows(); rowi++) {
            for (ES::SpMatD::InnerIterator it(buf->hess_orig22, rowi); it; ++it) {
                entries.emplace_back((int)buf->CTH11C.rows() + (int)it.row(), (int)buf->CTH11C.cols() + (int)it.col(),
                                     it.value());
            }
        }
...
void VertexAffineToPositionEnergy::compute_x_orig(pgo::EigenSupport::ConstRefVecXd x,
                                                  pgo::EigenSupport::RefVecXd      x_orig) const {
    // x = C A
...
void VertexAffineToPositionEnergy::gradient(pgo::EigenSupport::ConstRefVecXd x,
