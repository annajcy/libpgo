...
public:
    SmoothRSEnergy(const Mesh::TetMeshGeo& tetMesh, const EigenSupport::SpMatD& G, const EigenSupport::SpMatD& L,
                   const double coeffs[2], int isPosition = 1, int dofOffsets = 0);
...
protected:
    ...
    void computeBlockSparsePattern(const EigenSupport::SpMatD&                     LTL,
                                   std::unordered_map<std::pair<int, int>, MatrixType, EigenSupport::IntPairHash,
                                                      EigenSupport::IntPairEqual>& blocks) const;
