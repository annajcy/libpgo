...
TriangleAffineToPositionEnergy::TriangleAffineToPositionEnergy(int totalNumDOFs, const Mesh::TriMeshGeo& mesh,
                                                               std::shared_ptr<const PotentialEnergy> origE)
    : nAll(totalNumDOFs), inputMesh(mesh), originalEnergy(origE) {
...
void TriangleAffineToPositionEnergy::computeWA() {
        ...
        for (int j = 0; j < (int)nTris.size(); j++) {
            ...
            for (int k = 0; k < 3; k++) {
                if (inputMesh.triVtxID(nTris[j], k) == vi) {
                    std::get<1>(vertexNeighboringTriangleWeights[vi][j]) =
                        Mesh::getTriangleAngle(inputMesh.pos(nTris[j], k), inputMesh.pos(nTris[j], (k + 1) % 3),
                                               inputMesh.pos(nTris[j], (k + 2) % 3));
                    break;
                }
            }
        }
    ...
    for (int vi = 0; vi < inputMesh.numVertices(); vi++) {
        for (int ni = 0; ni < (int)vertexNeighboringTriangleWeights[vi].size(); ni++) {
            for (int k = 0; k < 12; k++) {
                entries.emplace_back(vi * 12 + k, std::get<0>(vertexNeighboringTriangleWeights[vi][ni]) * 12 + k,
                                     std::get<1>(vertexNeighboringTriangleWeights[vi][ni]));
            }
        }
    }
...
void TriangleAffineToPositionEnergy::compute_A_orig(pgo::EigenSupport::ConstRefVecXd A_tri,
                                                    pgo::EigenSupport::RefVecXd      A_orig) const {
    // x = C A
...
void TriangleAffineToPositionEnergy::gradient(pgo::EigenSupport::ConstRefVecXd A,
                                              pgo::EigenSupport::RefVecXd      grad) const {
    compute_A_orig(A, buf->A_orig);
...
void TriangleAffineToPositionEnergy::hessian(pgo::EigenSupport::ConstRefVecXd A,
                                             pgo::EigenSupport::SpMatD&       hess) const {
    compute_A_orig(A, buf->A_orig);
