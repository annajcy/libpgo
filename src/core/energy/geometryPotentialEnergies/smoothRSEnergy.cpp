...
struct SparseCSR {
    void init(const sparse_matrix_t& mat, int buildCopy) {
        sparse_status_t ret =
            mkl_sparse_d_export_csr(mat, &baseIndex, &numRows, &numCols, &rowStarts, &rowEnds, &colIndices, &values);
...
struct SmoothRSEnergyBuffer {
    SmoothRSEnergyBuffer(int n3, int nele) : blockTLS(n3, nele) {}
...
void SmoothRSEnergy::computeBlockSparsePattern(
    const ES::SpMatD&                                                                       LTL,
    std::unordered_map<std::pair<int, int>, MatrixType, ES::IntPairHash, ES::IntPairEqual>& blocks) const {
...
SmoothRSEnergy::SmoothRSEnergy(const Mesh::TetMeshGeo& tetMesh_, const ES::SpMatD& G_, const ES::SpMatD& L,
                               const double coeffs[2], int isPosition, int doff)
...
ES::M3d SmoothRSEnergy::getMat(const ES::VXd& matVec, int ei) const {
    ...
    F << FVecLocal[0], FVecLocal[3], FVecLocal[6], FVecLocal[1], FVecLocal[4], FVecLocal[7], FVecLocal[2], FVecLocal[5],
        FVecLocal[8];
...
void SmoothRSEnergy::gradient(EigenSupport::ConstRefVecXd x, EigenSupport::RefVecXd grad) const {
    ...
    tbb::parallel_for(
        ...
        [&](int ele) {
            ...
            NonlinearOptimization::PolarDecompositionDerivatives::computeFirstOrderDerivatives(Flocal, Rlocal, Slocal,
                                                                                               SInv, dRdFi, dSdFi);
            ...
            dEdF.segment<9>(ele * 9) =
                dSdFT * LTLS.segment<9>(ele * 9) * coeffS + dRdFT * LTLR.segment<9>(ele * 9) * coeffR;
        },
...
void SmoothRSEnergy::hessian(EigenSupport::ConstRefVecXd x, EigenSupport::SpMatD& hess) const {
    ...
    tbb::parallel_for(
        ...
        [&](int ele) {
            ...
            ES::M3d dsqrtA_dA[9];
            ES::M9d MInv;
            NonlinearOptimization::PolarDecompositionDerivatives::computeFirstOrderDerivatives(
                Flocal, Rlocal, Slocal, SInv, dRdFi, dSdFi, dFdFi, dAdFi, dsqrtA_dA, &MInv);

            ES::M3d d2RdF2i[9][9];
            ES::M3d d2SdF2i[9][9];
            NonlinearOptimization::PolarDecompositionDerivatives::computeSecondOrderDerivatives(
                Flocal, Rlocal, Slocal, SInv, MInv, dRdFi, dSdFi, dFdFi, dAdFi, dsqrtA_dA, d2SdF2i, d2RdF2i);
            ...
            for (int i = 0; i < 9; i++) {
                for (int j = 0; j < 9; j++) {
                    ...
                    d2EdF2(i, j) = dSdFidFj.dot(LTLS.segment<9>(ele * 9)) * coeffS +
                                   dRdFidFj.dot(LTLR.segment<9>(ele * 9)) * coeffR;
                }
            }
    ...
    cblas_daxpy(evaluationBuf->h1.nonZeros(), coeffR, evaluationBuf->h3.valuePtr(), coeffS,
                evaluationBuf->h1.valuePtr(), 1);
