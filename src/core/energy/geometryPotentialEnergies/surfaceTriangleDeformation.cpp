...
struct SurfaceTriangleDeformationBuf {
    SurfaceTriangleDeformationBuf(int numV) : locks(numV) {}
    std::vector<tbb::spin_mutex> locks;
};
}  // namespace PredefinedPotentialEnergies
}  // namespace pgo

SurfaceTriangleDeformation::SurfaceTriangleDeformation(const EigenSupport::VXd& restp, const Mesh::TriMeshGeo& meshIn,
                                                       double eps)
    ...
    for (int ei = 0; ei < mesh.numTriangles(); ei++) {
        for (int vi = 0; vi < 3; vi++) {
            ...
            for (int vj = 0; vj < 3; vj++) {
                ...
                for (int r = 0; r < 3; r++) {
                    for (int c = 0; c < 3; c++) {
                        elementOffsets[ei](vi * 3 + r, vj * 3 + c) =
                            ES::findEntryOffset(hessTemplate, vidx_i * 3 + r, vidx_j * 3 + c);
...
void SurfaceTriangleDeformation::updateRestInfo() {
    tbb::parallel_for(0, mesh.numTriangles(), [&](int ei) {
        ...
        ES::M3x2d F;
        F.col(0) =
            restPositions.segment<3>(mesh.triVtxID(ei, 1) * 3) - restPositions.segment<3>(mesh.triVtxID(ei, 0) * 3);
        F.col(1) =
            restPositions.segment<3>(mesh.triVtxID(ei, 2) * 3) - restPositions.segment<3>(mesh.triVtxID(ei, 0) * 3);
    ...
    double energyAll = tbb::parallel_reduce(
        ...
        [&](const tbb::blocked_range<int>& r, double init) -> double {
            for (int ei = r.begin(); ei != r.end(); ++ei) {
                ...
                // double energy = (S(0) - S(1)) * (S(0) - S(1)) * coeffs[0] + (S - ES::V2d::Ones()).squaredNorm() *
                // coeffs[1];
            ...
            for (int r0 = 0; r0 < 3; r0++) {
                ...
                for (int c1 = 0; c1 < 2; c1++) {
                    for (int r1 = 0; r1 < 3; r1++) {
                        ...
                        ES::M2d d2AdFidFj =
                            elementAbarInv[ei] * (dFi.transpose() * dFj + dFj.transpose() * dFi) * elementAbarInv[ei];
