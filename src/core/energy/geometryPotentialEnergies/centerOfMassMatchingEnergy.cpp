...
autodiff::var determinant(const autodiff::Vector3var& a, const autodiff::Vector3var& b, const autodiff::Vector3var& c,
                          const autodiff::Vector3var& d) {
    return dot(d - a, cross(b - a, c - a));
}

autodiff::Vector3var centerOfMassNumerator(const autodiff::Vector3var& a, const autodiff::Vector3var& b,
                                           const autodiff::Vector3var& c, const autodiff::Vector3var& d) {
...
autodiff::var centerOfMassDenominator(const autodiff::Vector3var& a, const autodiff::Vector3var& b,
                                      const autodiff::Vector3var& c, const autodiff::Vector3var& d) {
...
CenterOfMassMatchingEnergy::CenterOfMassMatchingEnergy(const pgo::Mesh::TetMeshGeo& tetMesh, const ES::V3d& tgtCoM,
                                                       const ES::M3d& projMat) {
    ES::MXd vtxInit;
    ES::MXi tetInit;
...
double CenterOfMassMatchingEnergy::func(EigenSupport::ConstRefVecXd x) const {
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, m_tet.rows()), [&](const tbb::blocked_range<int>& range) {
        for (int ele = range.begin(); ele < range.end(); ++ele) {
            ...
            combNumerator.local() +=
                ES::V3d(autodiff::val(valNumerator(0)), autodiff::val(valNumerator(1)), autodiff::val(valNumerator(2)));
...
void CenterOfMassMatchingEnergy::compute_com_and_energy_and_grad(ES::ConstRefVecXd x, ES::V3d& com, double& energy,
    ...
    if (0) {
        ...
        tbb::parallel_for(tbb::blocked_range<int>(0, m_tet.rows()), [&](const tbb::blocked_range<int>& range) {
            for (int ele = range.begin(); ele < range.end(); ++ele) {
                ...
                combNumerator.local() += ES::V3d(autodiff::val(valNumerator(0)), autodiff::val(valNumerator(1)),
                                                 autodiff::val(valNumerator(2)));
                combDenominator.local() += autodiff::val(valDenominator);
