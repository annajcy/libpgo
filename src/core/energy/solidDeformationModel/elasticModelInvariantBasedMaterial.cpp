...
double ElasticModelInvariantBasedMaterial::compute_psi(const double* /*param*/, const double*, const double*,
                                                       const double*, const double S[3]) const {
...
void ElasticModelInvariantBasedMaterial::compute_P(const double* /*param*/, const double*, const double U[9],
                                                   const double V[9], const double S[3], double POut[9]) const {
    double lambda2[3] = {S[0] * S[0], S[1] * S[1], S[2] * S[2]};
    ...
    (Eigen::Map<Eigen::Matrix3d>(POut)) =
        Eigen::Map<const Eigen::Matrix3d>(U) * PDiag.asDiagonal() * Eigen::Map<const Eigen::Matrix3d>(V).transpose();
}
...
inline double gammaValue(int i, int j, const double sigma[3], const double invariants[3], const double gradient[3],
                         const double hessian[6]) {
    ...
    productResult[0] = (tempGammaVec2[0] * hessian[0] + tempGammaVec2[1] * hessian[1] + tempGammaVec2[2] * hessian[2]);
    productResult[1] = (tempGammaVec2[0] * hessian[1] + tempGammaVec2[1] * hessian[3] + tempGammaVec2[2] * hessian[4]);
    productResult[2] = (tempGammaVec2[0] * hessian[2] + tempGammaVec2[1] * hessian[4] + tempGammaVec2[2] * hessian[5]);
...
inline void fixPositiveIndefiniteness(double& A11, double& A12, double& A13, double& A22, double& A23, double& A33) {
    Eigen::Matrix3d mat;
    mat << A11, A12, A13, A12, A22, A23, A13, A23, A33;
...
void ElasticModelInvariantBasedMaterial::compute_dPdF(const double* /*param*/, const double*, const double UIn[9],
                                                      const double VIn[9], const double S[3],
    ...
    invariants[1] = (sigma1square * sigma1square + sigma2square * sigma2square + sigma3square * sigma3square);
    ...
    */
    ...
    (Eigen::Map<Eigen::Matrix<double, 9, 9>>(dPdFOut)) =
        perm.transpose() * (Eigen::Map<const Eigen::Matrix<double, 9, 9>>(dPdF)) * perm;
}
