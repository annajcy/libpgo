...
public:
    ...
    ES::M2d compute_a_and_derivatives(const ES::V3d x[3], Eigen::Matrix<double, 4, 9>* da_dx, ES::M9d ahess[4]);

    ES::M2d compute_b_and_derivatives(const ES::V3d x[6], int hasVtx[6], Eigen::Matrix<double, 4, 18>* db_dx,
                                      ES::M18d bhess[4]);

    ES::V3d secondFundamentalFormEntries(const ES::V3d x[6], int hasVtx[6], Eigen::Matrix<double, 3, 18>* derivative,
                                         ES::M18d hessian[3]);
    ...
    ES::V3d faceNormal(const ES::V3d x0, const ES::V3d x1, const ES::V3d x2, Eigen::Matrix<double, 3, 9>* derivative,
                       ES::M9d hessian[3]);
};
...
void KoiterDeformationModel::prepareData(const double* x, const double* param, const double* materialParam,
...
double KoiterDeformationModel::computeEnergy(const CacheData* cacheDataBase) const {
    const KoiterDeformationModelCacheData* cacheData =
        dynamic_cast<const KoiterDeformationModelCacheData*>(cacheDataBase);

    double E1 =
        ind->elasticModel->compute_psi_a(cacheData->elasticParams.data(), cacheData->a.data(), cacheData->abar.data());
    double E2 = ind->elasticModel->compute_psi_b(cacheData->elasticParams.data(), cacheData->b.data(),
...
void KoiterDeformationModel::compute_dE_dx(const CacheData* cacheDataBase, double* grad) const {
    ...
    ES::M2d dEda;
    ES::M2d dEdb;
    ind->elasticModel->compute_dpsi_da(cacheData->elasticParams.data(), cacheData->a.data(), cacheData->abar.data(),
                                       dEda.data());
    ind->elasticModel->compute_dpsi_db(cacheData->elasticParams.data(), cacheData->b.data(), cacheData->abar.data(),
                                       cacheData->bbar.data(), dEdb.data());
...
void KoiterDeformationModel::compute_d2E_dx2(const CacheData* cacheDataBase, double* hess) const {
    ...
    ES::M4d d2Eda2;
    ES::M4d d2Edb2;
    ind->elasticModel->compute_d2psi_da2(cacheData->elasticParams.data(), cacheData->a.data(), cacheData->abar.data(),
                                         d2Eda2.data());
    ind->elasticModel->compute_d2psi_db2(cacheData->elasticParams.data(), cacheData->b.data(), cacheData->abar.data(),
                                         cacheData->bbar.data(), d2Edb2.data());
...
void KoiterDeformationModel::compute_d2E_dxda(const CacheData* cacheDataBase, double* hess) const {
    ...
    ind->elasticModel->compute_dpsi_da(cacheData->elasticParams.data(), cacheData->a.data(), cacheData->abar.data(),
                                       dpsi_da.data());
    ind->elasticModel->compute_dpsi_db(cacheData->elasticParams.data(), cacheData->b.data(), cacheData->abar.data(),
                                       cacheData->bbar.data(), dpsi_db.data());

    ES::M4d d2psi_da_dabar, d2psi_db_dabar, d2psi_db_dbbar;
    ind->elasticModel->compute_d2psi_dadabar(cacheData->elasticParams.data(), cacheData->a.data(),
                                             cacheData->abar.data(), d2psi_da_dabar.data());
    ind->elasticModel->compute_d2psi_db_dabar(cacheData->elasticParams.data(), cacheData->b.data(),
                                              cacheData->abar.data(), cacheData->bbar.data(), d2psi_db_dabar.data());
    ind->elasticModel->compute_d2psi_db_dbbar(cacheData->elasticParams.data(), cacheData->b.data(),
    ...
    hessMap.block(0, 0, 9, pm->getNumParameters()) +=
        dadx.transpose() * d2psi_da_dabar * dabar_dF.leftCols(pm->getNumParameters()) * cacheData->area;
    hessMap.block(0, 0, 9, pm->getNumParameters()) += dpsi_a_dx * darea_dF.head(pm->getNumParameters()).transpose();

    hessMap.block(0, 0, 18, pm->getNumParameters()) +=
        dbdx.transpose() * d2psi_db_dabar * dabar_dF.leftCols(pm->getNumParameters()) * cacheData->area;
    hessMap.block(0, 0, 18, pm->getNumParameters()) +=
        dbdx.transpose() * d2psi_db_dbbar * dbbar_dF.leftCols(pm->getNumParameters()) * cacheData->area;
...
void KoiterDeformationModel::compute_d2E_dxdb(const CacheData* cacheDataBase, double* hess) const {
    ...
    ind->elasticModel->compute_d2psi_da_dparam(cacheData->elasticParams.data(), cacheData->a.data(),
                                               cacheData->abar.data(), d2psi_da_dparam.data());
    ind->elasticModel->compute_d2psi_db_dparam(cacheData->elasticParams.data(), cacheData->b.data(),
    ...
    hessMap.block(0, 0, 9, em->getNumParameters()) +=
        dadx.transpose() * d2psi_da_dparam.leftCols(em->getNumParameters()) * cacheData->area;
    hessMap.block(0, 0, 18, em->getNumParameters()) +=
        dbdx.transpose() * d2psi_db_dparam.leftCols(em->getNumParameters()) * cacheData->area;
}
...
ES::V3d KoiterDeformationModelInternal::secondFundamentalFormEntries(const ES::V3d x[6], int hasVtx[6],
    ...
    ES::M9d                     hcn[3];
    ES::V3d cNormal = faceNormal(x[0], x[1], x[2], (derivative || hessian) ? &dcn : nullptr, hessian ? hcn : nullptr);
    ...
    for (int i = 0; i < 3; i++) {
        if (hasVtx[oppVtx[i]] == 0) {
            ...
            oppNormals[i] =
                faceNormal(x0, x1, x2, (derivative || hessian) ? &dn[i] : nullptr, hessian ? hn[i] : nullptr);
        }
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (derivative) {
            ...
            derivative->block<1, 3>(i, 9 + 3 * (oppVtx[i] - 3)) +=
                qvec[i].transpose() / mnorms[i] * dn[i].block<3, 3>(0, 0);
            ...
            derivative->block<1, 3>(i, 9 + 3 * (oppVtx[i] - 3)) += -qvec[i].dot(oppNormals[i]) / mnorms[i] / mnorms[i] /
                                                                   mnorms[i] * mvec[i].transpose() *
                                                                   dn[i].block<3, 3>(0, 0);
            derivative->block<1, 3>(i, 3 * ip2) += -qvec[i].dot(oppNormals[i]) / mnorms[i] / mnorms[i] / mnorms[i] *
                                                   mvec[i].transpose() * dn[i].block<3, 3>(0, 3);
            derivative->block<1, 3>(i, 3 * ip1) += -qvec[i].dot(oppNormals[i]) / mnorms[i] / mnorms[i] / mnorms[i] *
                                                   mvec[i].transpose() * dn[i].block<3, 3>(0, 6);

            derivative->block<1, 3>(i, 0) += -qvec[i].dot(oppNormals[i]) / mnorms[i] / mnorms[i] / mnorms[i] *
                                             mvec[i].transpose() * dcn.block<3, 3>(0, 0);
            derivative->block<1, 3>(i, 3) += -qvec[i].dot(oppNormals[i]) / mnorms[i] / mnorms[i] / mnorms[i] *
                                             mvec[i].transpose() * dcn.block<3, 3>(0, 3);
            derivative->block<1, 3>(i, 6) += -qvec[i].dot(oppNormals[i]) / mnorms[i] / mnorms[i] / mnorms[i] *
                                             mvec[i].transpose() * dcn.block<3, 3>(0, 6);
        }
        ...
        if (hessian) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                for (int k = 0; k < 3; k++) {
                    hessian[i].block<3, 3>(miidx[j], miidx[k]) += (-1.0 / mnorms[i] / mnorms[i] / mnorms[i]) *
                                                                  (dnij[j].transpose() * mvec[i]) *
                                                                  (qvec[i].transpose() * dnij[k]);
                    hessian[i].block<3, 3>(3 * j, miidx[k]) += (-1.0 / mnorms[i] / mnorms[i] / mnorms[i]) *
                                                               (dcn.block<3, 3>(0, 3 * j).transpose() * mvec[i]) *
                                                               (qvec[i].transpose() * dnij[k]);
                }
                ...
                for (int k = 0; k < 3; k++) {
                    hessian[i].block<3, 3>(miidx[j], miidx[k]) +=
                        (-1.0 / mnorms[i] / mnorms[i] / mnorms[i]) * dnijTq * (mvec[i].transpose() * dnij[k]);
                    hessian[i].block<3, 3>(miidx[j], 3 * k) += (-1.0 / mnorms[i] / mnorms[i] / mnorms[i]) * dnijTq *
                                                               (mvec[i].transpose() * dcn.block<3, 3>(0, 3 * k));
                }
                ...
                for (int k = 0; k < 3; k++) {
                    hessian[i].block<3, 3>(miidx[j], miidx[k]) +=
                        (-1.0 / mnorms[i] / mnorms[i] / mnorms[i]) * qdoto * dnij[j].transpose() * dnij[k];
                    hessian[i].block<3, 3>(miidx[j], 3 * k) += (-1.0 / mnorms[i] / mnorms[i] / mnorms[i]) * qdoto *
                                                               dnij[j].transpose() * dcn.block<3, 3>(0, 3 * k);
                    hessian[i].block<3, 3>(3 * j, miidx[k]) += (-1.0 / mnorms[i] / mnorms[i] / mnorms[i]) * qdoto *
                                                               dcn.block<3, 3>(0, 3 * j).transpose() * dnij[k];
                    hessian[i].block<3, 3>(3 * j, 3 * k) += (-1.0 / mnorms[i] / mnorms[i] / mnorms[i]) * qdoto *
                                                            dcn.block<3, 3>(0, 3 * j).transpose() *
                                                            dcn.block<3, 3>(0, 3 * k);

                    hessian[i].block<3, 3>(miidx[j], miidx[k]) +=
                        (3.0 / mnorms[i] / mnorms[i] / mnorms[i] / mnorms[i] / mnorms[i]) * qdoto * dnijTm *
                        (mvec[i].transpose() * dnij[k]);
                    hessian[i].block<3, 3>(miidx[j], 3 * k) +=
                        (3.0 / mnorms[i] / mnorms[i] / mnorms[i] / mnorms[i] / mnorms[i]) * qdoto * dnijTm *
                        (mvec[i].transpose() * dcn.block<3, 3>(0, 3 * k));
                    hessian[i].block<3, 3>(3 * j, miidx[k]) +=
                        (3.0 / mnorms[i] / mnorms[i] / mnorms[i] / mnorms[i] / mnorms[i]) * qdoto * dcnjTm *
                        (mvec[i].transpose() * dnij[k]);
                    hessian[i].block<3, 3>(3 * j, 3 * k) +=
                        (3.0 / mnorms[i] / mnorms[i] / mnorms[i] / mnorms[i] / mnorms[i]) * qdoto * dcnjTm *
                        (mvec[i].transpose() * dcn.block<3, 3>(0, 3 * k));
                    ...
                    for (int l = 0; l < 3; l++) {
                        hessian[i].block<3, 3>(miidx[j], miidx[k]) += (-1.0 / mnorms[i] / mnorms[i] / mnorms[i]) *
                                                                      qdoto * mvec[i][l] *
                                                                      hn[i][l].block<3, 3>(3 * j, 3 * k);
                        hessian[i].block<3, 3>(3 * j, 3 * k) += (-1.0 / mnorms[i] / mnorms[i] / mnorms[i]) * qdoto *
                                                                mvec[i][l] * hcn[l].block<3, 3>(3 * j, 3 * k);
                        hessian[i].block<3, 3>(miidx[j], miidx[k]) +=
                            (1.0 / mnorms[i]) * qvec[i][l] * hn[i][l].block<3, 3>(3 * j, 3 * k);
                    }
                }
            }
        }
...
ES::M2d KoiterDeformationModelInternal::compute_b_and_derivatives(const ES::V3d x[6], int hasVtx[6],
    ...
    ES::V3d II = secondFundamentalFormEntries(x, hasVtx, db_dx ? &IIderiv : nullptr, bhess ? IIhess : nullptr);
...
ES::V3d KoiterDeformationModelInternal::faceNormal(const ES::V3d x0, const ES::V3d x1, const ES::V3d x2,
                                                   Eigen::Matrix<double, 3, 9>* derivative, ES::M9d hessian[3]) {
    if (derivative)
        derivative->setZero();
