...
inline ES::M3d H_I1bar_apply(const ES::M3d& F, const ES::M3d& Finv, const ES::M3d& FinvT, double J, double Jm23,
                             double I1, const ES::M3d& dF) {
    // Precomputations
    ...
    //           + I1 * (-2/3) δ(J^{-2/3}) F^{-T}
    //           + I1 * (-2/3) J^{-2/3} δ(F^{-T})
    return dJm23 * gradI1 + Jm23 * (2.0 * dF) + dI1 * (-2.0 / 3.0) * Jm23 * FinvT + I1 * (-2.0 / 3.0) * dJm23 * FinvT +
           I1 * (-2.0 / 3.0) * Jm23 * dFinvT;
}

inline ES::M3d H_I2bar_apply(const ES::M3d& F, const ES::M3d& C, const ES::M3d& Finv, const ES::M3d& FinvT, double J,
                             double Jm43, double I1, double I2, const ES::M3d& dF) {
    ...
    return dJm43 * gradI2 + Jm43 * HI2 + dI2 * (-4.0 / 3.0) * Jm43 * FinvT + I2 * (-4.0 / 3.0) * dJm43 * FinvT +
           I2 * (-4.0 / 3.0) * Jm43 * dFinvT;
}
}  // namespace pgo::SolidDeformationModel
...
double ElasticModel3DMooneyRivlin::compute_psi(const double* param, const double _F[9], const double U[9],
...
void ElasticModel3DMooneyRivlin::compute_P(const double* param, const double _F[9], const double U[9],
...
void ElasticModel3DMooneyRivlin::compute_dPdF(const double* param, const double _F[9], const double U[9],
                                              const double V[9], const double S[3], double dPdF[81]) const {
    const ES::M3d F = ES::Mp<const ES::M3d>(_F);
