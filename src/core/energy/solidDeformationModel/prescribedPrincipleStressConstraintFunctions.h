...
public:
    PrescribedPrincipleStressConstraintFunctions(int nAll, int dofOffset, int numElements, const int* elementIDs,
    ...
    void setTargetPHat(const double* phat) {
        targetPrincipleStress = EigenSupport::Mp<const EigenSupport::VXd>(phat, elements.size() * 3);
    }

    void   computeForceFromTargetPHat(EigenSupport::ConstRefVecXd x, EigenSupport::RefVecXd fext) const;
    double computeSurfaceNormalTractionFromElement(EigenSupport::ConstRefVecXd x, const EigenSupport::V3d& n,
                                                   int eleID) const;
