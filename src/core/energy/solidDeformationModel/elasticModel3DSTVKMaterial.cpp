...
ElasticModel3DSTVKMaterial::ElasticModel3DSTVKMaterial(double _mu, double _lambda) : mu(_mu), lambda(_lambda) {
    C[0] << 0, -1, 0, 1, 0, 0, 0, 0, 0;
    C[1] << 0, 0, 0, 0, 0, 1, 0, -1, 0;
    C[2] << 0, 0, 1, 0, 0, 0, -1, 0, 0;
}

ElasticModel3DSTVKMaterial::~ElasticModel3DSTVKMaterial() {}

ES::V3d ElasticModel3DSTVKMaterial::computeLowerInvariance(const ES::M3d& F, ES::M3d& R, EigenSupport::V3d* S_,
                                                           EigenSupport::M3d* U_, EigenSupport::M3d* V_) const {
...
double ElasticModel3DSTVKMaterial::compute_psi(const double* param, const double _F[9], const double _U[],
                                               const double _V[], const double _S[]) const {
    ...
    double energy = lambda / 8.0 * (I2 - 3) * (I2 - 3) +
                    mu / 8.0 * (8 * I1 * I3 + I2 * I2 + 2 * I1 * I1 * I2 - 4 * I2 - I1 * I1 * I1 * I1 + 6);

    return energy;
}

void ElasticModel3DSTVKMaterial::compute_P(const double* param, const double _F[9], const double _U[],
                                           const double _V[], const double _S[], double P[9]) const {
    ES::M3d F = Eigen::Map<const ES::M3d>(_F), R;
    ES::V3d i = computeLowerInvariance(F, R);
...
void ElasticModel3DSTVKMaterial::compute_dPdF(const double* param, const double _F[9], const double _U[],
                                              const double _V[], const double _S[], double dPdF[81]) const {
