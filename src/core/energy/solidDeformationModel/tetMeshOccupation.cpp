...
void pgo::SolidDeformationModel::computeTetMeshOccupation(int numTetVertices, const Vec3d* tetVertices, int numTets,
                                                          const Vec4i* tets, int numSurfaceVertices,
                                                          const Vec3d* surfaceVertices, int numTriangles,
                                                          const Vec3i* triangles, double minThreshold, int sampleCount,
                                                          double* weights) {
...
void pgo::SolidDeformationModel::computeTetMeshOccupation(int numTetVertices, const Vec3d* tetVertices, int numTets,
                                                          const Vec4i* tets, const Vec3d bbIn[2], double minThreshold,
                                                          int sampleCount, double* weights) {
    std::random_device                     rd;
    std::mt19937                           gen(rd());
    std::uniform_real_distribution<double> randomNumber(0.0, 1.0);
    std::atomic<int>                       numFinished(0);
