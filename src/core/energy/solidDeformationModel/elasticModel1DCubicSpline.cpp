...
ElasticModel1DCubicSpline::ElasticModel1DCubicSpline(double coeff, int nPoints, double xLeft, double xRight) {
    ...
    impl->spline = std::make_shared<NonlinearOptimization::NaturalCubicSpline2DWithParameterDerivatives>(
        nPoints, impl->xValues.data());
    impl->c = coeff;
}
...
double ElasticModel1DCubicSpline::compute_psi(const double* param, const double F[9], const double[], const double[],
...
void ElasticModel1DCubicSpline::compute_P(const double* param, const double F[9], const double[], const double[],
                                          const double[], double            P[]) const {
    double x = F[0] - F[1];
    P[0]     = impl->spline->dy_dx(x, param) * impl->c;
}

void ElasticModel1DCubicSpline::compute_dPdF(const double* param, const double F[9], const double[], const double[],
                                             const double[], double            dPdF[]) const {
    double x = F[0] - F[1];
...
double ElasticModel1DCubicSpline::compute_dpsi_dparam(const double* param, int i, const double F[], const double[],
...
double ElasticModel1DCubicSpline::compute_d2psi_dparam2(const double* param, int i, int j, const double F[],
...
void ElasticModel1DCubicSpline::compute_dP_dparam(const double* param, int i, const double F[], const double[],
                                                  const double[], const double[], double*  ret) const {
    double x = F[0] - F[1];
