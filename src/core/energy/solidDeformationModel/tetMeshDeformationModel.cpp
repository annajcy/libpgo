...
TetMeshDeformationModel::TetMeshDeformationModel(const double X0[3], const double X1[3], const double X2[3],
                                                 const double X3[3], ElasticModel* elasticModel,
...
void TetMeshDeformationModel::computeF(const double* x, int materialLocationIDs, double F[9]) const {
    ES::V3d xs[4] = {ES::V3d(x[0], x[1], x[2]), ES::V3d(x[3], x[4], x[5]), ES::V3d(x[6], x[7], x[8]),
...
void TetMeshDeformationModel::computeP(const CacheData* cacheDataBase, int materialLocationIDs, double POut[9]) const {
    const TetMeshDeformationModelCacheData* cacheData =
        static_cast<const TetMeshDeformationModelCacheData*>(cacheDataBase);

    ES::M3d P;
    ind->elasticModel->compute_P(cacheData->materialParam.data(), cacheData->Fe.data(), cacheData->U.data(),
                                 cacheData->V.data(), cacheData->S.data(), P.data());

    (ES::Mp<ES::M3d>(POut)) = P;
}

void TetMeshDeformationModel::computedPdF(const CacheData* cacheDataBase, int materialLocationIDs,
                                          double dPdFOut[81]) const {
    const TetMeshDeformationModelCacheData* cacheData =
        static_cast<const TetMeshDeformationModelCacheData*>(cacheDataBase);

    ES::M9d dPdF;
    ind->elasticModel->compute_dPdF(cacheData->materialParam.data(), cacheData->Fe.data(), cacheData->U.data(),
                                    cacheData->V.data(), cacheData->S.data(), dPdF.data());

    (Eigen::Map<ES::M9d>(dPdFOut)) = dPdF;
}

void TetMeshDeformationModel::computedFdx(const CacheData* cacheDataBase, int materialLocationIDs,
                                          double* dFdxOut) const {
    const TetMeshDeformationModelCacheData* cacheData =
        static_cast<const TetMeshDeformationModelCacheData*>(cacheDataBase);
...
void TetMeshDeformationModel::prepareData(const double* x, const double* param, const double* materialParam,
    ...
    ind->plasticModel->compute_ddetA_da(cacheData->plasticParam.data(), cacheData->ddetA_da.data(),
                                        TetMeshDeformationModelCacheData::maxNumPlasticParam);
...
double TetMeshDeformationModel::computeEnergy(const CacheData* cacheDataBase) const {
    ...
    return ind->elasticModel->compute_psi(cacheData->materialParam.data(), cacheData->Fe.data(), cacheData->U.data(),
                                          cacheData->V.data(), cacheData->S.data()) *
...
void TetMeshDeformationModel::vonMisesStress(const CacheData* cacheDataBase, int& nPt, double* stresses) const {
    ...
    double t4 = 6 * (cauchyStress(1, 2) * cauchyStress(1, 2) + cauchyStress(2, 0) * cauchyStress(2, 0) +
...
void TetMeshDeformationModel::compute_d2E_da2(const CacheData* cacheDataBase, double* hess) const {
    ...
    for (int i = 0; i < ind->plasticModel->getNumParameters(); i++) {
        for (int j = i; j < ind->plasticModel->getNumParameters(); j++) {
            d2psi_da2(i, j) =
                ind->compute_d2psi_dai_daj(cacheData->Ds, cacheData->dAInv_dai[i], cacheData->dAInv_dai[j],
    ...
    for (int i = 0; i < ind->plasticModel->getNumParameters(); i++) {
        for (int j = 0; j < ind->plasticModel->getNumParameters(); j++) {
            hessMap(i, j) =
                d2volda2(i, j) * psi + dvolda[i] * dpsi_da[j] + dvolda[j] * dpsi_da[i] + vol * d2psi_da2(i, j);
        }
    }
}
...
void TetMeshDeformationModel::compute_d2E_dxda(const CacheData* cacheDataBase, double* hess) const {
    ...
    (Eigen::Map<ES::MXd>(hess, 12, ind->plasticModel->getNumParameters())) =
        d2E_duda.block(0, 0, 12, ind->plasticModel->getNumParameters());
}
...
void TetMeshDeformationModel::compute_dE_db(const CacheData* cacheDataBase, double* grad) const {
    ...
    for (int i = 0; i < ind->elasticModel->getNumParameters(); i++) {
        double val =
            ind->elasticModel->compute_dpsi_dparam(cacheData->materialParam.data(), i, cacheData->Fe.data(),
                                                   cacheData->U.data(), cacheData->V.data(), cacheData->S.data());
        grad[i] = val * V;
    }
}
...
void TetMeshDeformationModel::compute_d2E_db2(const CacheData* cacheDataBase, double* hess) const {
    ...
    for (int i = 0; i < ind->elasticModel->getNumParameters(); i++) {
        for (int j = 0; j < ind->elasticModel->getNumParameters(); j++) {
            double val =
                ind->elasticModel->compute_d2psi_dparam2(cacheData->materialParam.data(), i, j, cacheData->Fe.data(),
                                                         cacheData->U.data(), cacheData->V.data(), cacheData->S.data());
...
void TetMeshDeformationModel::compute_d2E_dadb(const CacheData* cacheDataBase, double* hess) const {
    ...
    for (int i = 0; i < ind->plasticModel->getNumParameters(); i++) {
        for (int j = 0; j < ind->elasticModel->getNumParameters(); j++) {
            d2E_dadb(i, j) +=
                Eigen::Map<const ES::V9d>(dPdb[j].data()).dot(Eigen::Map<const ES::V9d>(dFda[i].data())) * V;
        }
    }
}
...
void TetMeshDeformationModelInternal::compute_dFe_dai(const ES::M3d& Ds, const ES::M3d& dAInvdai,
                                                      ES::M3d& dFdai) const {
    // F * d(Fp^-1)_dai
    dFdai = Ds * restDmInv * dAInvdai;
}

void TetMeshDeformationModelInternal::compute_d2Fe_dai_daj(const ES::M3d& Ds, const ES::M3d& dAInvdaidaj,
                                                           ES::M3d& d2Fdaidaj) const {
    // F * d2(Fp^-1)_dai_daj
    d2Fdaidaj = Ds * restDmInv * dAInvdaidaj;
}

void TetMeshDeformationModelInternal::compute_d2Fe_dx_dai(const ES::M3d& dAInvdai, const ES::M9x12d& dFdx,
                                                          ES::M9x12d& d2Fdudai) const {
...
double TetMeshDeformationModelInternal::compute_dpsi_dai(const ES::M3d& Ds, const ES::M3d& dAInv_dai,
...
double TetMeshDeformationModelInternal::compute_d2psi_dai_daj(const ES::M3d& Ds, const ES::M3d& dAInv_dai,
...
void TetMeshDeformationModelInternal::computeDs(const ES::V3d& x0, const ES::V3d& x1, const ES::V3d& x2,
                                                const ES::V3d& x3, ES::M3d& Ds) const {
    Ds.col(0) = x1 - x0;
    Ds.col(1) = x2 - x0;
    Ds.col(2) = x3 - x0;
}
...
void TetMeshDeformationModelInternal::computeDmInv(const ES::V3d& X0, const ES::V3d& X1, const ES::V3d& X2,
                                                   const ES::V3d& X3, ES::M3d& DmInv) const {
    ES::M3d Dm;
    Dm.col(0) = X1 - X0;
    Dm.col(1) = X2 - X0;
    Dm.col(2) = X3 - X0;
...
double TetMeshDeformationModelInternal::computeVolume(const double X0[3], const double X1[3], const double X2[3],
                                                      const double X3[3]) const {
...
double TetMeshDeformationModel::computeVolume(const double X0[3], const double X1[3], const double X2[3],
                                              const double X3[3]) {
    return Mesh::getTetVolume(asVec3d(X0), asVec3d(X1), asVec3d(X2), asVec3d(X3));
}
