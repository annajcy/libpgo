...
public:
    ...
    void computeHessian(const double* x, const double* plasticParams, const double* elasticParams,
                        EigenSupport::SpMatD& hess) const;

    void compute_df_da(const double* x, const double* plasticParams, const double* elasticParams,
                       EigenSupport::SpMatD& hess) const;
    void compute_df_db(const double* x, const double* plasticParams, const double* elasticParams,
                       EigenSupport::SpMatD& hess) const;

    void computeVonMisesStresses(const double* x, const double* plasticParams, const double* elasticParams,
                                 double* elementStresses) const;
    void computeMaxStrains(const double* x, const double* plasticParams, const double* elasticParams,
                           double* elementStrain) const;

    int getNumDOFs() const { return n3; }

    std::shared_ptr<const DeformationModelManager> getDeformationModelManager() const {
        return deformationModelManager;
    }
