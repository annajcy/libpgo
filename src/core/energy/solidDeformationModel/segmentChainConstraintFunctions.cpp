...
SegmentChainConstraintFunctions::SegmentChainConstraintFunctions(int nAll, int numPoints, const double* const points,
                                                                 int isCircle)
    ...
    for (int vi = 0; vi < nele; vi++) {
        restLengths[vi] =
            (restPositions.segment<3>((vi + 1) % numPoints * 3) - restPositions.segment<3>(vi * 3)).norm();
    }
