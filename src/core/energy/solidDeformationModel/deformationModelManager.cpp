...
std::map<DeformationModelPlasticMaterial, int> numPlasticDOFs{
    {DeformationModelPlasticMaterial::VOLUMETRIC_DOF0, 0}, {DeformationModelPlasticMaterial::VOLUMETRIC_DOF3, 3},
    {DeformationModelPlasticMaterial::VOLUMETRIC_DOF6, 6},

    {DeformationModelPlasticMaterial::SHELL_FF_DOF0, 0},   {DeformationModelPlasticMaterial::SHELL_FF_DOF1, 1},
...
void DeformationModelManager::setMesh(const SimulationMesh* simulationMesh, const double* elementFiberDirections,
...
void DeformationModelManager::init(DeformationModelPlasticMaterial plasticModelType,
                                   DeformationModelElasticMaterial elasticMaterialType, int enforceSPD) {
    ...
    tbb::parallel_for(
        ...
        [&](int ele) {
            ...
            if (elasticMaterialType == DeformationModelElasticMaterial::HILL_STABLE_NEO) {
                ...
                data->combined2Materials[ele] = new ElasticModelCombinedMaterial<2>(
                    data->stableNeoHookeanMaterials[ele], data->hillTypeMaterials[ele]);
                ...
                data->hillTypeMaterials[ele]  = new ElasticModelHillTypeMaterial(gamma, Eact, lo, dir.data());
                data->combined2Materials[ele] = new ElasticModelCombinedMaterial<2>(data->invariantBasedMaterials[ele],
                                                                                    data->hillTypeMaterials[ele]);
                ...
                data->combined3Materials[ele] = new ElasticModelCombinedMaterial<3>(
                    data->invariantBasedMaterials[ele], data->hillTypeMaterials[ele], data->volumeMaterials[ele]);
                ...
                data->combined2Materials[ele] =
                    new ElasticModelCombinedMaterial<2>(data->invariantBasedMaterials[ele], data->volumeMaterials[ele]);
            ...
            if (data->simulationMesh->getElementType() == SimulationMeshType::TET) {
                ...
                if (elasticMaterialType == DeformationModelElasticMaterial::STVK ||
                    ...
                    data->elementFEMs[ele] = new KoiterDeformationModel(
                        restPosition.data(), restPosition.data() + 3, restPosition.data() + 6, restPosition.data() + 9,
                        restPosition.data() + 12, restPosition.data() + 15, data->elementMaterials[ele], pm,
                        data->enforceSPD);
                } else {
                    throw std::logic_error("unsupported elastic material for shell element");
                    {};
...
void DeformationModelManager::updateMeshRigidTransformation(const double R[9]) {
    ...
    tbb::parallel_for(
        0, (int)data->fiberAxes.cols() / 3,
        [this](int i) {
            data->fiberAxes.block<3, 3>(0, i * 3) =
                data->fiberAxesRest.block<3, 3>(0, i * 3) * data->globalRotation.transpose();
        },
        tbb::static_partitioner());

    tbb::parallel_for(
        0, (int)data->vertexFiberAxes.cols() / 3,
        [this](int i) {
            data->vertexFiberAxes.block<3, 3>(0, i * 3) =
                data->vertexFiberAxesRest.block<3, 3>(0, i * 3) * data->globalRotation.transpose();
        },
        tbb::static_partitioner());
...
void DeformationModelManager::setElementAlignedMatrix(int id, double R[9]) {
    ...
    data->fiberAxes.block<3, 3>(0, id * 3) =
        data->fiberAxesRest.block<3, 3>(0, id * 3) * data->globalRotation.transpose();
