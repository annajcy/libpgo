...
PrescribedPrincipleStressConstraintFunctions::PrescribedPrincipleStressConstraintFunctions(
    int nAll, int doff, int numElements, const int* elementIDs, const DeformationModelManager* tmdmm)
            ...
            for (int j = 0; j < 4; j++) {
                for (int dof = 0; dof < 3; dof++) {
                    entries.emplace_back(
                        i * 3 + r, dofStart + tetMeshDMM->getMesh()->getVertexIndex(elements[i], j) * 3 + dof, 1.0);
                }
            }
        }
    }
...
void PrescribedPrincipleStressConstraintFunctions::func(ES::ConstRefVecXd x, ES::RefVecXd g) const {
    for (int i = 0; i < (int)elements.size(); i++) {
        ...
        const TetMeshDeformationModel* fem =
            dynamic_cast<const TetMeshDeformationModel*>(tetMeshDMM->getDeformationModel(eleID));
...
void PrescribedPrincipleStressConstraintFunctions::computeForceFromTargetPHat(ES::ConstRefVecXd x,
                                                                              ES::RefVecXd      fext) const {
    fext.setZero();
...
double PrescribedPrincipleStressConstraintFunctions::computeSurfaceNormalTractionFromElement(ES::ConstRefVecXd x,
                                                                                             const ES::V3d&    n,
                                                                                             int eleID) const {
    const TetMeshDeformationModel* fem =
        dynamic_cast<const TetMeshDeformationModel*>(tetMeshDMM->getDeformationModel(eleID));
    ...
    for (int i = 0; i < (int)elements.size(); i++) {
        ...
        for (int ci = 0; ci < 12; ci++) {
            for (int ri = 0; ri < 3; ri++) {
                ...
                auto it = jacEntries.find(
                    std::make_pair(i * 3 + ri, tetMeshDMM->getMesh()->getVertexIndex(eleID, vid) * 3 + dof));
...
void PrescribedPrincipleStressConstraintFunctions::hessian(ES::ConstRefVecXd x, ES::ConstRefVecXd lambda,
                                                           ES::SpMatD& hess) const {
        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                ...
                NonlinearOptimization::SVDDerivatives::unorderedSquareMatrixSVD3Derivatices(
                    P, S, U, V, dP, dSdPi[i], nullptr, nullptr, &d2P, &d2SdPidPj[i][j]);
            }
        }
