...
public:
    DeformationModel(ElasticModel* elasticModel, PlasticModel* plasticModel) : em(elasticModel), pm(plasticModel) {}
    ...
    // inline static double d3E_dx3_ijk(const double *tensor, int i, int j, int k, int dim) { return tensor[k * dim *
    // dim + j * dim + i]; } inline static double &d3E_dx3_ijk(double *tensor, int i, int j, int k, int dim) { return
    // tensor[k * dim * dim + j * dim + i]; }
