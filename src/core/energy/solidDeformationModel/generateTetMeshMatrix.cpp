...
void TetMeshMatrix::generateBasicElementLaplacianMatrix(const Mesh::TetMeshRef& tetMesh, ES::SpMatD& L,
    ...
    tbb::parallel_for((size_t)0, vertexNeighbors.size(),
                      [&](size_t vi) {
    ...
    tbb::parallel_for(0, numElements, [&](int el) {
        ...
        for (int vtxIdx = 0; vtxIdx < numElementVertices; vtxIdx++) {
            int vertexIndex = tetMesh.tetVtxID(el, vtxIdx);
            elementNeighbors.insert(elementNeighbors.end(), vertexNeighbors[vertexIndex].begin(),
                                    vertexNeighbors[vertexIndex].end());
        }
