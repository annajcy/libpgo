...
SegmentBinormalConstraintFunctions::SegmentBinormalConstraintFunctions(int nAll, int pStart, int sStart,
                                                                       int numSegments, const double* positions)
    : NonlinearOptimization::ConstraintFunctions(nAll),
      nSeg(numSegments),
      positionDOFStart(pStart),
      segmentDOFStart(sStart) {
    if (positions) {
    ...
    for (int ei = 0; ei < nSeg; ei++) {
        if (restPositions.size() == 0) {
            int dofStart[3] = {positionDOFStart + ei * 3, positionDOFStart + ei * 3 + 3, segmentDOFStart + ei * 3};
            ...
            int dofStart[1] = {segmentDOFStart + ei * 3};
...
void SegmentBinormalConstraintFunctions::hessian(ES::ConstRefVecXd x, ES::ConstRefVecXd lambda,
