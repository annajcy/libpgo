...
TetVolumeConstraintFunctions::TetVolumeConstraintFunctions(const SimulationMesh* tm, int nAll, const ES::VXd* rp,
                                                           const ES::M3Xd* DmInvIn)
    ...
    for (int ei = 0; ei < nele; ei++) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                for (int k = 0; k < 3; k++) {
                    for (int l = 0; l < 3; l++) {
                        entries.emplace_back(tetMesh->getVertexIndex(ei, i) * 3 + k,
                                             tetMesh->getVertexIndex(ei, j) * 3 + l, 1.0);
                    }
                }
            }
        }
...
void TetVolumeConstraintFunctions::func(ES::ConstRefVecXd x, ES::RefVecXd g) const {
    ...
    tbb::parallel_for(0, nele, [&](int ei) {
        ...
        if (restPosition) {
            for (int i = 0; i < 4; i++) {
                xlocal.segment<3>(i * 3) =
                    restPosition->segment<3>(vertexIndices[i] * 3) + x.segment<3>(vertexIndices[i] * 3);
            }
        } else {
            for (int i = 0; i < 4; i++) {
