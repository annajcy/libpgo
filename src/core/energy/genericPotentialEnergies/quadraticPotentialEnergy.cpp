...
public:
    QuadraticPotentialEnergyCache(int n) { temp.setZero(n); }
...
QuadraticPotentialEnergy::QuadraticPotentialEnergy(const ES::SpMatD& A_) : A(A_), b(nullptr), isInParentheses(0) {
...
QuadraticPotentialEnergy::QuadraticPotentialEnergy(const ES::SpMatD& A_, int) : A(ATA), b(nullptr), isInParentheses(1) {
    ES::mm(A_, A_, ATA, 1);
...
void QuadraticPotentialEnergy::hessianVector(EigenSupport::ConstRefVecXd, EigenSupport::ConstRefVecXd vec,
                                             EigenSupport::RefVecXd hVec) const {
    ES::mv(A, vec, hVec);
}
