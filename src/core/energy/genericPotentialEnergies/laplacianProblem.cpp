...
void LaplacianProblem::setVariableConstraints(const std::vector<int>& fixedDOFs, const EigenSupport::VXd& fixedValues,
                                              double variableLow, double variableHi) {
...
void LaplacianProblem::solve(EigenSupport::RefVecXd xfinal, int numIter, double eps, int verbose) {
    ...
    std::shared_ptr<NonlinearOptimization::PotentialEnergies> energyAll =
        std::make_shared<NonlinearOptimization::PotentialEnergies>(sys.rows());
    ...
    xfinal  = xinit;
    int ret = NonlinearOptimization::EnergyOptimizer::minimizeUsingKnitro(
        xfinal, energyAll, xlow, xhi, lambda, g, nullptr, clow, chi, numIter, eps, verbose, "");
