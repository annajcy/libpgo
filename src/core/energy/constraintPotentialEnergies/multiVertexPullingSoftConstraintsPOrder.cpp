...
struct MultipleVertexPullingSoftConstraintsPOrderBuf {
    MultipleVertexPullingSoftConstraintsPOrderBuf(int numLocks) : energyTLS(0.0), locks(numLocks) {}
...
MultipleVertexPullingSoftConstraintsPOrder::MultipleVertexPullingSoftConstraintsPOrder(
    const ES::SpMatD& Koff, const EigenSupport::VXd& restPositionsAll, int numPoints, const double* constraintCoeffs,
    const double* constraintTargetPositions, const double* constraintNormals, const int* idx, double bcCoeff, int useN,
    int isd, double pval)
    ...
    tbb::parallel_for(
        ...
        [&](int ci) {
            ...
            if (isL2) {
                ...
                hessianLocal =
                    halfPower * (halfPower - 1) * std::pow(proj2, halfPower - 2) * diff * diff.transpose() * 2;
