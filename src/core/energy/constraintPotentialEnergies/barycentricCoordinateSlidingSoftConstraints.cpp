...
struct BarycentricCoordinateSlidingBuf {
    BarycentricCoordinateSlidingBuf(int n) : energyTLS(0.0), locks(n) {}
...
BarycentricCoordinateSliding::BarycentricCoordinateSliding(const ES::SpMatD& Koff, const ES::VXd& restPositionsAll,
                                                           int numPoints, const double* constraintCoeffs,
                                                           const double* constraintTargetPositions,
                                                           const double* constraintNormals, const int* idx,
                                                           const double* barycentricWeights, int nEleVtx,
                                                           double bcCoeff, int useN, int cp, int isDisp)
    : PotentialEnergyAligningMeshConnectivity(Koff),
      restpAll(restPositionsAll),
      coeffAll(bcCoeff),
      numElementVertices(nEleVtx),
      useNormal(useN),
      checkPenetration(cp),
      isInputDisplacement(isDisp) {
        ...
        [&](int ci) {
            ...
            if (useNormal) {
                ...
                if ((checkPenetration && proj < 0) || checkPenetration == 0) {
                    for (int vi = 0; vi < numElementVertices; vi++) {
                        ES::V3d gradLocal =
                            n * vertexBarycentricWeights[ci * numElementVertices + vi] * proj * coeffs[ci] * coeffAll;
                        int vid = vertexIndices[ci * numElementVertices + vi];
                ...
                for (int vi = 0; vi < numElementVertices; vi++) {
                    ES::V3d gradLocal =
                        diff * vertexBarycentricWeights[ci * numElementVertices + vi] * coeffs[ci] * coeffAll;
                    int vid = vertexIndices[ci * numElementVertices + vi];
