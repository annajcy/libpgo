...
MultipleVertexConstrainedRigidMotion::MultipleVertexConstrainedRigidMotion(const ES::VXd&          rp,
                                                                           const std::vector<int>& vi, const double* vw,
                                                                           double coeffR, double coefft, bool frm,
                                                                           bool id)
    : restPositions(rp), vertexIndices(vi), flexibleRigidMotion(frm), isDisp(id) {
    if (vw) {
...
void MultipleVertexConstrainedRigidMotion::hessian(ES::ConstRefVecXd u, ES::SpMatD& hess) const {
    ...
    if (flexibleRigidMotion) {
        for (ES::IDX rowi = 0; rowi < Z.rows(); rowi++) {
            for (ES::SpMatD::InnerIterator it(Z, rowi); it; ++it) {
                auto iter =
                    entryMap.find(std::pair<int, int>((int)it.row() + (int)restPositions.size(), (int)it.col()));
                PGO_ALOG(iter != entryMap.end());
                hess.valuePtr()[iter->second] += -it.value() * coeff[1];
        ...
        for (int i = 0; i < 3; i++) {
            auto iter =
                entryMap.find(std::pair<int, int>(i + (int)restPositions.size(), i + (int)restPositions.size()));
            PGO_ALOG(iter != entryMap.end());
    ...
    if (coeff[0] > 0) {
                ...
                for (int dofi = 0; dofi < 3; dofi++) {
                    for (int dofj = 0; dofj < 3; dofj++) {
                        ...
                        for (int k = 0; k < 9; k++) {
                            for (int l = 0; l < 9; l++) {
                                d2Rdxidxj +=
                                    d2RdF2[k][l] * dFdxi[vj * 3 + dofj].data()[k] * dFdxi[vi * 3 + dofi].data()[l];
                            }
                        }
        ...
        if (flexibleRigidMotion) {
            ...
            // dRbar dRbar = I
            for (int i = 0; i < 9; i++) {
                auto iter = entryMap.find(
                    std::pair<int, int>(i + (int)restPositions.size() + 3, i + (int)restPositions.size() + 3));
                PGO_ALOG(iter != entryMap.end());
