...
double getTwoTriangleDihedralAngle(const Vec3d& e0, const Vec3d& e1, const Vec3d& t0, const Vec3d& t1) {
    ...
    Vec3d d0 = t0 - e0;
    Vec3d p0 = e0 + (lineVector.dot(d0) * invLineLen2) *
                        lineVector;  // the equation is gotten from the function getClosestPointToLine()
...
Vec3d getBarycentricWeightProjectedOnTrianglePlane(const Vec3d& queryPoint, const Vec3d& t0, const Vec3d& t1,
                                                   const Vec3d& t2) {
    ...
    // to find a way for unprojected version, we note that the equation above is true if the equation is projected onto
    // vec1 or vec2: dot(w[1] vec1 + w[2] vec2, vec1) = dot(diff', vec1) = dot(diff, vec1) dot(w[1] vec1 + w[2] vec2,
    // vec2) = dot(diff', vec2) = dot(diff, vec2) so: w[1] dot(vec1, vec1) + w[2] dot(vec2, vec1) = dot(diff, vec1) w[1]
    // dot(vec1, vec2) + w[2] dot(vec2, vec2) = dot(diff, vec2)
...
Vec3d getClosestPointToTriangleWithFeature(const Vec3d& queryPoint, const Vec3d& t0, const Vec3d& t1, const Vec3d& t2,
                                           int feature) {
    if (feature == 0)
        return t0;
    else if (feature == 1)
...
Vec3d getClosestPointToTriangleWithNormalAndFeature(const Vec3d& queryPoint, const Vec3d& t0, const Vec3d& t1,
                                                    const Vec3d& t2, const Vec3d& normal, int feature) {
    if (feature == 0)
        return t0;
    else if (feature == 1)
        return t1;
...
bool isToLeftOfTriangleEdge(const Vec3d& queryPoint, const Vec3d& scaledTriangleNormal, const Vec3d& edgeStart,
                            const Vec3d& edgeEnd) {
...
//  5: edge among 20
//  6: the face itself
double getSquaredDistanceToTriangle(const Vec3d& queryPoint, const Vec3d& vertex0, const Vec3d& vertex1,
                                    const Vec3d& vertex2, int& feature) {
    ...
    if (scaledNormal.cwiseAbs().sum() > std::numeric_limits<double>::epsilon() &&
        ...
        // the closest point on triangle to queryPoint is the same closet point on the plane where the triangle lies to
        // queryPoint
        feature = 6;
        return getSquaredDistanceToPlaneWithScaledNormalAndDelta(scaledNormal, queryPoint - vertex0);
    } else  // the projection of the queryPoint onto the triangle plane is outside the triangle, or the triangle is
            // degenerate
...
double getSquaredDistanceToTriangle(const Vec3d& queryPoint, const Vec3d& vertex0, const Vec3d& vertex1,
                                    const Vec3d& vertex2, int& feature, Vec3d& closestPointOnTriangle,
    ...
    if (scaledNormal.cwiseAbs().sum() > std::numeric_limits<double>::epsilon() &&
        ...
        closestPointOnTriangle =
            barycentricWeight[0] * vertex0 + barycentricWeight[1] * vertex1 + barycentricWeight[2] * vertex2;
...
double whetherTriangleIntersectBoundingBox(const Vec3d& t0, const Vec3d& t1, const Vec3d& t2, const Vec3d& bmin,
                                           const Vec3d& bmax) {
...
void getUnitMassTriangleInertiaTensorVectorAroundOrigin(const Vec3d& v0, const Vec3d& v1, const Vec3d& v2,
                                                        double t[6]) {
    t[0] = (v0[1] * v0[1] + v0[2] * v0[2] + v1[1] * v1[1] + v1[2] * v1[2] + v1[1] * v2[1] + v2[1] * v2[1] +
            v0[1] * (v1[1] + v2[1]) + v1[2] * v2[2] + v2[2] * v2[2] + v0[2] * (v1[2] + v2[2])) /
           6;
    ...
    t[2] = (-2 * v1[0] * v1[2] - v1[2] * v2[0] - v0[2] * (v1[0] + v2[0]) - v1[0] * v2[2] - 2 * v2[0] * v2[2] -
            v0[0] * (2 * v0[2] + v1[2] + v2[2])) /
           12;

    t[3] = (v0[0] * v0[0] + v0[2] * v0[2] + v1[0] * v1[0] + v1[2] * v1[2] + v1[0] * v2[0] + v2[0] * v2[0] +
            v0[0] * (v1[0] + v2[0]) + v1[2] * v2[2] + v2[2] * v2[2] + v0[2] * (v1[2] + v2[2])) /
           6;

    t[4] = (-2 * v1[1] * v1[2] - v1[2] * v2[1] - v0[2] * (v1[1] + v2[1]) - v1[1] * v2[2] - 2 * v2[1] * v2[2] -
            v0[1] * (2 * v0[2] + v1[2] + v2[2])) /
           12;

    t[5] = (v0[0] * v0[0] + v0[1] * v0[1] + v1[0] * v1[0] + v1[1] * v1[1] + v1[0] * v2[0] + v2[0] * v2[0] +
            v0[0] * (v1[0] + v2[0]) + v1[1] * v2[1] + v2[1] * v2[1] + v0[1] * (v1[1] + v2[1])) /
           6;
}
...
void getTetBarycentricWeights(const Vec3d& queryPoint, const Vec3d& a, const Vec3d& b, const Vec3d& c, const Vec3d& d,
                              double weight[4]) {
...
double getSquaredDistanceToTet(const Vec3d& queryPoint, const Vec3d& a, const Vec3d& b, const Vec3d& c,
                               const Vec3d& d) {
...
Mat3d getTetInertiaTensorAroudOrigin(const Vec3d& a, const Vec3d& b, const Vec3d& c, const Vec3d& d,
...
Mat3d getTetInertiaTensorAroundCOM(const Vec3d& t0, const Vec3d& t1, const Vec3d& t2, const Vec3d& t3,
                                   const Vec3d& tetCenterOfMass, double tetDeterminant) {
...
Mat3d getBoxInertiaTensorAroundCOM(const Vec3d& side, double volume) {
    ...
    return asMat3d(mover12 * side[1] * side[1] + side[2] * side[2], 0.0, 0.0, 0.0,
                   mover12 * side[0] * side[0] + side[2] * side[2], 0.0, 0.0, 0.0,
                   mover12 * side[0] * side[0] + side[1] * side[1]);
}

void getBoundingBoxFromSelectedVertices(int numVtx, const int* vtxIndices, const Vec3d* vertices, Vec3d& bmin,
                                        Vec3d& bmax) {
