...
double BoundingVolumeTreeBase::nearestQuery(std::vector<std::tuple<double, double, int>>& nodeStack,
                                            DistanceToBBType toBB, DistanceToElementType toElement, double distanceHi,
                                            double distanceLo) const {
    ...
    while (nodeStack.size() > 0) {
        ...
        if (nodeLowerBound > minDistance)  // if this BV node's distance lower bound is larger than distanceHi
            continue;
        // note here we don't use if (nodeLowerBound >= minDistance), so in this way two node which share the same lower
        // bound as minDistance will be both visited
...
template <class DistanceToBBType, class DistanceToElementType>
double BoundingVolumeTreeBase::nearestQuery(DistanceToBBType toBB, DistanceToElementType toElement, double distanceHi,
                                            double distanceLo) const {
...
void VertexBVTree::buildByInertiaPartition(const ArrayRef<Vec3d> vertices, int maxDepth, int maxNumVerticesPerNode) {
    ...
    auto subdivide = [&](const std::vector<int>& elementList, const BoundingBox& bb,
        ...
        elementsInertiaBinaryPartition(
            elementList, bb, childIDs, INT_MAX, [&](int) { return 1.0; }, [&](int i) { return vertices[i]; },
...
void VertexBVTree::rangeQuery(const ArrayRef<Vec3d> vertices, const SimpleSphere& simpleSphere,
                              std::vector<int>& vertexIDList) const {
    PGO_ALOG(numVertices == vertices.size());
    auto toBB = [&](const BoundingBox& bb) -> bool { return simpleSphere.doesBoundingBoxIntersect(bb); };

    auto toEle = [&](int idx) -> bool { return simpleSphere.inside(vertices[idx]); };

    BoundingVolumeTreeBase::rangeQuery(toBB, toEle, vertexIDList);
}

void VertexBVTree::rangeQuery(const ArrayRef<Vec3d> vertices, const HalfSpace& halfSpace,
                              std::vector<int>& vertexIDList) const {
    PGO_ALOG(numVertices == vertices.size());
    auto toBB = [&](const BoundingBox& bb) -> bool { return halfSpace.intersect(bb); };

    auto toEle = [&](int idx) -> bool { return halfSpace.outside(vertices[idx]) <= 0; };
...
bool VertexBVTree::intersectAABB(const ArrayRef<Vec3d> vertices, const BoundingBox& bbQuery) const {
    PGO_ALOG(numVertices == vertices.size());
    auto toBB = [&](const BoundingBox& bb) -> bool { return bbQuery.intersect(bb); };

    auto toEle = [&](int idx) -> bool { return bbQuery.checkInside(vertices[idx]); };
...
void BoundingBoxBVTree::buildByInertiaPartition(const ArrayRef<BoundingBox> boundingBoxes, int maxDepth,
    ...
    auto subdivide = [&](const std::vector<int>& elementList, const BoundingBox& bb,
        ...
        elementsInertiaBinaryPartition(
            elementList, bb, childIDs, INT_MAX, [&](int i) { return bbVolumes[i]; },
            [&](int i) { return boundingBoxes[i].center(); },
            [&](int i, const Vec3d& reference) {
                return shiftInertiaTensorAroundMassCenterToReferencePoint(bbInertia[i], bbVolumes[i],
...
void BoundingBoxBVTree::getClosestBoundingBoxes(const ArrayRef<BoundingBox> boundingBoxes, const Vec3d& queryPosition,
    ...
    // we assign the fourth parameter of nearestQuery() to be distanceLowerBound = -1.0 to avoid early termination of
    // the function once it finds minDistance == 0.0 In this way, we can find all the bounding boxes that contain
    // queryPosition
...
void BoundingBoxBVTree::intersectAABB(const ArrayRef<BoundingBox> bbs, const BoundingBox& bbQuery,
                                      std::vector<int>& bbIDList) const {
    auto toBB = [&](const BoundingBox& bb) -> bool { return bbQuery.intersect(bb); };

    auto toEle = [&](int idx) -> bool { return bbQuery.intersect(bbs[idx]); };
...
void TriMeshBVTree::buildExactOctree(const TriMeshRef triMesh, int maxDepth, int maxNumTrianglesPerNode) {
    ...
    auto triBBIntersect = [&](int triID, const BoundingBox& bb) -> bool {
        ...
        return intersectTriAABB(triMesh.pos(triID, 0).data(), triMesh.pos(triID, 1).data(),
                                triMesh.pos(triID, 2).data(), bb.bmin().data(), bb.bmax().data());
    };
...
void TriMeshBVTree::buildInexactOctree(const TriMeshRef triMesh, const BoundingBox& initialBoundingBox, int maxDepth,
                                       int maxNumTrianglesPerNode) {
    nodes.clear();
    numVertices  = triMesh.numVertices();
    numTriangles = triMesh.numTriangles();
    ...
    auto triBBIntersect = [&](int triID, const BoundingBox& bb) -> bool {
        ...
        return triBoxOverlap(bb.center().data(), bb.halfSides().data(), triMesh.pos(triID, 0).data(),
                             triMesh.pos(triID, 1).data(), triMesh.pos(triID, 2).data());
    };
...
void TriMeshBVTree::buildByInertiaPartition(const TriMeshRef triMesh, int maxDepth, int maxNumTrianglesPerNode) {
    ...
    auto subdivide = [&](const std::vector<int>& elementList, const BoundingBox& bb,
        ...
        // PGO_ALOG(bb.verifyBox());
        elementsInertiaBinaryPartition(
            elementList, bb, childIDs, INT_MAX, [&](int i) { return triSurfaceAreas[i]; },
            [&](int i) { return triCenterOfMass[i]; },
            [&](int i, const Vec3d& reference) {
                return shiftInertiaTensorAroundMassCenterToReferencePoint(triInertia[i], triSurfaceAreas[i],
...
void TriMeshBVTree::selfIntersectionExact(const TriMeshRef                  triMesh,
    ...
    std::vector<UEdgeKey> candidatePairs(
        candidateSet.begin(), candidateSet.end());  // store all those candidate triangle pairs for parallel evaluations
    ...
    tbb::parallel_for(0, sizei(candidatePairs), [&](int i) {
        ...
        if (intersectTriTri(triMesh.pos(triIDA, 0).data(), triMesh.pos(triIDA, 1).data(), triMesh.pos(triIDA, 2).data(),
                            triMesh.pos(triIDB, 0).data(), triMesh.pos(triIDB, 1).data(),
                            triMesh.pos(triIDB, 2).data())) {
            intersected[i] = 1;
        }
    });  // end for locations
...
void TriMeshBVTree::intersectionExact(const TriMeshRef triMesh, const TriMeshRef& otherMesh,
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, otherMesh.numTriangles()), [&](const tbb::blocked_range<int>& rng) {
        ...
        for (int oID = rng.begin(); oID != rng.end(); oID++) {
            IDlist.clear();
            triangleIntersectionExact(triMesh, otherMesh.pos(oID, 0), otherMesh.pos(oID, 1), otherMesh.pos(oID, 2),
                                      IDlist);
...
void TriMeshBVTree::intersectionExact(const TriMeshRef triMesh, const TriMeshBVTree& otherTree,
                                      const TriMeshRef&                 otherMesh,
                                      std::vector<std::pair<int, int>>& triangleIDList) const {
    PGO_ALOG(numVertices == triMesh.numVertices());
    PGO_ALOG(numTriangles == triMesh.numTriangles());
    PGO_ALOG(nodes.empty() == false);
    ...
    std::vector<char> intersected(candidatePairs.size(), 0);
    tbb::parallel_for(
        tbb::blocked_range<int>(0, candidatePairs.size()),
        [&](const tbb::blocked_range<int>& rng) {
            for (int i = rng.begin(); i != rng.end(); ++i) {
                ...
                if (intersectTriTri(triMesh.pos(triIDA, 0).data(), triMesh.pos(triIDA, 1).data(),
                                    triMesh.pos(triIDA, 2).data(), otherMesh.pos(triIDB, 0).data(),
                                    otherMesh.pos(triIDB, 1).data(), otherMesh.pos(triIDB, 2).data())) {
                    intersected[i] = 1;
                }
            }
        },
...
void TriMeshBVTree::lineSegmentIntersectionExact(const TriMeshRef triMesh, Vec3d segStart, Vec3d segEnd,
                                                 std::vector<int>& triangleIDList) const {
    PGO_ALOG(numVertices == triMesh.numVertices());
    PGO_ALOG(numTriangles == triMesh.numTriangles());
    ...
    auto toEle = [&](int idx) -> bool {
        return intersectSegTri(&segStart[0], &segEnd[0], &triMesh.pos(idx, 0)[0], &triMesh.pos(idx, 1)[0],
                               &triMesh.pos(idx, 2)[0]);
    };
...
void TriMeshBVTree::triangleIntersectionExact(const TriMeshRef triMesh, Vec3d t0, Vec3d t1, Vec3d t2,
                                              std::vector<int>& triangleIDList) const {
    PGO_ALOG(numVertices == triMesh.numVertices());
    PGO_ALOG(numTriangles == triMesh.numTriangles());
    ...
    auto toEle = [&](int idx) -> bool {
        return intersectTriTri(&t0[0], &t1[0], &t2[0], &triMesh.pos(idx, 0)[0], &triMesh.pos(idx, 1)[0],
                               &triMesh.pos(idx, 2)[0]);
    };
...
int TriMeshBVTree::lineSegmentFirstIntersectionPointSemiExact(const TriMeshRef triMesh, Vec3d segStart, Vec3d segEnd,
                                                              double segWeight[2], double triWeight[3],
                                                              std::stack<int>* externalStack) const {
    PGO_ALOG(numVertices == triMesh.numVertices());
    PGO_ALOG(numTriangles == triMesh.numTriangles());
    ...
    auto   toEle         = [&](int triID) -> void {
        double alpha[5];
        if (intersectSegTri(segStart.data(), segEnd.data(), triMesh.pos(triID, 0).data(), triMesh.pos(triID, 1).data(),
                                      triMesh.pos(triID, 2).data(), alpha, alpha + 2)) {
...
bool TriMeshBVTree::hasLineSegmentIntersectionExact(const TriMeshRef triMesh, Vec3d segStart, Vec3d segEnd,
                                                    std::vector<int>* skippedTriIDs) const {
    PGO_ALOG(numVertices == triMesh.numVertices());
    PGO_ALOG(numTriangles == triMesh.numTriangles());
...
std::pair<int, double> TriMeshBVTree::getClosestTriangleHelper(
    GetTriangleDistance2 getTriangleDistance2, const Vec3d& queryPosition, int* feature, int warmStartTriID,
    double dist2HigherBound, std::vector<std::tuple<double, double, int>>& nodeStack) const {
...
template <class GetTriangleDistance2>
std::pair<int, double> TriMeshBVTree::getClosestTriangleHelper(GetTriangleDistance2 getTriangleDistance2,
                                                               const Vec3d& queryPosition, int* feature,
                                                               int warmStartTriID, double distance2HigherBound) const {
    std::vector<std::tuple<double, double, int>> nodeStack;
    return getClosestTriangleHelper<GetTriangleDistance2>(getTriangleDistance2, queryPosition, feature, warmStartTriID,
                                                          distance2HigherBound, nodeStack);
}

int TriMeshBVTree::getClosestTriangle(const TriMeshRef triMesh, const Vec3d& queryPosition, int* feature,
    ...
    auto getTriangleDist2 = [&](const Vec3d& queryPosition, int triID, int& feature) {
        return getSquaredDistanceToTriangle(queryPosition, triMesh.pos(triID, 0), triMesh.pos(triID, 1),
                                            triMesh.pos(triID, 2), feature);
    };

    return getClosestTriangleHelper(getTriangleDist2, queryPosition, feature, warmStartTriID, dist2HigherBound).first;
}

int TriMeshBVTree::getClosestTriangle(const TriangleWithCollisionInfo* triangleWithColInfos, const Vec3d& queryPosition,
                                      int* feature, int warmStartTriID, double dist2HigherBound) const {
    auto getTriangleDist2 = [&](const Vec3d& queryPosition, int triID, int& feature) {
...
TriMeshBVTree::ClosestTriangleQueryResult TriMeshBVTree::closestTriangleQuery(const TriMeshRef triMesh,
                                                                              const Vec3d&     queryPosition,
                                                                              int              warmStartTriID,
...
TriMeshBVTree::ClosestTriangleQueryResult TriMeshBVTree::closestTriangleQuery(
    const TriMeshRef triMesh, const Vec3d& queryPosition, std::vector<std::tuple<double, double, int>>& nodeStack,
    int warmStartTriID, double dist2HigherBound) const {
    PGO_ALOG(triMesh.numVertices() == numVertices && triMesh.numTriangles() == numTriangles);
    ...
    auto returnedPair = getClosestTriangleHelper(getTriangleDist2, queryPosition, &(ret.feature), warmStartTriID,
                                                 dist2HigherBound, nodeStack);
    ret.triID         = returnedPair.first;
    ret.dist2         = returnedPair.second;

    if (ret.triID >= 0) {
        ret.closestPosition =
            getClosestPointToTriangleWithFeature(queryPosition, triMesh.pos(ret.triID, 0), triMesh.pos(ret.triID, 1),
                                                 triMesh.pos(ret.triID, 2), ret.feature);

        ret.triBaryWeight = getBarycentricWeightProjectedOnTrianglePlane(
            ret.closestPosition, triMesh.pos(ret.triID, 0), triMesh.pos(ret.triID, 1), triMesh.pos(ret.triID, 2));
    }
...
TriMeshBVTree::ClosestTriangleQueryResult TriMeshBVTree::closestTriangleQuery(
    const TriangleWithCollisionInfo* triangles, const Vec3d& queryPosition,
    std::vector<std::tuple<double, double, int>>& nodeStack, int warmStartTriID, double dist2HigherBound) const {
    ClosestTriangleQueryResult ret;
    auto                       getTriangleDist2 = [&](const Vec3d& queryPosition, int triID, int& feature) {
...
void TetMeshBVTree::buildByInertiaPartition(const TetMeshRef tetMesh, int maxDepth, int maxNumTetsPerNode) {
    ...
    auto subdivide = [&](const std::vector<int>& elementList, const BoundingBox& bb,
        ...
        // PGO_ALOG(bb.verifyBox());
        elementsInertiaBinaryPartition(
            elementList, bb, childIDs, INT_MAX, [&](int i) { return tetVolume[i]; },
            [&](int i) { return tetCenterOfMass[i]; },
            [&](int i, const Vec3d& reference) {
                return shiftInertiaTensorAroundMassCenterToReferencePoint(tetInertia[i], tetVolume[i],
...
void TetMeshBVTree::buildExactOctree(const TetMeshRef tetMesh, int maxDepth, int maxNumTetsPerNode) {
    ...
    auto tetBBIntersect = [&](int tetID, const BoundingBox& bb) -> bool {
        return intersectTetAABB(tetMesh.pos(tetID, 0).data(), tetMesh.pos(tetID, 1).data(),
                                tetMesh.pos(tetID, 2).data(), tetMesh.pos(tetID, 3).data(), bb.bmin().data(),
                                bb.bmax().data());
    };
    auto subdivide = [&](const std::vector<int>& elementList, const BoundingBox& bb,
                         std::vector<std::vector<int>>& childIDs, std::vector<BoundingBox>& subBBs) -> void {
...
int TetMeshBVTree::getClosestTet(const TetMeshRef tetMesh, const Vec3d& queryPosition) const {
    ...
    auto toElement = [&](int tetID, double minDistance) -> double {
        ...
        double dist2 = getSquaredDistanceToTet(queryPosition, tetMesh.pos(tetID, 0), tetMesh.pos(tetID, 1),
                                               tetMesh.pos(tetID, 2), tetMesh.pos(tetID, 3));
        if (dist2 <= minDistance) {
            closestTetID = tetID;
        }
        return dist2;
...
int TetMeshBVTree::getClosestTet(const TetMeshRef tetMesh, const Vec3d& queryPosition,
                                 std::vector<std::tuple<double, double, int>>& nodeStack) const {
    PGO_ALOG(numVertices == tetMesh.numVertices());
    PGO_ALOG(numTets == tetMesh.numTets());
