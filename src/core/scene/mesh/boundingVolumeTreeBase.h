...
public:
    int getDepth() const {
        return depth;
    }  // get depth of the tree, which equals to the maximum Node::depth in all nodes
    ...
    // Usually it is set to be DBL_MAX. But it can be other values to cull away elements that are too far away as a
    // priori. distanceLowerBound: when the value is found related to an element, then the search is done. Usually it is
    // set to be 0.0. But it can be other >0 values if you just need a close-enough element. return the minimum distance
    // found
    template <class DistanceToBB, class DistanceToElement>
    double nearestQuery(std::vector<std::tuple<double, double, int>>& nodeStack, DistanceToBB toBB,
                        DistanceToElement toElement, double distanceHigherBound = DBL_MAX,
...
protected:
    ...
    // shrinkBoundingBoxes: whether to shrink the bounding box to tightly fit the elements after bounding box
    // subdivision
    //                      Shrinking reduce the size of bounding boxes, improving performance.
    //                      But you may also want to not shrink when building an inexact octree to prevent elements
    //                      lying on the bounding box faces. In an inexact octree, elements lying on the bounding box
    //                      faces may counted as not intersecting with the bounding box Due to numerical issues.
    static void boundingBoxOctreePartition(const std::vector<int>& elements, const BoundingBox& bb,
                                           std::vector<std::vector<int>>& childIDs, std::vector<BoundingBox>& subBBs,
                                           const std::vector<BoundingBox>& elementBBs, BBFromElementList,
                                           ElementBBIntersect, bool shrinkBoundingBoxes = true);
    ...
    static void elementsInertiaPartition(const std::vector<int>& elements, const BoundingBox& bb, int numPartitions,
                                         std::vector<std::vector<int>>& childIDs, ElementMass, ElementCenterOfMass,
                                         ElementInertiaTensor);
    // binary partition
    ...
    struct Node {
        ...
        // nextChildIndex: [0, 8]
        int getNextChild(int nextChildIndex) const {
            return (nextChildIndex < numChildren() ? nextChildIndex + 1 : nextChildIndex);
        }
...
inline void BoundingVolumeTreeBase::rangeQuery(BBFilter toBB, ElementFilter toEle,
                                               std::vector<int>& elementIDList) const {
