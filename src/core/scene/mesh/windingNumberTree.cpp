...
WindingNumberTree::Node::Node(int d, const TriMeshRef mesh)
    : depth(d),
      // We don't build a bounding box out of all triMesh's positions because some vertices might not be used by
      // triangles
      triangles(mesh.exportTriangles()),
      bouFaces(buildBoundary(triangles)),
      bb(mesh.computeTriangleBoundingBox()) {}
...
void WindingNumberTree::build(const TriMeshRef mesh, int maxDepth, int maxNumTrianglesPerNode) {
    ...
    while (nodeStack.empty() == false) {
        ...
        if (numNodeTri <= (size_t)maxNumTrianglesPerNode || (nodes[nodeID].bouFaces.size() >= numNodeTri) ||
            (nodeDepth >= maxDepth)) {
            continue;
        }
...
void WindingNumberTree::print() const {
    ...
    while (!nodeStack.empty()) {
        ...
        for (int i = 0; i < node.depth; i++)
            std::cout << "  ";
        std::cout << "nodeID: " << nodeID << " #tri " << node.triangles.size() << " #bou " << node.bouFaces.size()
                  << " ch " << node.childrenIDs[0] << " " << node.childrenIDs[1] << std::endl;
        if (node.childrenIDs[0] >= 0) {
            nodeStack.push(node.childrenIDs[0]);
            nodeStack.push(node.childrenIDs[1]);
        }
