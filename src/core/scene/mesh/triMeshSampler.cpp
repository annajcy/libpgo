...
struct SampleInfoHash {
    void hc(int v, std::size_t& seed) const { seed ^= std::hash<int>()(v) + 0x9e3779b9 + (seed << 6) + (seed >> 2); }
...
int sampleTriangle(const TriMeshGeo& mesh, int subdivideTriangle, std::vector<int>& sampleTriangleIDs,
                   std::vector<double>& sampleWeights, std::vector<std::array<double, 3>>& sampleBarycentricWeights) {
    ...
    std::cout << "#vtx: " << mesh.numVertices() << "; #tri: " << mesh.numTriangles()
              << "; #samples: " << sampleIDQueryTableCC.size() << std::endl;
