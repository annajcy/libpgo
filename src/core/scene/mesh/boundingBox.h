...
class BoundingBox {
public:
    BoundingBox() : bmin_(0.0, 0.0, 0.0), bmax_(1.0, 1.0, 1.0), center_(0.5, 0.5, 0.5), halfSides_(0.5, 0.5, 0.5) {}
    ...
    // an array of all the vertices, an array of all the triangles and an int container holding selected indices of the
    // triangles
    template <class IntContainer>
    ...
    explicit BoundingBox(const std::vector<BoundingBox>& bbs) : BoundingBox(static_cast<int>(bbs.size()), bbs.data()) {}
...
public:
    LightBoundingBox() : bmin_(DBL_MAX, DBL_MAX, DBL_MAX), bmax_(-DBL_MAX, -DBL_MAX, -DBL_MAX) {}
    LightBoundingBox(Vec3d bmin_g, Vec3d bmax_g) : bmin_(bmin_g), bmax_(bmax_g) {}
...
template <class Vec3dContainer>
LightBoundingBox::LightBoundingBox(const Vec3dContainer& vertices) : LightBoundingBox() {
    for (const Vec3d& p : vertices)
        expand(p);
}

template <class IntContainer>
LightBoundingBox::LightBoundingBox(const Vec3d* allVertices, const IntContainer& vertexIDs) : LightBoundingBox() {
    for (int ID : vertexIDs)
...
inline bool LightBoundingBox::checkInside(const Vec3d& p) const {
    return bmin_[0] < p[0] && p[0] < bmax_[0] && bmin_[1] < p[1] && p[1] < bmax_[1] && bmin_[2] < p[2] &&
           p[2] < bmax_[2];
}

inline bool LightBoundingBox::checkInside(const LightBoundingBox& b) const {
    return bmin_[0] < b.bmin_[0] && bmin_[1] < b.bmin_[1] && bmin_[2] < b.bmin_[2] && bmax_[0] > b.bmax_[0] &&
           bmax_[1] > b.bmax_[1] && bmax_[2] > b.bmax_[2];
}
...
inline bool LightBoundingBox::intersect(const LightBoundingBox& bb) const {
    return (bmin_[0] < bb.bmax_[0]) && (bmax_[0] > bb.bmin_[0]) && (bmin_[1] < bb.bmax_[1]) &&
           (bmax_[1] > bb.bmin_[1]) && (bmin_[2] < bb.bmax_[2]) && (bmax_[2] > bb.bmin_[2]);
}
