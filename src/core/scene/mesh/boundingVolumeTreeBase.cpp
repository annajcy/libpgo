...
void BoundingVolumeTreeBase::intersectionQuery(const BoundingVolumeTreeBase& otherTree,
                                               ElementPairProcess            elementsIntersect) const {
    ...
    while (true) {
        ...
        // cout << "====== " << iter++ << " ======" << endl;
        // printf("Top off stack: %d vs %d. Child index: %x . Stack size: %d\n", stackEntry.first, stackEntry.second,
        // stackEntry.third, (int)traversalStack.size());
        ...
        if ((potentialCollision == false) || bothLeaves) {
            ...
            while (true) {
                ...
                nodeBID =
                    parentEntry.second;  // now it stores the parent of the previous NodeBID
...
void BoundingVolumeTreeBase::selfIntersectionQuery(ElementPairProcess elementsIntersect) const {
    ...
    while (true) {
        ...
        auto goToValidChild = [&](int childIndex) {
            bool hasChild = true;
            if (nodeA == nodeB) {
                // for intra-node tests, we store (i,j) indices of current child pair in upper and lower parts of
                // childIndex; we only traverse children with i <= j; this avoids checking pairs of nodes twice
                short int* childIndexB = (short int*)&childIndex;
                short int* childIndexA = childIndexB + 1;

                // find an available childIndexA
                for (; *childIndexA < nodeA->numChildren() && nodeA->childrenIDs[*childIndexA] < 0;
                ...
                if (*childIndexB >= nodeB->numChildren()) {
                    ...
                    for ((*childIndexA)++; *childIndexA < nodeA->numChildren() && nodeA->childrenIDs[*childIndexA] < 0;
                         (*childIndexA)++) {
                    }
                    (*childIndexB) = (*childIndexA);
            ...
            } else  // A and B are different nodes
            {
                ...
                for (; childIndex < recurseNode->numChildren() && recurseNode->childrenIDs[childIndex] < 0;
                     childIndex++) {
                }
                ...
                if (hasChild)  // go to this node pair by pushing it into the stack
                {
                    if (recurseOnFirstNode)
                        ...
                        nodeBID = nodeB->childrenIDs[childIndex];
                    //          cout << "Find a new node pair on different nodes: " << nodeAID << " " << nodeBID << " "
                    //          << childIndex << endl;
                    traversalStack.push(triple<int, int, int>(nodeAID, nodeBID, childIndex));
                }
            }
        ...
        if (potentialCollision == false || bothLeaves) {
            ...
            while (true) {
                ...
                //        cout << "found a new stack top " << nodeAID << " " << nodeBID << " previous childIndex " <<
                //        childIndex << endl;
                if (nodeA == nodeB) {
                    short int* childIndexB = (short int*)&childIndex;
                    (*childIndexB)++;
                ...
                if (goToValidChild(childIndex))  // if we successfully find the next children pair to recursive from
                                                 // this pair as in parentEntry, then break
...
void BoundingVolumeTreeBase::boundingBoxOctreePartition(const std::vector<int>& elementList, const BoundingBox& bb,
                                                        std::vector<std::vector<int>>&  childIDs,
                                                        std::vector<BoundingBox>&       subBBs,
                                                        const std::vector<BoundingBox>& elementBBs,
                                                        BBFromElementList               createBBFromElements,
                                                        ElementBBIntersect intersectElementBB, bool shrink) {
...
void BoundingVolumeTreeBase::elementsInertiaPartition(const std::vector<int>& elements, const BoundingBox& /*bb*/,
                                                      ...
                                                      ElementMass getEleMass, ElementCenterOfMass getEleCOM,
                                                      ElementInertiaTensor getEleInertia) {
    ...
    // we create (numPartitions-1) equally spaced partitioning planes, partitioning the data range along the normal
    // vector
...
void BoundingVolumeTreeBase::elementsInertiaBinaryPartition(const std::vector<int>& elements, const BoundingBox& /*bb*/,
                                                            std::vector<std::vector<int>>& childIDs,
                                                            int childSizeBalanceThreshold, ElementMass getEleMass,
                                                            ElementCenterOfMass  getEleCOM,
                                                            ElementInertiaTensor getEleInertia) {
    Vec3d  centerOfMass           = asVec3d(0.0);
    Vec3d  unweightedCenterOfMass = asVec3d(0.0);
    double totalMass              = 0.0;
    ...
    if (std::abs(BasicAlgorithms::sizei(childIDs[0]) - BasicAlgorithms::sizei(childIDs[1])) >
            childSizeBalanceThreshold ||
