...
bool pgo::Mesh::triBoxOverlap(const double boxcenter[3], const double boxhalfsize[3], const double trivert0[3],
                              const double trivert1[3], const double trivert2[3]) {
    ...
    if ((boxhalfsize[0] < minx) || (maxx < -boxhalfsize[0]) || (boxhalfsize[1] < miny) || (maxy < -boxhalfsize[1]) ||
        (boxhalfsize[2] < minz) || (maxz < -boxhalfsize[2]))
...
#undef LOOP1
    ...
    /*
      ...
      for ( int i = 0; i <= 2; i ++ )
      {
        for ( int j = 0; j <= 2; j ++ )
        {
          ...
          double radius = ( boxhalfsize[0] * fabs(a[0]) ) + ( boxhalfsize[1] * fabs(a[1]) ) + ( boxhalfsize[2] *
      fabs(a[2]) );
    ...
    */
    ...
    // no need to normalize the normal as we only check the sign of expressions in the calculations that depend on
    // normal this also makes the code robust against degenerate triangles with zero-length normals double len2 =
    // normal[0] * normal[0] + normal[1] * normal[1] + normal[2] * normal[2]; double invlen = 1.0 / sqrt(len2);
    // normal[0] *= invlen;
    // normal[1] *= invlen;
    // normal[2] *= invlen;
