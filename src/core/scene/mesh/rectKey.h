...
struct ORectKey {
    ...
    inline OEdgeKey getSharedOppositeOEdge(
        const ORectKey& nbr) const;  // return first owned OEdge whose reverse is owned by nbr; return (-1,-1) otherwise
    inline UEdgeKey getSharedUEdge(
        const ORectKey& nbr) const;  // return the first shared UEdge; return (-1,-1) if no shared UEdge
