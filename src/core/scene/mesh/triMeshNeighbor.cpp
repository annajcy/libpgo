...
TriangleNeighbor::TriangleNeighbor(ArrayRef<Vec3i> triangles)
    : numTriangles(triangles.size()), triNbrs(triangles.size(), Vec3i(-1, -1, -1)) {
...
std::vector<int> TriangleNeighbor::findTrianglesArroundBoundaryVertex(int prevVtxID, int vtxID,
                                                                      ArrayRef<Vec3i> triangles) const {
    PGO_ALOG(sizei(triNbrs) == triangles.size());
    std::vector<int> ret;
...
TriMeshNeighbor::TriMeshNeighbor(TriMeshRef triMesh)
    : numVertices(triMesh.numVertices()),
      ...
      vtxNbrTri(triMesh.numVertices()),
      triNbrs(triMesh.numTriangles(), Vec3i(-1, -1, -1)),
      vtxEdgeTri(triMesh.numVertices()) {
    for (int triID = 0; triID < numTriangles; triID++) {
        for (int j = 0; j < 3; j++) {
...
void visitTriangleFanNeighboringVtx(ArrayRef<Vec3i> triangles, const std::unordered_map<OEdgeKey, int>& oedgeTri,
                                    int vtxID, int startingTriID, int localVtxID,
...
std::vector<int> getNonManifoldVerticesOnEdgeManifoldTriangles(ArrayRef<Vec3i>                          triangles,
    ...
    for (int triID = 0; triID < numTriangles; triID++) {
        for (int lvID = 0; lvID < 3; lvID++)  // local vtx ID
        {
            ...
            visitTriangleFanNeighboringVtx(triangles, oedgeTri, curVtxID, triID, lvID,
                                           [&](int nextTriID, int nextlvID) {
...
void fixNonManifoldVerticesOnEdgeManifoldTriangles(TriMeshGeo&                              triMesh,
                                                   const std::unordered_map<OEdgeKey, int>& oedgeTri,
...
std::vector<std::vector<int>> getConnectedComponentsByEdge(ArrayRef<Vec3i> triangles) {
    ...
    auto getNeighbor = [&](int triID) -> const std::vector<int>& { return triNbrs[triID]; };
...
std::vector<std::vector<int>> getConnectedComponentsByVertex(ArrayRef<Vec3i> triangles) {
    ...
    auto getNeighbor = [&](int triID) -> const std::vector<int>& { return triNbrTri[triID]; };
...
std::vector<int> getOneOuterTriMeshConnectedComponentByVertex(TriMeshRef& triMesh) {
    ...
    auto              outerTriIDs =
        findNodeIDsFromSeedBFS(convertArrayToFunction(surfaceTriNbrTri), surfaceTriangleVisited, mvi.index);
