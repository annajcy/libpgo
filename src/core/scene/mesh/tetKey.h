...
struct UTetKey {
    ...
    inline UTriKey getSharedUFace(
        const UTetKey& nbr) const;  // return the first shared UTriKey; return (-1,-1, -1) if no shared UTriKey
    ...
    inline bool hasIndex(int vtxIndex) const {
        return v[0] == vtxIndex || v[1] == vtxIndex || v[2] == vtxIndex || v[3] == vtxIndex;
    }

    // opposite face for each vtx in a tet. The faces are ordered so that its normals pointing outside the tet if tet
    // has positive orientation static const int tetFaceIndex[4][3];
protected:
    Vec4i v;
};
...
struct OTetKey {
    ...
    // permute v0, v1, v2, v3 and store into r0, r1, r2, r3 so that they share the same orientation but r0 =
    // min(v0,v1,v2,v3)
...
inline UTriKey UTetKey::uFaceKey(int ind) const {
    return UTriKey(v[OTetKey::tetFaceIndex[ind][0]], v[OTetKey::tetFaceIndex[ind][1]],
                   v[OTetKey::tetFaceIndex[ind][2]]);
}
