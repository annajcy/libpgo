...
TriMeshPseudoNormal::TriMeshPseudoNormal(TriMeshRef triMesh, const TriMeshNeighbor* externalNbr,
                                         const Vec3d* extTriNormals) {
...
void TriMeshPseudoNormal::updateVertexPositions(TriMeshRef triMesh, const TriMeshNeighbor* externalNbr,
                                                const Vec3d* extTriNormals) {
    buildPseudoNormals(triMesh, externalNbr, extTriNormals);
}

int TriMeshPseudoNormal::buildPseudoNormals(TriMeshRef triMesh, const TriMeshNeighbor* externalNbr,
                                            const Vec3d* extTriNormals) {
    int ret = 0;
    ...
    tbb::parallel_for(0, triMesh.numVertices(), [&](int vtxID) {
        ...
        } else if (nbr->getVtxNearbyTriangles(vtxID).size() >
                   0)  // this vtx has nbr triangles but its computed vtx normal has zero length
        {
...
int TriMeshPseudoNormal::buildPseudoNormals(TriMeshRef triMesh, const Vec3d* extTriNormals) {
    ...
    if (extTriNormals)
        ...
        tbb::parallel_for(
            0, triMesh.numTriangles(), [&](int triID) { triNormals[triID] = triMesh.computeTriangleNormal(triID); },
