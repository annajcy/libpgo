...
// the returned value is positive if queryPoint is above triangle, negative if under and zero if on the plane where the
// triangle lies the returned value is 2 * tri_area * signed_distance
inline double getScaledSignedDistanceToTrianglePlane(const Vec3d& queryPoint, const Vec3d& v0, const Vec3d& v1,
                                                     const Vec3d& v2) {
...
Vec3d getClosestPointToPlaneWithScaledNormal(const Vec3d& queryPoint, const Vec3d& scaledNormal,
                                             const Vec3d& planeStart);
...
// queryPoint does not need to be on the triangle, the computed weight is the weight for the projected version of
// queryPoint on the triangle plane
...
double        getSquaredDistanceToTriangle(const Vec3d& queryPoint, const Vec3d& t0, const Vec3d& t1, const Vec3d& t2,
                                           int& feature);
double        getSquaredDistanceToTriangle(const Vec3d& queryPoint, const Vec3d& t0, const Vec3d& t1, const Vec3d& t2,
                                           int& feature, Vec3d& closestPointOnTriangle, Vec3d& barycentricWeight);
...
// use parallel axis theorem to compute a new inertia tensor from a reference point given the inertia tensor around the
// center of mass note vectorBetweenMassCenterAndReferencePoint can be computed either by from center of mass to
// reference point, or by from reference point to center of mass. It does not matter in parallel axis theorem.
...
inline Mat3d shiftInertiaTensorAroundReferencePointToMassCenter(const Mat3d& inertiaTensorAroundReferencePoint,
                                                                double       mass,
                                                                const Vec3d& vectorBetweenMassCenterAndReferencePoint) {
...
void getUnitMassTriangleInertiaTensorVectorAroundOrigin(const Vec3d& t0, const Vec3d& t1, const Vec3d& t2,
                                                        double tensorVec[6]);

// assume triangle has uniform density of 1
// compute its inertia tensor around the origin
Mat3d getTriangleInertiaTensorAroundOrigin(const Vec3d& t0, const Vec3d& t1, const Vec3d& t2,
...
inline Mat3d getTriangleInertiaTensorAroundCOM(const Vec3d& t0, const Vec3d& t1, const Vec3d& t2,
                                               const Vec3d& triangleCenterOfMass, double triangleSurfaceArea) {
    return getTriangleInertiaTensorAroundOrigin(t0 - triangleCenterOfMass, t1 - triangleCenterOfMass,
                                                t2 - triangleCenterOfMass, triangleSurfaceArea);
}

inline Mat3d getTriangleInertiaTensorAroundCOM(const Vec3d& t0, const Vec3d& t1, const Vec3d& t2) {
    return getTriangleInertiaTensorAroundCOM(t0, t1, t2, getTriangleCenterOfMass(t0, t1, t2),
                                             getTriangleArea(t0, t1, t2));
}

// assume triangle has uniform density of 1
inline void getTriangleInertiaTensorVectorAroundOrigin(const Vec3d& t0, const Vec3d& t1, const Vec3d& t2,
                                                       double triangleArea, double tensorVec[6]) {
...
Mat3d        getTetInertiaTensorAroundCOM(const Vec3d& a, const Vec3d& b, const Vec3d& c, const Vec3d& d,
