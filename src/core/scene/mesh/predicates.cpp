...
inline bool inBoundingBox(const double p[3], const double bbmin[3], const double bbmax[3]) {
    return inClosedRange(p[0], bbmin[0], bbmax[0]) && inClosedRange(p[1], bbmin[1], bbmax[1]) &&
           inClosedRange(p[2], bbmin[2], bbmax[2]);
}
...
double orient2d(const double pa[2], const double pb[2], const double pc[2]) {
    K      k;
    double ret = k.orientation_2_object()(K::Point_2(pa[0], pa[1]), K::Point_2(pb[0], pb[1]), K::Point_2(pc[0], pc[1]));
...
double orient3d(const double pa[3], const double pb[3], const double pc[3], const double pd[3]) {
    K      k;
    double ret = k.orientation_3_object()(K::Point_3(pa[0], pa[1], pa[2]), K::Point_3(pb[0], pb[1], pb[2]),
                                          K::Point_3(pc[0], pc[1], pc[2]), K::Point_3(pd[0], pd[1], pd[2]));
...
double incircle(const double pa[2], const double pb[2], const double pc[2], const double pd[2]) {
    K      k;
    double ret = k.side_of_oriented_circle_2_object()(K::Point_2(pa[0], pa[1]), K::Point_2(pb[0], pb[1]),
                                                      K::Point_2(pc[0], pc[1]), K::Point_2(pd[0], pd[1]));
...
double insphere(const double pa[3], const double pb[3], const double pc[3], const double pd[3], const double pe[3]) {
    K      k;
    double ret = k.side_of_oriented_sphere_3_object()(K::Point_3(pa[0], pa[1], pa[2]), K::Point_3(pb[0], pb[1], pb[2]),
                                                      K::Point_3(pc[0], pc[1], pc[2]), K::Point_3(pd[0], pd[1], pd[2]),
...
bool pointInTet(const double point[3], const double teta[3], const double tetb[3], const double tetc[3],
                const double tetd[3]) {
    K::Tetrahedron_3 tet(K::Point_3(teta[0], teta[1], teta[2]), K::Point_3(tetb[0], tetb[1], tetb[2]),
                         K::Point_3(tetc[0], tetc[1], tetc[2]), K::Point_3(tetd[0], tetd[1], tetd[2]));
...
bool intersectSegTri(const double sega[3], const double segb[3], const double tria[3], const double trib[3],
                     const double tric[3]) {
    K::Segment_3 seg(K::Point_3(sega[0], sega[1], sega[2]), K::Point_3(segb[0], segb[1], segb[2]));

    K::Triangle_3 tri(K::Point_3(tria[0], tria[1], tria[2]), K::Point_3(trib[0], trib[1], trib[2]),
...
bool intersectSegTri(const double sega[3], const double segb[3], const double tria[3], const double trib[3],
                     const double tric[3], double segWeight[2], double triangleWeight[3]) {
    K::Segment_3 seg(K::Point_3(sega[0], sega[1], sega[2]), K::Point_3(segb[0], segb[1], segb[2]));
    ...
    if (ret) {
        ...
        std::array<K::FT, 3> w = CGAL::Polygon_mesh_processing::barycentric_coordinates(tri.vertex(0), tri.vertex(1),
                                                                                        tri.vertex(2), pt, K());
        if (triangleWeight) {
...
bool intersectTriTri(const double pa[3], const double pb[3], const double pc[3], const double qx[3], const double qy[3],
                     const double qz[3]) {
    K::Triangle_3 tri0(K::Point_3(pa[0], pa[1], pa[2]), K::Point_3(pb[0], pb[1], pb[2]),
                       K::Point_3(pc[0], pc[1], pc[2]));

    K::Triangle_3 tri1(K::Point_3(qx[0], qx[1], qx[2]), K::Point_3(qy[0], qy[1], qy[2]),
                       K::Point_3(qz[0], qz[1], qz[2]));

    return CGAL::do_intersect(tri0, tri1);
}

bool intersectTriTet(const double tria[3], const double trib[3], const double tric[3], const double teta[3],
                     const double tetb[3], const double tetc[3], const double tetd[3]) {
    K::Triangle_3 tri(K::Point_3(tria[0], tria[1], tria[2]), K::Point_3(trib[0], trib[1], trib[2]),
                      K::Point_3(tric[0], tric[1], tric[2]));
...
bool intersectTriAABB(const double ta[3], const double tb[3], const double tc[3], const double bmin[3],
                      const double bmax[3]) {
    K::Triangle_3 tri(K::Point_3(ta[0], ta[1], ta[2]), K::Point_3(tb[0], tb[1], tb[2]),
                      K::Point_3(tc[0], tc[1], tc[2]));

    K::Iso_cuboid_3 bb(K::Point_3(bmin[0], bmin[1], bmin[2]), K::Point_3(bmax[0], bmax[1], bmax[2]));

    return CGAL::do_intersect(tri, bb);
}

bool intersectTetAABB(const double teta[3], const double tetb[3], const double tetc[3], const double tetd[3],
...
bool intersectTetTet(const double tet1a[3], const double tet1b[3], const double tet1c[3], const double tet1d[3],
    ...
    K::Tetrahedron_3 tet1(K::Point_3(tet1a[0], tet1a[1], tet1a[2]), K::Point_3(tet1b[0], tet1b[1], tet1b[2]),
                          K::Point_3(tet1c[0], tet1c[1], tet1c[2]), K::Point_3(tet1d[0], tet1d[1], tet1d[2]));

    K::Tetrahedron_3 tet2(K::Point_3(tet2a[0], tet2a[1], tet2a[2]), K::Point_3(tet2b[0], tet2b[1], tet2b[2]),
                          K::Point_3(tet2c[0], tet2c[1], tet2c[2]), K::Point_3(tet2d[0], tet2d[1], tet2d[2]));
...
bool intersectSegSeg2d(const double sa[2], const double sb[2], const double ta[2], const double tb[2]) {
    K::Segment_2 seg1(K::Point_2(sa[0], sa[1]), K::Point_2(sb[0], sb[1]));

    K::Segment_2 seg2(K::Point_2(ta[0], ta[1]), K::Point_2(tb[0], tb[1]));

    return CGAL::do_intersect(seg1, seg2);
}

bool intersectSegSeg2d(const double sa[2], const double sb[2], const double ta[2], const double tb[2], double sw[2],
                       double tw[2]) {
    K::Segment_2 seg1(K::Point_2(sa[0], sa[1]), K::Point_2(sb[0], sb[1]));
...
double inspherefast(const double pa[3], const double pb[3], const double pc[3], const double pd[3],
                    const double pe[3]) {
...
int simplex_intersection3d(int k, const double* x0, const double* x1, const double* x2, const double* x3,
                           const double* x4, double* alpha0, double* alpha1, double* alpha2, double* alpha3,
                           double* alpha4);
...
bool intersectSegTriWithOrientOnSeg(const double sega[3], const double segb[3], const double trip[3],
                                    const double triq[3], const double trir[3], double osega, double osegb) {
...
bool pointInTet(const double point[3], const double teta[3], const double tetb[3], const double tetc[3],
                const double tetd[3]) {
    double alpha[5];
    return simplex_intersection3d(1, point, teta, tetb, tetc, tetd, alpha, alpha + 1, alpha + 2, alpha + 3,
                                  alpha + 4) != 0;
}
...
bool intersectSegTri(const double sega[3], const double segb[3], const double tria[3], const double trib[3],
                     const double tric[3], double segWeight[2], double triangleWeight[3]) {
    return simplex_intersection3d(2, sega, segb, tria, trib, tric, segWeight, segWeight + 1, triangleWeight,
                                  triangleWeight + 1, triangleWeight + 2) != 0;
}
...
bool intersectTriTri(const double pa[3], const double pb[3], const double pc[3], const double qx[3], const double qy[3],
    ...
    //
    //  auto intersectSegTriWithOrient = [&](const double sega[3], const double segb[3], const double trip[3], const
    //  double triq[3], const double trir[3],
    //      double osega, double osegb, int oabpqID, int oabqrID, int oabrpID)->bool {
    //    if ((osega > 0 && osegb > 0) || (osega < 0 && osegb < 0)) return false;
    //
    //    auto computeOrient = [&otherOrientComputed, &otherOrient](const double a[3], const double b[3], const double
    //    c[3], const double d[3], int ID) -> double {
    //      double alpha = 0.0;
...
bool intersectSegAABBFace(int d, const double sa[3], const double sb[3], const double ra[3], const double rb[3],
                          const double rc[3], const double rd[3]) {
    double x = ra[d];
    if (sa[d] < x && sb[d] < x)
        return 0;
    if (sa[d] > x && sb[d] > x)
    ...
    if (simplex_intersection3d(2, sa, sb, ra, rb, &a2, &a3, &a4,
                               &a5))  // whether segment sa, sb intersect segment ra, rb
    {
        return 1;
    }
    if (simplex_intersection3d(2, sa, sb, rb, rc, &a2, &a3, &a4,
                               &a5))  // whether segment sa, sb intersect segment rb, rc
    {
        return 1;
    }
    if (simplex_intersection3d(2, sa, sb, rc, rd, &a2, &a3, &a4,
                               &a5))  // whether segment sa, sb intersect segment rc, rd
    {
        return 1;
    }
    if (simplex_intersection3d(2, sa, sb, rd, ra, &a2, &a3, &a4,
                               &a5))  // whether segment sa, sb intersect segment rd, ra
    {
        return 1;
    }
    return 0;
...
bool intersectTriAABBFace(int d, const double ta[3], const double tb[3], const double tc[3],  // t for triangle
                          const double ra[3], const double rb[3], const double rc[3],
                          const double rd[3])  // r for rectangle
{
    //  return intersectSegTri(pa, pb, qa, qb, qc) || intersectSegTri(pa, pc, qa, qb, qc) || intersectSegTri(pb, pc, qa,
    //  qb, qc) ||
    //      intersectSegTri(qa, qb, pa, pb, pc) || intersectSegTri(qa, qc, pa, pb, pc) || intersectSegTri(qb, qc, pa,
    //      pb, pc);
    return intersectSegAABBFace(d, ta, tb, ra, rb, rc, rd) || intersectSegAABBFace(d, ta, tc, ra, rb, rc, rd) ||
           intersectSegAABBFace(d, tb, tc, ra, rb, rc, rd) || intersectSegTri(ra, rb, ta, tb, tc) ||
           intersectSegTri(rb, rc, ta, tb, tc) || intersectSegTri(rc, rd, ta, tb, tc) ||
...
// const double p[8][3] = { {bmin[0], bmin[1], bmin[2]}, { bmax[0], bmin[1], bmin[2] }, { bmax[0], bmax[1], bmin[2] }, {
// bmin[0], bmax[1], bmin[2] },
//                        { bmin[0], bmin[1], bmax[2] }, { bmax[0], bmin[1], bmax[2] }, {bmax[0], bmax[1], bmax[2]}, {
...
bool intersectTriAABB(const double ta[3], const double tb[3], const double tc[3], const double bmin[3],
    ...
    if ((bmax[0] < minx) || (maxx < bmin[0]) || (bmax[1] < miny) || (maxy < bmin[1]) || (bmax[2] < minz) ||
        (maxz < bmin[2]))
        return false;  // no overlap

    // create a box mesh for AABB
    const double p[8][3] = {{bmin[0], bmin[1], bmin[2]}, {bmax[0], bmin[1], bmin[2]}, {bmax[0], bmax[1], bmin[2]},
                            {bmin[0], bmax[1], bmin[2]}, {bmin[0], bmin[1], bmax[2]}, {bmax[0], bmin[1], bmax[2]},
    ...
    if (intersectSegAABBAllFaces(ta, tb, p) || intersectSegAABBAllFaces(ta, tc, p) ||
    ...
    //  if (intersectTriAABBFace(0, ta, tb, tc, p[0], p[4], p[7], p[3])) return true; // check intersection with face
    //  0-4-7-3 if (intersectTriAABBFace(0, ta, tb, tc, p[1], p[2], p[6], p[5])) return true;
    //
...
bool intersectTetAABB(const double ta[3], const double tb[3], const double tc[3], const double td[3],
                      const double bmin[3], const double bmax[3]) {
    assert(isValidBoundingBox(bmin, bmax));
    if (inBoundingBox(ta, bmin, bmax))
        return true;
    ...
    if (intersectSegAABBAllFaces(ta, tb, p) || intersectSegAABBAllFaces(ta, tc, p) ||
        intersectSegAABBAllFaces(ta, td, p) || intersectSegAABBAllFaces(tb, tc, p) ||
        intersectSegAABBAllFaces(tb, td, p) || intersectSegAABBAllFaces(tc, td, p))
        return true;

    // for each bounding box edge, check whether it intersects with the tet face
    if (intersectTriAABBAllEdges(ta, tb, tc, p) || intersectTriAABBAllEdges(ta, tb, td, p) ||
        intersectTriAABBAllEdges(ta, tc, td, p) || intersectTriAABBAllEdges(tb, tc, td, p))
        return true;
