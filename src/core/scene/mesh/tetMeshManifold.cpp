...
const TetMeshManifold::Tetrahedron* TetMeshManifold::add(int v0, int v1, int v2, int v3, int id) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (triit == triangles.end())  // this triangle face is not inside triangles; it's new
        {
            ...
            if (surface.find(otrikeys[i].getReversedTriKey()) ==
                surface.end())  // the orientation of this face is wrong
            {
                fail = true;
                break;
            }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (tri == nullptr) {
            ...
            // we don't add okey to surfaceManifold here now because this might cause the surfaceManifold to become
            // non-manifold
