...
//  /\  /\  /\     //
// /__\/__\/__\    //
// In 2D, for subdivision k, the number of sub-triangles are: 1 + 3 + 5 + ... + (2k-1), where the first row has 1
// sub-trianlge, second row 3, ...
//                                                           = k*k
...
public:
    ...
    using SamplePositionKernel =
        std::function<void(int i, int j, double area, const Vec3d& baryWeight, const Vec3d& pos)>;
    ...
    void visitSample(const Vec3d& triPosA, const Vec3d& triPosB, const Vec3d& triPosC, double triangleSurfaceArea,
                     Kernel f) const;
...
inline TriangleSampler::TriangleSampler(int N) : N(N), cornerIDs{Vec2i(0, 0), Vec2i(N, 0), Vec2i(N, N)} {
    invN               = 1.0 / N;
...
template <class Kernel>
void TriangleSampler::visitSample(const Vec3d& triPosA, const Vec3d& triPosB, const Vec3d& triPosC,
                                  double triangleSurfaceArea, Kernel kernel) const {
    ...
    for (int i = 0; i <= N; i++, basei += vec1) {
        ...
        for (int j = 0; j <= i; j++, p += vec2) {
            const Vec3d baryWeight(1 - i * invN, (i - j) * invN,
                                   j * invN);  // this wieght is also correct, no matter how the triangle deformes
...
inline void TriangleSampler::getBarycentricWeightFromKey(const UTriKey& sampleKey, Vec3i& vtxIndex,
                                                         Vec3d& barycentricWeight) const {
