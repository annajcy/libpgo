...
bool TetMeshRef::save(const std::string& filename) const {
    ...
    std::cout << "Saved mesh (#v: " << numVertices() << ", #t: " << numTets() << ") to " << filename << "."
              << std::endl;
    return true;
}
...
TetNeighbor::TetNeighbor(const std::vector<Vec4i>& tets)
    : nTets(static_cast<int>(tets.size())), tetNbrs(tets.size(), Vec4i(-1)) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            if (otriTet.find(key) != otriTet.end()) {
                std::cout << "Error, the tet neighbor is not manifold, tetID " << tetID << " tet " << tets[tetID] << " "
                          << "key " << key << " prevTetID: " << otriTet[key] << " prevTet " << tets[otriTet[key]]
                          << std::endl;
...
NonManifoldTetNeighbor::NonManifoldTetNeighbor(const std::vector<Vec4i>& tets) : nTets(static_cast<int>(tets.size())) {
    for (int tetID = 0; tetID < nTets; tetID++) {
        OTetKey tetkey(tets[tetID]);
        for (int j = 0; j < 4; j++) {
            int     v0  = tets[tetID][j];
