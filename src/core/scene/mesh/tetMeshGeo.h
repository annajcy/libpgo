...
public:
    ...
    double computeDistanceToTet(int tetID, const Vec3d& queryPos) const {
        return std::sqrt(computeSquaredDistanceToTet(tetID, queryPos));
    }
...
public:
    ...
    inline std::vector<std::pair<int, OTriKey>> findTetBoundaries(const std::vector<Vec4i>& tets) const {
        return findTetBoundaries((int)(tets.size()), tets.data());
    }
