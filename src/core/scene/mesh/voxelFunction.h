...
public:
    ...
    inline Vec3d getGridPosition(const Vec3i& gridIndex) const {
        return getGridPosition(gridIndex[0], gridIndex[1], gridIndex[2]);
    }
    inline Vec3d getGridPosition(const packedIndex packedGridIndex) const {
        Vec3i g = unpackGridIndex(packedGridIndex);
        return getGridPosition(g);
    }
...
public:
    ...
    typename std::vector<T>::const_reference value(const Vec3i& voxelIndex) const {
        return buffer[packedVoxelIndex(voxelIndex)];
    }

    typename std::vector<T>::reference       value(packedIndex packedVoxelIndex) { return buffer[packedVoxelIndex]; }
    typename std::vector<T>::const_reference value(packedIndex packedVoxelIndex) const {
        return buffer[packedVoxelIndex];
    }
...
inline bool VoxelGrid::voxelInsideBox(const Vec3i& index) const {
    return index[0] >= 0 && index[0] < resolution[0] && index[1] >= 0 && index[1] < resolution[1] && index[2] >= 0 &&
...
inline bool VoxelGrid::gridInsideBox(const Vec3i& gridIndex) const {
    return gridIndex[0] >= 0 && gridIndex[0] < gridResolution[0] && gridIndex[1] >= 0 &&
           gridIndex[1] < gridResolution[1] && gridIndex[2] >= 0 && gridIndex[2] < gridResolution[2];
}
