...
namespace pgo {
namespace Mesh {
class TriangleWithCollisionInfo;  // a better triangle class which pre-computes necessary data for fast intersection
                                  // queries
...
public:
    ...
    // build the octree
    void buildOctree(const BasicAlgorithms::ArrayRef<Vec3d> verticesList, int maxDepth = 10,
                     int maxNumVerticesPerNode = 10);

    // Build a BV tree by partitioning the mesh along a primary axis. The primary axis is computed from doing SVD on the
    // inertia tensor of the vertex group. Each tree node will have at most two children.
    void buildByInertiaPartition(const BasicAlgorithms::ArrayRef<Vec3d> verticesList, int maxDepth = INT_MAX,
                                 int maxNumTrianglesPerNode = 5);

    // query vertices inside simpleSphere/halfSpace with double-precision
    // vertexIDList are not sorted or deduplicated
    void rangeQuery(const BasicAlgorithms::ArrayRef<Vec3d> vertices, const SimpleSphere& simpleSphere,
                    std::vector<int>& vertexIDList) const;
    void rangeQuery(const BasicAlgorithms::ArrayRef<Vec3d> vertices, const HalfSpace& halfSpace,
                    std::vector<int>& vertexIDList) const;
...
public:
    ...
    // Build a BV tree by partitioning the boxes along a primary axis. The primary axis is computed from doing SVD on
    // the inertia tensor of the boxes. Each tree node will have at most two children.
    void buildByInertiaPartition(const BasicAlgorithms::ArrayRef<BoundingBox> boundingBoxes, int maxDepth = INT_MAX,
                                 int maxNumBBsPerNode = 5);
    ...
    void getClosestBoundingBoxes(const BasicAlgorithms::ArrayRef<BoundingBox> boundingBoxes, const Vec3d& queryPosition,
                                 std::vector<int>& bbIDList) const;

    void intersectAABB(const BasicAlgorithms::ArrayRef<BoundingBox> vertices, const BoundingBox& bb,
                       std::vector<int>& bbIDList) const;
...
public:
    ...
    // Inexact bounding volume hierarchy will result in some triangles missing from its bounding box node if they are
    // just touching the bounding box or they completely lie on the surface of the bounding box. An exact bounding
    // volume hierarchy is needed if you want exact intersection An inexact bounding volume hierarchy is acceptable if
    // you just want a rough estimation on the distance between a point and the triangle octree
    ...
    void buildInexactOctree(const TriMeshRef triMesh, const BoundingBox& initialBoundingBox, int maxDepth,
                            int maxNumTrianglesPerNode);

    // Build a BV tree by partitioning the mesh along a primary axis. The primary axis is computed from doing SVD on the
    // inertia tensor of the mesh. Each tree node will have at most two children.
    ...
    void lineSegmentIntersectionExact(const TriMeshRef triMesh, Vec3d segStart, Vec3d segEnd,
                                      std::vector<int>& triangleIDList) const;
    void triangleIntersectionExact(const TriMeshRef triMesh, Vec3d t0, Vec3d t1, Vec3d t2,
                                   std::vector<int>& triangleIDList) const;
    ...
    // buildExactOctree() or buildByInertiaPartition() is called, it cannot guarantee the returned triangle is the first
    // to be hit, since the comparison on which is the first is done in double-precision. For the default version, the
    // computation is done in double-precision so it can happen that intersections are missing.
    ...
    int lineSegmentFirstIntersectionPointSemiExact(const TriMeshRef triMesh, Vec3d segStart, Vec3d segEnd,
                                                   double segWeight[2] = nullptr, double triWeights[3] = nullptr,
                                                   std::stack<int>* externalStack = nullptr) const;

    // if skippedTriIDs is not nullptr, it must be pre-sorted
    // triangles in skippedTriIDs will be skipped during the search
    bool hasLineSegmentIntersectionExact(const TriMeshRef triMesh, Vec3d segStart, Vec3d segEnd,
                                         std::vector<int>* skippedTriIDs = nullptr) const;
    ...
    int getClosestTriangle(const TriangleWithCollisionInfo* triangles, const Vec3d& queryPosition,
                           int* feature = nullptr, int warmStartTriID = -1,
    ...
    ClosestTriangleQueryResult closestTriangleQuery(const TriMeshRef triMesh, const Vec3d& queryPosition,
                                                    ...
                                                    double distance2HigherBound = DBL_MAX) const;
    ClosestTriangleQueryResult closestTriangleQuery(
        const TriMeshRef triMesh, const Vec3d& queryPosition,
    ...
    //  void closestTriangleQueryFromTriMeshVertices(const TriangleWithCollisionInfo * triangles, const TriMeshBVTree &
    //  otherBVTree,
...
protected:
    ...
    std::pair<int, double> getClosestTriangleHelper(GetTriangleDistance2, const Vec3d& queryPosition, int* feature,
                                                    int warmStartTriID, double distance2HigherBound,
...
public:
    ...
    int getClosestTet(const TetMeshRef tetMesh, const Vec3d& queryPosition,
                      std::vector<std::tuple<double, double, int>>& nodeStack) const;
    ...
    // otherwise, return -1
    //  int getClosestTetIfInsideBoundingBox(const TetMeshRef tetMesh, const Vec3d & queryPosition, int * feature =
    //  nullptr, int warmStartTriID = -1) const;
