...
// isTetOuter returns true if tetID is outside the surface. One way to detect this fast is to use winding number. It
// will get called at most once per tet return a vector of size numTets, each bool represents whether tetID is outer
...
std::vector<bool> labelOuterTets(const TetMeshRef& tetMesh, const TetNeighbor& tetNeighbor,
                                 std::function<bool(int tetID)> isTetBoundary,
                                 std::function<bool(int tetID)> isTetOuter);
}  // namespace Mesh
}  // namespace pgo
