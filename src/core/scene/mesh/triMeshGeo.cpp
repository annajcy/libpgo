...
double TriMeshRef::computeWindingNumber(const Vec3d& queryPos) const {
    ...
    for (int j = 0; j < numTriangles(); j++) {
        ...
        double omega =
            2 * std::atan2(mat.determinant(), (la * lb * lc + a.dot(b) * lc + b.dot(c) * la + c.dot(a) * lb));
...
bool saveToAscii(const TriMeshRef& mesh, const std::string& filename) {
    ...
    std::cout << "Saved mesh (#v: " << mesh.numVertices() << ", #t: " << mesh.numTriangles() << ") to " << filename
              << "." << std::endl;
    fout.close();
    return true;
}
...
std::vector<Vec3d> removeIdenticalVertices(const std::vector<Vec3d>& vertices, std::vector<int>* newVtxID2OldVtxID,
...
TriMeshGeo removeIsolatedVertices(const TriMeshRef meshRef, std::vector<int>* newVtxID2OldVtxID,
                                  std::vector<int>* oldVtxID2NewVtxID) {
...
TriMeshGeo removeIsolatedVerticesWithMap(const TriMeshRef meshRef, std::vector<int>* newVtxID2OldVtxID,
                                         std::map<int, int>* oldVtxID2NewVtxID) {
    std::vector<int> usedVtxIDs;
    for (int triID = 0; triID < meshRef.numTriangles(); triID++) {
        for (int i = 0; i < 3; i++)
            usedVtxIDs.push_back(meshRef.tri(triID)[i]);
