...
public:
    ...
    inline double distanceToPoint(const Vec3d& point) const {
        return std::sqrt(TriangleBasic::distanceToPoint2(point));
    }
    ...
    int lineSegmentIntersection(const Vec3d& segmentStart, const Vec3d& segmentEnd, Vec3d* intersectionPoint = nullptr,
                                double* t = nullptr, double barycentricWeight[3] = nullptr) const;

    Vec3d getBarycentricLocation(double alpha, double beta, double gamma) const {
        return alpha * vertex[0] + beta * vertex[1] + gamma * vertex[2];
    }
...
public:
    TriangleWithCollisionInfo(const Vec3d& first_g, const Vec3d& second_g, const Vec3d& third_g)
        : TriangleBasic(first_g, second_g, third_g) {
    ...
    double distanceToPoint2(
        const Vec3d& point, int* closestFeature, double* alpha = nullptr, double* beta = nullptr,
        double* gamma = nullptr) const;  // also returns the barycentric coordinates of the closest point
    inline double distanceToPoint2(const Vec3d& point) const {
        int f = -1;
        return distanceToPoint2(point, &f);
    }
    inline double distanceToPoint(const Vec3d& point) const {
        return std::sqrt(TriangleWithCollisionInfo::distanceToPoint2(point));
    }

protected:
    // note: the following collision detection parameters are pre-computed with respect to a permuted set of triangle
    // indices (a cycle)
    Mat3d  Q     = Mat3d::Zero();
    Vec3d  x0    = Vec3d::Zero();
    double sidea = 0, sideb = 0, sidec = 0, area = 0;
    Vec3d  S1 = Vec3d::Zero(), S2 = Vec3d::Zero(), N11 = Vec3d::Zero(), N12 = Vec3d::Zero(), N21 = Vec3d::Zero(),
          N22 = Vec3d::Zero();
...
public:
    ...
    TriangleWithCollisionInfoAndPseudoNormals(const Vec3d& first_g, const Vec3d& second_g, const Vec3d& third_g,
                                              const Vec3d pseudoNormals[7]);
    ...
    inline Vec3d        interpolatedVertexPseudoNormal(double alpha, double beta, double gamma) const {
        return (alpha * pseudoNormal_[0] + beta * pseudoNormal_[1] + gamma * pseudoNormal_[2]).normalized();
    }

    // for vertices, returns the vertex itself
    // for edges, returns the midpoint of the edge (for the purposes of distance sign test, this could be any point on
    // the edge) for faces, returns the face centroid
    inline const Vec3d& pseudoClosestPosition(int closestFeature) const {
        return pseudoClosestPosition_[closestFeature];
    }
...
template <typename T>
concept IsTriangle = is_decay_same<T, TriangleBasic> || is_decay_same<T, TriangleWithCollisionInfo> ||
                     is_decay_same<T, TriangleWithCollisionInfoAndPseudoNormals>;
