...
public:
    ...
    BasicAlgorithms::ArrayRef<Vec3d> positionsRef() const {
        return BasicAlgorithms::makeArrayRef(numVertices_, positions_);
    }
    BasicAlgorithms::ArrayRef<Vec3i> trianglesRef() const {
        return BasicAlgorithms::makeArrayRef(numTriangles_, triangles_);
    }
    ...
    inline Vec3d getSurfacePositionWithBarycentricWeights(const Vec3i& vtxIDs, const Vec3d& weight,
                                                          const Vec3d* vtxDisp) const;
...
// get the shared vertex IDs from two triangles, assuming input t0 and t1 are valid triangles (v[i] >= 0 && v[i] !=
// v[j]) if no vtx shared, return {-1, -1, -1} if v0 is shared, return {v0, -1, -1} if v0, v1 are shared, return {v0,
// v1, -1} if v0, v1, v2 are shared, return {v0, v1, v2}
...
std::vector<Vec3d> removeIdenticalVertices(const std::vector<Vec3d>& vertices,
                                           std::vector<int>*         newVtxID2OldVtxID = nullptr,
...
TriMeshGeo getSubTriMeshOnlyOnSortedSubVertexIDs(const TriMeshRef&              mesh,
                                                 BasicAlgorithms::ArrayRef<int> sortedSubVtxIDs);
// return triangle IDs in the sub mesh
void getSubTriMeshOnlyOnSortedSubVertexIDs(const TriMeshRef& mesh, BasicAlgorithms::ArrayRef<int> sortedSubVtxIDs,
                                           std::vector<int>& outputTriangleIDs);
...
inline Vec3d TriMeshRef::getSurfacePositionWithBarycentricWeights(const Vec3i& vtxIDs, const Vec3d& w,
                                                                  const Vec3d* vtxDisp) const {
    Vec3d p;
    p.setZero();
    for (int i = 0; i < 3; i++) {
