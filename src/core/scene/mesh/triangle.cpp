...
void TriangleWithCollisionInfo::ComputeCollisionInfo() {
    ...
    y = Q * p2 + x0;
    if (fabs(y[2]) > 1E-5) {
        std::cout << "Warning: transformation does not map p2 vector into the xy plane (z=" << y[2] << ")."
                  << std::endl;
    }
    if (y[1] < 0) {
        std::cout << "Warning: transformation does not map p2 vector into the upper half of the xy plane. (y=" << y[1]
                  << ")." << std::endl;
    }
    if (y[0] < 0) {
        std::cout << "Warning: transformation does not map p2 vector into the positive right half of the xy plane. (x="
                  << y[0] << ")." << std::endl;
    }
    if (y[0] > sidea) {
        std::cout
            << "Warning: transformation does not map p2 vector into a vector with smaller x-coordinate than sidea="
            << sidea << " (x=" << y[0] << ")." << std::endl;
    }
...
double TriangleWithCollisionInfo::distanceToPoint2(const Vec3d& point, int* closestFeature, double* alpha, double* beta,
                                                   double* gamma) const {
    ...
    */
    ...
    double* barycentricCoords[3]    = {barycentricCoordsRaw[permutation[0]], barycentricCoordsRaw[permutation[1]],
                                       barycentricCoordsRaw[permutation[2]]};
...
TriangleWithCollisionInfoAndPseudoNormals::TriangleWithCollisionInfoAndPseudoNormals(const Vec3d& first_g,
                                                                                     const Vec3d& second_g,
                                                                                     const Vec3d& third_g,
...
template void makeUniqueList<TriangleBasic>(const std::vector<TriangleBasic*>& triangleList,
                                            std::vector<TriangleBasic*>&       uniqueList);

template void makeUniqueList<TriangleWithCollisionInfo>(std::vector<TriangleWithCollisionInfo*>& triangleList);
template void makeUniqueList<TriangleWithCollisionInfo>(const std::vector<TriangleWithCollisionInfo*>& triangleList,
                                                        std::vector<TriangleWithCollisionInfo*>&       uniqueList);

template void makeUniqueList<TriangleWithCollisionInfoAndPseudoNormals>(
    std::vector<TriangleWithCollisionInfoAndPseudoNormals*>& triangleList);
template void makeUniqueList<TriangleWithCollisionInfoAndPseudoNormals>(
    const std::vector<TriangleWithCollisionInfoAndPseudoNormals*>& triangleList,
    std::vector<TriangleWithCollisionInfoAndPseudoNormals*>&       uniqueList);
