...
std::vector<std::vector<int>> pgo::Mesh::computeTrianglesIntersectingEachTetExact(const TetMeshRef     tetMesh,
                                                                                  const TriMeshRef     triMesh,
                                                                                  const TriMeshBVTree& triMeshBVTree) {
    ...
    tbb::parallel_for(0, tetMesh.numTets(), [&](int tetID) {
        ...
        auto toBB  = [&](const BoundingBox& bb) { return (tetbb.intersect(bb)); };
        auto toTri = [&](int tri) {
