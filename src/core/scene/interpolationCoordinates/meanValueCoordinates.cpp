...
MeanValueCoordinates::MeanValueCoordinates(int numLocations_, const double* locations_, int numCageVertices_,
                                           const double* cageVertices_, int numCageTriangles_,
                                           const int* cageTriangles_) {
        ...
        [&](const tbb::blocked_range<int>& rng) {
            ...
            for (int i = rng.begin(); i != rng.end(); ++i) {
                ...
                for (int j = 0; j < numCageTriangles; j++) {
                    ...
                    for (int k = 0; k < 3; k++) {
                        c[k] =
                            (2 * sin(h) * sin(h - theta[k])) / (sin(theta[(k + 1) % 3]) * sin(theta[(k + 2) % 3])) - 1;
                    }
                    ...
                    for (int k = 0; k < 3; k++) {
                        double w =
                            (theta[k] - c[(k + 1) % 3] * theta[(k + 2) % 3] - c[(k + 2) % 3] * theta[(k + 1) % 3]) /
                            (d[pi[k]] * sin(theta[(k + 1) % 3]) * s[(k + 2) % 3]);
