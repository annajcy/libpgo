...
GreenCoordinates::GreenCoordinates(int numLocations, const double* locations, const Mesh::TriMeshRef& cage) {
    ...
    tbb::parallel_for(
        tbb::blocked_range<int>(0, numLocations),
            ...
            for (int i = rng.begin(); i != rng.end(); ++i) {
                ...
                for (int fi = 0; fi < cage.numTriangles(); fi++) {
                    ...
                    for (int k = 0; k < 3; k++) {
                        ...
                        N[k] =
                            ((q[k].squaredNorm() < std::numeric_limits<double>::epsilon()) ? zero : q[k].normalized());
                    }
                    ...
                    if ((w).squaredNorm() > std::numeric_limits<double>::epsilon()) {
                        for (int k = 0; k < 3; k++) {
                            ...
                            vtxWeights[i * numVertices + pidx] +=
                                ((N[next].dot(v[k]) == 0) ? 0 : N[next].dot(w) / N[next].dot(v[k]));
                        }
                    }
        ...
        [&](const tbb::blocked_range<int>& rng) {
            for (int i = rng.begin(); i != rng.end(); ++i) {
                ...
                for (int j = 0; j < numTriangles; j++) {
                    ...
                    double s = sqrt(newVec[0].squaredNorm() * oldVec[1].squaredNorm() -
                                    2 * newVec[0].dot(newVec[1]) * oldVec[0].dot(oldVec[1]) +
                                    newVec[1].squaredNorm() * oldVec[0].squaredNorm()) /
                               (sqrt(8) * triangleArea[j]);
...
double GreenCoordinates::GCTriInt(Vec3d& p, Vec3d& v1, Vec3d& v2, Vec3d& n) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        double C = cos(t[i]);
        I[i]     = (-0.5) * sign(S) *
               (2 * sqrtc * atan(sqrtc * C / sqrt(lambda + S * S * c)) +
                sqrtlambda * log(2 * sqrtlambda * S * S *
                                 (1 - 2 * c * C / (c * (1 + C) + lambda + sqrt(lambda * lambda + lambda * c * S * S))) /
                                 ((1 - C) * (1 - C))));
    }
