...
void write_binary_mesh(const MeshT& mesh, std::ostream& out, unsigned int* bytes_written) {
    unsigned int total_bytes_written = 0;

    auto count_write = [&](const auto* data, size_t count) {
        using T = std::remove_pointer_t<decltype(data)>;
        detail::write_exact(out, data, count);
...
template <concepts::VolumetricMeshLike MeshT>
void write_binary_mesh(const MeshT& mesh, const std::filesystem::path& filename, unsigned int* bytes_written) {
