...
void VolumetricMesh::loadFromAscii(const std::filesystem::path& filename, ElementType* elementType_, int verbose) {
    ...
    std::map<std::string, int> materialMap;  // establishes a relationship between material's string name and its index
                                             // in the "materials" array
    std::map<std::string, int>
        setMap;  // establishes a relationship between set's string name and its index in the "sets" array
    ...
    while (volumetricMeshParser.getNextLine(lineBuffer, 0, 0) != nullptr) {
            ...
            *(materialType + materialTypeLength) = 0;
            ...
            if (strcmp(materialType, "ENU") == 0) {
                ...
                if ((sscanf(ch, "%lf,%lf,%lf,%lf", &density, &E1, &E2, &E3) == 4) &&
                    ...
                    if (useNuAndG && (nu > -1.0) &&
                        (nu < 0.5))  // if nu is not in this range, then G_ij is still 0, finally produce an error
                    {
                        // formulas from:
        ...
        if (parseState == 11) {
            ...
            // we know that lineBuffer[0] != '*' (i.e., not the end of the list), as that case was already previously
            // handled
            ...
            while ((pch != nullptr) && (isdigit(*pch))) {
                ...
                sets[countNumSets - 1].insert(
                    ind);  // sets are 0-indexed, but .veg files may be 1-indexed (oneIndexedElements == 1)
                pch = strtok(nullptr, ",");
            }
        }
...
void VolumetricMesh::loadFromBinaryGeneric(void* binaryInputStream_, ElementType* elementType_, int memoryLoad) {
    ...
    if (numElementVertices > 0 && numEleVer != numElementVertices) {
        printf(
            "Error in VolumetricMesh::loadFromBinaryGeneric: mismatch in the number of vertices of every element from "
            "file stream.\n");
        throw 5;
    }
    ...
    // input all elements
    elements.resize(numElements * numElementVertices);
    if ((int)genericRead(elements.data(), sizeof(int), numElementVertices * numElements, binaryInputStream) !=
        numElementVertices * numElements) {
    ...
    for (int setIndex = 1; setIndex < numSets; setIndex++) {
        ...
        if ((int)genericRead(&cardinality, sizeof(int), 1, binaryInputStream) != 1) {
            printf(
                "Error in VolumetricMesh::loadFromBinaryGeneric: cannot read the number of elements in current set.\n");
            throw 0;
        }
    ...
    for (int materialIndex = 0; materialIndex < numMaterials; materialIndex++) {
        ...
        if (materials[materialIndex]->getType() == MaterialType::Orthotropic) {
            ...
            fprintf(fout,
                    ...
                    density, E1, E2, E3, nu12, nu23, nu31, G12, G23, G31, R[0], R[1], R[2], R[3], R[4], R[5], R[6],
                    R[7], R[8]);
        }
...
unsigned int VolumetricMesh::readFromMemory(void* buf, unsigned int elementSize, unsigned int numElements,
...
int VolumetricMesh::saveToBinary(FILE* binaryOutputStream, unsigned int* bytesWritten, ElementType elementType_,
    ...
    for (int materialIndex = 0; materialIndex < numMaterials; materialIndex++) {
        ...
        if (!countBytesOnly)
            itemsWritten =
                fwrite(&length, sizeof(int), 1, binaryOutputStream);  // write the length of the material name
        if (itemsWritten < 1)
            return 1;
        totalBytesWritten += itemsWritten * sizeof(int);
...
// if vertexMap is non-null, it also returns a renaming datastructure: vertexMap[big mesh vertex] is the vertex index in
// the subset mesh
void VolumetricMesh::setToSubsetMesh(const std::set<int>& subsetElements, int toRemoveIsolatedVertices,
                                     std::map<int, int>* vertexMap) {
    // oldEleID -> newEleID
    ...
    for (int tail = 0; tail < numElements; tail++) {
        if (subsetElements.find(tail) != subsetElements.end()) {
            if (head != tail) {
                memcpy(&elements[head * numElementVertices], &elements[tail * numElementVertices],
