...
NaturalCubicSpline2DWithParameterDerivatives::NaturalCubicSpline2DWithParameterDerivatives(int           numPoints,
...
void NaturalCubicSpline2DWithParameterDerivatives::d2y_dparam_dx(double x, const double* yValues,
                                                                 double* dparam) const {
    // if x is left outside of the spline
    if (x < xNodeValue[0]) {
        ES::V4d dk_dabcd(0.0, 0.0, 1.0, 0.0);
...
NaturalCubicSpline2DAsNonlinearConstraints::NaturalCubicSpline2DAsNonlinearConstraints(int np, int xoff, int yoff,
                                                                                       int poff, int na, double y0Prime,
                                                                                       double ynPrime)
...
void NaturalCubicSpline2DAsNonlinearConstraints::func(ES::ConstRefVecXd x, ES::RefVecXd g) const {
    ...
    // fi' (x_i+1) - f'_{i+1}(x_i+1) = 0
    for (int i = 0; i < numPoints - 2; i++) {
        g[si++] = compute_fi_1stDeriv(getParami(x, i), getxi(x, i + 1)) -
                  compute_fi_1stDeriv(getParami(x, i + 1), getxi(x, i + 1));
    }

    // fi'' (x_i+1) - f''_{i+1}(x_i+1) = 0
    for (int i = 0; i < numPoints - 2; i++) {
        g[si++] = compute_fi_2ndDeriv(getParami(x, i), getxi(x, i + 1)) -
                  compute_fi_2ndDeriv(getParami(x, i + 1), getxi(x, i + 1));
    }
...
void NaturalCubicSpline2DAsNonlinearConstraints::hessian(ES::ConstRefVecXd x, ES::ConstRefVecXd lambda,
    ...
    for (int i = 0; i < numPoints - 2; i++) {
        ...
        // dx_{i+1} dx_{i+1}
        hess.coeffRef(xDOFOffset + i + 1, xDOFOffset + i + 1) +=
            (compute_fi_2ndDeriv(getParami(x, i), xi1) - compute_fi_2ndDeriv(getParami(x, i + 1), xi1)) * lambda[si];
