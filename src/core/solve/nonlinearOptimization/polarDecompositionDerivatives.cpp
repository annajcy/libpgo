...
void computeFirstOrderDerivatives(const ES::M3d& F, const ES::M3d& R, const ES::M3d& S, const ES::M3d& SInv,
                                  ES::M3d dRdF[9], ES::M3d dSdF[9], ES::M3d dFdFOut[9], ES::M3d dAdFOut[9],
                                  ES::M3d dsqrtA_dAOut[9], ES::M9d* MInvOut) {
    ES::M9d M    = KroneckerSum3(S, S);
    ES::M9d MInv = M.fullPivLu().inverse();
...
ES::M3d compute_d2S_dF2(const ES::M3d& F, int ei, int ej, const ES::M3d dsqrtA_dA[9], const ES::M9d& MInv,
                        const ES::M3d dFdF[9]) {
...
ES::M3d compute_d2R_dF2([[maybe_unused]] const ES::M3d& F, int ei, int ej, const ES::M3d& R, const ES::M3d& SInv,
                        const ES::M3d dRdF[9], const ES::M3d dSdF[9], const ES::M3d& d2SdF2) {
    ES::M3d ret = ES::M3d::Zero() - R * d2SdF2 - dRdF[ei] * dSdF[ej] - dRdF[ej] * dSdF[ei];
    return ret * SInv;
}

void computeSecondOrderDerivatives([[maybe_unused]] const ES::M3d& F, const ES::M3d& R,
                                   [[maybe_unused]] const ES::M3d& S, const ES::M3d& SInv, const ES::M9d& MInv,
                                   const ES::M3d dRdF[9], const ES::M3d dSdF[9], const ES::M3d dFdF[9],
                                   const ES::M3d dAdF[9], const ES::M3d dsqrtA_dA[9], ES::M3d d2S_dF2[9][9],
                                   ES::M3d d2R_dF2[9][9]) {
    ES::M3d d2sqrtAdA2[9][9];
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
...
void compute(const ES::M3d& F, ES::M3d& R, ES::M3d& S, ES::M3d dSdF[9], ES::M3d dRdF[9], ES::M3d d2SdF2[9][9],
             ES::M3d d2RdF2[9][9]) {
    S            = computeS(F);
    ES::M9d M    = KroneckerSum3(S, S);
    ES::M9d MInv = M.fullPivLu().inverse();
    ...
    if (dSdF == nullptr && dRdF == nullptr && d2RdF2 == nullptr && d2SdF2 == nullptr)
        return;
...
void fdTest(const ES::M3d& F) {
    ...
    for (int ei = 0; ei < 9; ei++) {
        for (int ej = 0; ej < 9; ej++) {
            ...
            std::cout << ei << ',' << ej << ": rel=" << diff.norm() / ret.norm() << "; abs=" << diff.norm()
                      << std::endl;
        }
    }
