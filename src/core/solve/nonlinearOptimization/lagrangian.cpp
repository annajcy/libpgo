...
Lagrangian::Lagrangian(PotentialEnergy_const_p eng, ConstraintFunctions_const_p cnt) : energy(eng), constraints(cnt) {
...
void Lagrangian::hessian(ES::ConstRefVecXd x, ES::SpMatD& hessOut) const {
    ...
    if (constraints) {
        if (constraints->isLinear() == false) {
            constraints->hessian(x.head(energy->getNumDOFs()), x.tail(constraints->getNumConstraints()),
                                 constraintHessian);
