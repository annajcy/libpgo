...
const double coeffs5[5] = {1.0 / 12.0, -2.0 / 3.0, 0, 2.0 / 3.0, -1.0 / 12.0};

const double coeffs3[3] = {-1.0 / 2.0, 0, 1.0 / 2.0};

FiniteDifference::FiniteDifference(METHOD m, double eps_) : method(m), eps(eps_) {
...
void FiniteDifference::testEnergy(std::shared_ptr<const PotentialEnergy> energy, bool testGradient, bool testHessian,
                                  double range, const double* x, int dofRange, double* gradRelError,
                                  double* hessRelError) const {
    ...
    if (testGradient) {
        ...
        for (int dofi : dofs) {
            ...
            if (std::abs(grad[dofi] - grad1[dofi]) > 1e-6)
                std::cout << dofi << ',' << grad[dofi] << "------" << grad1[dofi]
                          << "=====" << (grad[dofi] - grad1[dofi]) << ',' << absSquaredNorm << '\n';
        }
...
void FiniteDifference::testEnergy(EvalFunc evalFunc, int n, bool testGradient, bool testHessian, double range,
                                  const double* x, double* gradRelError, double* hessRelError) const {
    int     nAll = n;
    ES::VXd xcur(nAll);
...
void FiniteDifference::testConstraints(std::shared_ptr<const ConstraintFunctions> cfunc, double range, const double* x,
    ...
    std::cout << "  rel err: " << sqrt(absErr / norm) << "\n"
              << "  abs error: " << sqrt(absErr) << std::endl;
...
void FiniteDifference::testVecFunc(EvalVecFunc evalFunc, int m, int n, double range, const double* x,
                                   double* err) const {
    std::default_random_engine eng;
...
void FiniteDifference::gradient(const double* x, double* grad, int n, std::vector<int>* dofs,
                                std::function<double(const double*)> energyFunc) {
...
void FiniteDifference::hessian(const double* x, double* hess, int n, std::vector<int>* dofs,
                               std::function<void(const double*, double*)> gradFunc) {
    ES::VXd         xtemp(n), xcur = Eigen::Map<const ES::VXd>(x, n);
