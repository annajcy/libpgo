...
void real2x2JacobiSVD(const MatrixType& matrix, Index p, Index q, Eigen::JacobiRotation<RealScalar>& j_left,
                      Eigen::JacobiRotation<RealScalar>& j_right);
...
void unorderedSquareMatrixSVDDerivatices(const DerivedA& F, const DerivedS& S, const DerivedU& U, const DerivedV& V,
                                         const DerivedA& dF, DerivedS& dS, DerivedU* dU = nullptr,
                                         DerivedV* dV = nullptr, const DerivedA* d2F = nullptr, DerivedS* d2S = nullptr,
                                         DerivedU* d2U = nullptr, DerivedV* d2V = nullptr);
}  // namespace pgo::NonlinearOptimization::SVDDerivatives

template <typename RealScalar>
void NonlinearOptimization::SVDDerivatives::makeJacobi(const RealScalar& A11, const RealScalar& A12,
                                                       const RealScalar& A22, RealScalar& c, RealScalar& s) {
...
void NonlinearOptimization::SVDDerivatives::real2x2JacobiSVD(const MatrixType& matrix, Index p, Index q,
                                                             Eigen::JacobiRotation<RealScalar>& j_left,
                                                             Eigen::JacobiRotation<RealScalar>& j_right) {
    Eigen::Matrix<RealScalar, 2, 2> m;
    m << RealScalar(matrix.coeff(p, p)), RealScalar(matrix.coeff(p, q)), RealScalar(matrix.coeff(q, p)),
        RealScalar(matrix.coeff(q, q));
...
template <typename DerivedA, typename DerivedS, typename DerivedU, typename DerivedV>
void NonlinearOptimization::SVDDerivatives::unorderedSquareMatrixSVD(const DerivedA& A, DerivedS& S, DerivedU* U,
                                                                     DerivedV* V) {
    static_assert(DerivedA::ColsAtCompileTime == DerivedA::RowsAtCompileTime && DerivedA::RowsAtCompileTime > 0 &&
                  DerivedA::ColsAtCompileTime > 0);
    ...
    Eigen::Matrix<typename DerivedA::Scalar, DerivedA::ColsAtCompileTime, DerivedA::ColsAtCompileTime,
                  DerivedA::Options>
        workMatrix;
        ...
        for (ES::IDX p = 1; p < dim; ++p) {
            for (ES::IDX q = 0; q < p; ++q) {
                ...
                if (std::abs(workMatrix.coeff(p, q)) > threshold || std::abs(workMatrix.coeff(q, p)) > threshold) {
                    ...
                    maxDiagEntry =
                        std::max(maxDiagEntry, std::max(std::abs(workMatrix.coeff(p, p)), abs(workMatrix.coeff(q, q))));
                }
            }
        }
    }
...
template <typename DerivedA, typename DerivedS, typename DerivedU, typename DerivedV>
void NonlinearOptimization::SVDDerivatives::unorderedSquareMatrixSVDDerivatices(
    const DerivedA&, const DerivedS& S, const DerivedU& U, const DerivedV& V, const DerivedA& dF, DerivedS& dS,
    DerivedU* dU, DerivedV* dV, const DerivedA* d2F, DerivedS* d2S, DerivedU* d2U, DerivedV* d2V) {
    static_assert(DerivedA::ColsAtCompileTime == DerivedA::RowsAtCompileTime && DerivedA::RowsAtCompileTime > 0 &&
                  DerivedA::ColsAtCompileTime > 0);
...
void NonlinearOptimization::SVDDerivatives::unorderedSquareMatrixSVD3(const ES::M3d& F, ES::V3d& S, ES::M3d* U,
                                                                      ES::M3d* V) {
    unorderedSquareMatrixSVD(F, S, U, V);
}

void NonlinearOptimization::SVDDerivatives::unorderedSquareMatrixSVD3Derivatices(
    const ES::M3d& F, const ES::V3d& S, const ES::M3d& U, const ES::M3d& V, const ES::M3d& dF, ES::V3d& dS, ES::M3d* dU,
    ES::M3d* dV, const ES::M3d* d2F, ES::V3d* d2S, ES::M3d* d2U, ES::M3d* d2V) {
    unorderedSquareMatrixSVDDerivatices(F, S, U, V, dF, dS, dU, dV, d2F, d2S, d2U, d2V);
}

void NonlinearOptimization::SVDDerivatives::unorderedSquareMatrixSVD2(const ES::M2d& F, ES::V2d& S, ES::M2d* U,
                                                                      ES::M2d* V) {
    unorderedSquareMatrixSVD(F, S, U, V);
}

void NonlinearOptimization::SVDDerivatives::unorderedSquareMatrixSVD2Derivatices(
    const ES::M2d& F, const ES::V2d& S, const ES::M2d& U, const ES::M2d& V, const ES::M2d& dF, ES::V2d& dS, ES::M2d* dU,
    ES::M2d* dV, const ES::M2d* d2F, ES::V2d* d2S, ES::M2d* d2U, ES::M2d* d2V) {
    unorderedSquareMatrixSVDDerivatices(F, S, U, V, dF, dS, dU, dV, d2F, d2S, d2U, d2V);
}
