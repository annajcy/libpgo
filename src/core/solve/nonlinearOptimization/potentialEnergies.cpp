...
PotentialEnergies::PotentialEnergies(int n) : nAll(n) {
    buffer = std::make_shared<PotentialEnergiesBuffer>();
}

PotentialEnergies::PotentialEnergies(int n, std::shared_ptr<PotentialEnergiesBuffer> buf) : nAll(n), buffer(buf) {}
    ...
    for (auto energy : potentialEnergies) {
        ...
        */
        ...
        tbb::parallel_for((ES::IDX)0, h.outerSize(), [&](ES::IDX outeri) {
            for (ES::SpMatD::InnerIterator it(h, outeri); it; ++it) {
                entries.emplace_back((ES::SpMatD::StorageIndex)dofs[it.row()], (ES::SpMatD::StorageIndex)dofs[it.col()],
                                     1.0);
            }
        });
...
void PotentialEnergies::hessianVector(EigenSupport::ConstRefVecXd x, EigenSupport::ConstRefVecXd vec,
