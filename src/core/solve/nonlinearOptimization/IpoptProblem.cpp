...
IpoptProblem::IpoptProblem(PotentialEnergy_const_p p) : NonlinearProblem(p) {
...
IpoptProblem::IpoptProblem(PotentialEnergy_const_p p, ConstraintFunctions_const_p c) : NonlinearProblem(p, c) {
    for (int i = 0; i < problem->getNumDOFs(); i++) {
        xlow[i] = -inf();
        xhi[i]  = inf();
    }
...
bool IpoptProblem::get_nlp_info(Ipopt::Index& n, Ipopt::Index& m, Ipopt::Index& nnz_jac_g, Ipopt::Index& nnz_h_lag,
...
bool IpoptProblem::get_bounds_info(Ipopt::Index n, Ipopt::Number* x_l, Ipopt::Number* x_u, Ipopt::Index m,
...
bool IpoptProblem::eval_h(Ipopt::Index n, const Ipopt::Number* x, bool new_x, Ipopt::Number obj_factor, Ipopt::Index m,
    ...
    if (values == nullptr) {
        ...
        if (constraints) {
            ...
            constraints->hessian(Eigen::Map<const Eigen::VectorXd>(x, n), Eigen::Map<const Eigen::VectorXd>(lambda, m),
                                 lambdaHessian);
