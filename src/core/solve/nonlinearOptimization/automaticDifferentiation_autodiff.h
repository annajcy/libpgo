...
template <int n, typename Func, typename Derived, typename... Args>
void computeGradient(Func func, const Eigen::MatrixBase<Derived>& x, Eigen::Matrix<double, n, 1>& grad,
                     Args&&... args) {
    dual2nd dualx[n];
    for (int i = 0; i < n; i++) {
        dualx[i] = x(i);
    ...
    for (int i = 0; i < n; i++) {
        autodiff::dual1st val =
            autodiff::derivative(func, autodiff::wrt(dualx[i]), autodiff::at(dualx, std::forward<Args>(args)...));
        grad[i] = val.val;
    }
}
...
void computeGradient(Func func, const Eigen::MatrixBase<Derived>& x, EigenSupport::VXd& grad, Args&&... args) {
    ...
    for (typename Eigen::MatrixBase<Derived>::Index i = 0; i < x.size(); i++) {
        grad[i] = autodiff::derivative(func, autodiff::wrt(dualx[i]),
                                       autodiff::at(dualx.data(), std::forward<Args>(args)...));
    }
}
...
void computeHessian(Func func, const Eigen::MatrixBase<Derived>& x, Eigen::Matrix<double, n, n>& h, Args&&... args) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i; j < n; j++) {
            h(i, j) = autodiff::derivative<2>(func, autodiff::wrt(dualx[i], dualx[j]),
                                              autodiff::at(dualx, std::forward<Args>(args)...));
