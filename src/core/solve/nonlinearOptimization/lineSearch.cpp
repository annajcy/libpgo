...
Result LineSearch::golden(const double* x0, const double* p, double fx0, double initialAlpha) const {
    ...
    {
        if (verbose)
            cout << "failed to find a triplet: " << a << " " << b << " " << c << " | " << fa << " " << fb << " " << fc
                 << endl;
    ...
    for (int iter = 0; iter < maxIter && fabs(a - c) > max(absPrecision * 2, tol * (fabs(b) + fabs(d))); iter++) {
        if (verbose)
            cout << "iter: " << iter << "/" << maxIter << " (" << a << " " << b << " " << c << ") (" << fa << " " << fb
                 << " " << fc << ")" << endl;
        if (fb < fd) {
            // f ^               ----            * END OF GRAPH
            //   | \            /                *
...
Result LineSearch::BrentsMethod(const double* x0, const double* p, double fx0, double initialAlpha) const {
    ...
    for (int iter = 0; iter < maxIter; iter++) {
        if (verbose)
            cout << "iter: " << iter << "/" << maxIter << " [" << brentLowerBound << " " << brentUpperBound
                 << "] x = " << x << ", f(x) = " << fx << endl;
        ...
        if (fabs(x - middle) + 0.5 * (brentUpperBound - brentLowerBound) <=
            tol2)  // the distance between the bounds are small enough
        {
            if (verbose)
                cout << "Relative tolerance of " << tol << " reached." << endl;
            break;
        }
        if (fabs(step2) > tol1) {  // use x2, x, x3 to fit a parabola (x2=a, x=b, x3=c as in the function for parabola)
            ...
            // now the stepSize to go from x to the vertex of the parabola is: d - x = stepSize = numerator /
            // denominator
            ...
            // fabs(numerator) >= fabs(0.5*denominator*stepBeforeLast): test whether the movement from the best current
            // value x that is larger or equal to half the movement of the step before last.
            ...
            } else  // parabola is good
            {
                ...
                double xnew            = x + step1;
                if (xnew - brentLowerBound < tol2 ||
                    brentUpperBound - xnew < tol2)  // if xnew is too close to the bounds
                {
                    if (middle - x >= 0.0)
                        step1 = tol1;
                    else
...
int LineSearch::initializeTriplet(double& a, double& b, double& c, double& fa, double& fb, double& fc) const {
    ...
    if (verbose)
        cout << "after golden step, we have a = " << a << " b = " << b << " c = " << c << ", fa = " << fa
             << " fb = " << fb << " fc = " << fc << endl;
    ...
    while (fc < fb)  // while the condition f1D(b) <= min( f1D(a), f1D(c) ) on [a, b, c] are not satisfied
    {
        ...
        if (verbose)
            cout << "triplet iter " << iterID << ": " << a << " " << b << " " << c << " " << fa << " " << fb << " "
                 << fc << ", d = " << d << " dMax = " << dMax << endl;
