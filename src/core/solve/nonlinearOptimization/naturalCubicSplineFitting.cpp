...
public:
    FittingEnergy(const ES::VXd& x, const ES::VXd& y, int n) : xVals(x), yVals(y), numControlPoints(n) {
...
NaturalCubicSplineFitting::NaturalCubicSplineFitting(const ES::VXd& x, const ES::VXd& y, int n, double ld, double rd,
                                                     double minxd)
    ...
    splineC = std::make_shared<NaturalCubicSpline2DAsNonlinearConstraints>(
        numControlPoints, 0, numControlPoints, numControlPoints * 2, nAll, leftDerivative, rightDerivative);
}
