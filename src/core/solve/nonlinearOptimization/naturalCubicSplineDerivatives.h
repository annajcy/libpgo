...
public:
    NaturalCubicSpline2DAsNonlinearConstraints(int numPoints, int xDOFOffset, int yDOFOffset, int paramDOFOffset,
                                               int nAll, double y0Prime, double ynPrime);
...
protected:
    ...
    inline EigenSupport::V4d getParami(EigenSupport::ConstRefVecXd x, int i) const {
        return x.segment<4>(paramDOFOffset + i * 4);
    }

    inline double compute_fi(const EigenSupport::V4d& param, double x) const {
        return param[0] * x * x * x + param[1] * x * x + param[2] * x + param[3];
    }
    inline double compute_fi_1stDeriv(const EigenSupport::V4d& param, double x) const {
        return 3 * param[0] * x * x + 2 * param[1] * x + param[2];
    }
    inline double compute_fi_2ndDeriv(const EigenSupport::V4d& param, double x) const {
        return 6 * param[0] * x + 2 * param[1];
    }
