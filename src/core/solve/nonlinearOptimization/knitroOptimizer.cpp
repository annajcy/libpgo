...
static int callbackEvalFC(KN_context_ptr /*kc*/, CB_context_ptr /*cb*/, KN_eval_request_ptr const evalRequest,
                          KN_eval_result_ptr const evalResult, void* const userParams) {
    if (evalRequest->type != KN_RC_EVALFC) {
        SPDLOG_LOGGER_ERROR(Logging::lgr(), "*** callbackEvalFC incorrectly called with eval type {}",
                            evalRequest->type);
        return -1;
    }
...
static int callbackEvalGA(KN_context_ptr /*kc*/, CB_context_ptr /*cb*/, KN_eval_request_ptr const evalRequest,
                          KN_eval_result_ptr const evalResult, void* const userParams) {
    if (evalRequest->type != KN_RC_EVALGA) {
        SPDLOG_LOGGER_ERROR(Logging::lgr(), "*** callbackEvalGA incorrectly called with eval type {}",
                            evalRequest->type);
        return -1;
    }
...
int callbackEvalH(KN_context_ptr /*kc*/, CB_context_ptr /*cb*/, KN_eval_request_ptr const evalRequest,
                  KN_eval_result_ptr const evalResult, void* const userParams) {
    if (evalRequest->type != KN_RC_EVALH && evalRequest->type != KN_RC_EVALH_NO_F &&
        evalRequest->type != KN_RC_EVALHV && evalRequest->type != KN_RC_EVALHV_NO_F) {
    ...
    switch (evalRequest->type) {
        case KN_RC_EVALH: {
            ...
            if (problem->isDense()) {
                ...
                //   problem->getm() > 0 ? Eigen::Map<const ES::VXd>(evalRequest->lambda, problem->getm()) :
                //   emptyLambda, sigma);
        ...
        case KN_RC_EVALHV: {
            ...
            ES::VXd& hVec = problem->hessianVector(
                xmap,
                problem->getm() > 0 ? Eigen::Map<const ES::VXd>(evalRequest->lambda, problem->getm()) : emptyLambda,
                Eigen::Map<const ES::VXd>(evalRequest->vec, problem->getn()), sigma);
        ...
        case KN_RC_EVALHV_NO_F: {
            ...
            ES::VXd& hVec = problem->hessianVector(
                xmap,
                problem->getm() > 0 ? Eigen::Map<const ES::VXd>(evalRequest->lambda, problem->getm()) : emptyLambda,
                Eigen::Map<const ES::VXd>(evalRequest->vec, problem->getn()), 0.0);
        ...
        default:
            SPDLOG_LOGGER_ERROR(Logging::lgr(), "*** callbackEvalHess incorrectly called with eval code {}",
                                evalRequest->type);
            retCode = 1;
            break;
    }
...
static int callbackNewPt(KN_context_ptr /*kc*/, const double* const x, const double* const lambda,
                         void* const userParams) {
    KnitroHandles* handles = reinterpret_cast<KnitroHandles*>(userParams);
    KnitroProblem* problem = handles->problem;

    if (problem->getConstraintFunctions() /*|| problem->getDenseConstraintFunctions()*/) {
        problem->setCurrentSolution(Eigen::Map<const ES::VXd>(x, problem->getn()),
                                    Eigen::Map<const ES::VXd>(lambda, problem->getm()));
    } else {
...
KnitroOptimizer::KnitroOptimizer(KnitroProblem* p) {
    ...
    KNITRO_ERROR(KN_set_int_param(handles->kc, KN_PARAM_ALGORITHM, 1), throw std::domain_error("set algorithm"));
...
void KnitroOptimizer::setConfigFile(const char* filename) {
    if (filename && strlen(filename)) {
        KNITRO_ERROR(KN_load_param_file(handles->kc, filename), throw std::domain_error("set config file"));
...
void KnitroOptimizer::setMaxIter(int maxIter) {
    KNITRO_ERROR(KN_set_int_param(handles->kc, KN_PARAM_MAXIT, maxIter), throw std::domain_error("set max iter"));
}

void KnitroOptimizer::setFeasTol(double eps) {
    KNITRO_ERROR(KN_set_double_param(handles->kc, KN_PARAM_FEASTOL, eps), throw std::domain_error("set feasible tol"));
}

void KnitroOptimizer::setOptTol(double eps) {
    KNITRO_ERROR(KN_set_double_param(handles->kc, KN_PARAM_OPTTOL, eps), throw std::domain_error("set optimality tol"));
}

void KnitroOptimizer::setVerbose(int verbose) {
    KNITRO_ERROR(KN_set_int_param(handles->kc, KN_PARAM_OUTLEV, verbose), throw std::domain_error("set verbose level"));
}
...
void KnitroOptimizer::init() {
    ...
    KNITRO_ERROR(KN_add_vars(handles->kc, handles->problem->getn(), nullptr), throw std::domain_error("add variable"));
    ...
    if (handles->problem->getm() > 0) {
        ...
        if (handles->problem->isDense()) {
            ...
            //   KNITRO_ERROR(KN_add_con_linear_struct(handles->kc, (KNLONG)lTerms.size(), cIndices.data(),
            //   lIndices.data(), coeffs.data()),
            ...
            //   KNITRO_ERROR(KN_add_con_quadratic_struct(handles->kc, (KNLONG)qTerms.size(), qcIndices.data(),
            //   qIndices1.data(), qIndices2.data(), qcoeffs.data()),
            //     throw std::domain_error("set quadratic constraints"));

            //   KNITRO_ERROR(KN_add_con_linear_struct(handles->kc, (KNLONG)lTerms.size(), lcIndices.data(),
            //   lIndices.data(), lcoeffs.data()),
            //     throw std::domain_error("set linear constraints"));
        ...
        // if it is a sparse problem
            ...
            if (handles->problem->getConstraintFunctions()->isLinear()) {
                KNITRO_ERROR(KN_add_con_linear_struct(handles->kc, (KNLONG)jac.nonZeros(), jacRows.data(),
                                                      jacCols.data(), jacCoeffs.data()),
                             throw std::domain_error("set linear constraints"));
            ...
            // untested
                ...
                for (int i = 0; i < handles->problem->getm(); i++) {
                    ...
                    handles->problem->getConstraintFunctions()->hessian(handles->problem->getXInit(), lambda,
                                                                        lambdaHessian);
                ...
                KNITRO_ERROR(KN_add_con_quadratic_struct(handles->kc, (KNLONG)rows.size(), rows.data(), var0.data(),
                                                         var1.data(), coeffs.data()),
                             throw std::domain_error("set quadratic constraints"));
            }
            // if the constraints are general
            else {
            ...
            if (constraintsIndices.size()) {
                ...
                if (handles->problem->getObjectiveFunction()->isQuadratic()) {
                    ...
                    KNITRO_ERROR(KN_add_eval_callback(handles->kc, KNFALSE, (KNINT)constraintsIndices.size(),
                                                      constraintsIndices.data(), callbackEvalFC, &handles->cb),
                                 throw std::domain_error("set fc callback"));

                    KNITRO_ERROR(KN_set_cb_grad(handles->kc, handles->cb, 0, nullptr, (KNLONG)jacRows.size(),
                                                jacRows.data(), jacCols.data(), callbackEvalGA),
                                 throw std::domain_error("set ga callback"));
                }
                // otherwise we address it in the general way
                else {
                    KNITRO_ERROR(KN_add_eval_callback(handles->kc, KNTRUE, (KNINT)constraintsIndices.size(),
                                                      constraintsIndices.data(), callbackEvalFC, &handles->cb),
                                 throw std::domain_error("set fc callback"));

                    KNITRO_ERROR(KN_set_cb_grad(handles->kc, handles->cb, KN_DENSE, nullptr, (KNLONG)jacRows.size(),
                                                jacRows.data(), jacCols.data(), callbackEvalGA),
                                 throw std::domain_error("set ga callback"));
                }
            }
            // if there is no general constraints
                ...
                if (handles->problem->getObjectiveFunction()->isQuadratic()) {
                    ...
                    KNITRO_ERROR(KN_add_eval_callback(handles->kc, KNFALSE, 0, nullptr, callbackEvalFC, &handles->cb),
                                 throw std::domain_error("set fc callback"));

                    KNITRO_ERROR(
                        KN_set_cb_grad(handles->kc, handles->cb, 0, nullptr, 0, nullptr, nullptr, callbackEvalGA),
                        throw std::domain_error("set ga callback"));
                } else {
                    KNITRO_ERROR(KN_add_eval_callback(handles->kc, KNTRUE, 0, nullptr, callbackEvalFC, &handles->cb),
                                 throw std::domain_error("set fc callback"));
        ...
        if (handles->problem->isDense()) {
            ...
            if (handles->problem->getObjectiveFunction()->isQuadratic()) {
                ...
                KNITRO_ERROR(
                    KN_set_cb_grad(handles->kc, handles->cb, KN_DENSE, nullptr, 0, nullptr, nullptr, callbackEvalGA),
                    throw std::domain_error("set ga callback"));
            }
        }
    }
    ...
    if (handles->problem->isDense()) {
        ...
        KNITRO_ERROR(
            KN_set_cb_hess(handles->kc, handles->cb, nonZeros, hessianRows.data(), hessianCols.data(), callbackEvalH),
            throw std::domain_error("set hessian callback"));
    }
...
int KnitroOptimizer::solve() {
    ...
    double energy  = 0;
    KNITRO_ERROR(KN_get_solution(handles->kc, &retCode, &energy, x, lambda), throw std::domain_error("get solution"));

    if (handles->problem->hasConstraints()) {
        KNITRO_ERROR(KN_get_con_values_all(handles->kc, g), throw std::domain_error("get constraints"));
    }

    KNITRO_ERROR(KN_get_abs_feas_error(handles->kc, &feasError), throw std::domain_error("get feasibility error"));

    KNITRO_ERROR(KN_get_abs_opt_error(handles->kc, &optError), throw std::domain_error("get optimality error"));
...
void KnitroOptimizer::setxInit(const double* x) {
    /** Define an initial point.  If not set, Knitro will generate one. */
    KNITRO_ERROR(KN_set_var_primal_init_values_all(handles->kc, x), throw std::domain_error("set x init"));
}

void KnitroOptimizer::setxRange(const double* xlow, const double* xhi) {
    KNITRO_ERROR(KN_set_var_lobnds_all(handles->kc, xlow), throw std::domain_error("set x lower bound"));

    KNITRO_ERROR(KN_set_var_upbnds_all(handles->kc, xhi), throw std::domain_error("set x hi bound"));
}
...
void KnitroOptimizer::setcRange(const double* lo, const double* hi) {
    ...
    KNITRO_ERROR(KN_set_con_lobnds_all(handles->kc, clow.data()), throw std::domain_error("set constraints low bound"));

    KNITRO_ERROR(KN_set_con_upbnds_all(handles->kc, chi.data()), throw std::domain_error("set constraints hi bound"));
}

void KnitroOptimizer::printInfo() const {
    int niter = 0;
    KNITRO_ERROR(KN_get_number_iters(handles->kc, &niter), throw std::domain_error("get number iters"));

    double timeCost = 0;
    KNITRO_ERROR(KN_get_solve_time_real(handles->kc, &timeCost), throw std::domain_error("get solve time real"));
