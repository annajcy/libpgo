...
KnitroProblem::KnitroProblem(PotentialEnergy_const_p energy_) : NonlinearProblem(energy_) {
    for (int i = 0; i < problem->getNumDOFs(); i++) {
        xlow[i] = -inf();
        xhi[i]  = inf();
    }
...
ES::VXd& KnitroProblem::hessianVector(EigenSupport::ConstRefVecXd x, EigenSupport::ConstRefVecXd lambda,
                                      EigenSupport::ConstRefVecXd vec, double objScale) {
