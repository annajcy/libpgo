...
TRBDF2TimeIntegratorEnergy::TRBDF2TimeIntegratorEnergy(TRBDF2TimeIntegrator* integrator_, const ES::SpMatD& A_,
                                                       const ES::VXd& b_)
...
double TRBDF2TimeIntegratorEnergy::func(ES::ConstRefVecXd x) const {
    ...
    // std::cout << energy << ',';
    //// compute current u
    tbb::parallel_for(0, intg->n3, [&](int i) { intg->qz[i] = intg->q[i] + x[i]; }, tbb::static_partitioner());
