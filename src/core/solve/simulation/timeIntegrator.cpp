...
TimeIntegrator::TimeIntegrator(const ES::SpMatD& massMatrix, std::shared_ptr<const PotentialEnergy> elasticPotential,
                               double massDampingCoeff, double stiffnessDampingCoeff, double t, int numIter,
                               double eps_)
    : elasticEnergy(elasticPotential),
      timestep(t),
      dK(stiffnessDampingCoeff),
      dM(massDampingCoeff),
      nIter(numIter),
      eps(eps_) {
...
void TimeIntegrator::addImplicitForceModel(std::shared_ptr<PotentialEnergyAligningMeshConnectivity> fm, double kd,
                                           double md) {
        ...
        if (inc) {
            ...
            for (Eigen::Index outeri = 0; outeri < K.outerSize(); outeri++) {
                for (ES::SpMatD::InnerIterator it(K, outeri); it; ++it) {
                    entries.emplace_back((ES::SpMatD::StorageIndex)it.row(), (ES::SpMatD::StorageIndex)it.col(), 1.0);
                }
            }
            ...
            for (size_t i = 0; i < generalAdditionalForceModels.size(); i++) {
                for (Eigen::Index outeri = 0; outeri < generalAdditionalForceModels_K[i].outerSize(); outeri++) {
                    for (ES::SpMatD::InnerIterator it(generalAdditionalForceModels_K[i], outeri); it; ++it) {
                        entries.emplace_back((ES::SpMatD::StorageIndex)it.row(), (ES::SpMatD::StorageIndex)it.col(),
                                             1.0);
                    }
                }
            }
            ...
            for (size_t i = 0; i < generalAdditionalForceModels.size(); i++) {
                ES::small2Big(generalAdditionalForceModels_K[i], hessianAll, allDOFs,
                              generalAdditionalForceModels_Kmapping[i]);
            }
        } else {
            hessianAll = K;
        }
...
void TimeIntegrator::setFixedVertices(const std::vector<int>& fixedVertices, ES::ConstRefVecXd fixedRestP,
                                      ES::ConstRefVecXd fixedP, int isIndexSorted) {
    fixedDOFs.clear();
    ...
    if (isIndexSorted) {
        ...
        for (size_t i = 0; i < fixedVertices.size(); i++) {
            std::array<double, 6> pos = {fixedRestP[i * 3], fixedRestP[i * 3 + 1], fixedRestP[i * 3 + 2],
                                         fixedP[i * 3],     fixedP[i * 3 + 1],     fixedP[i * 3 + 2]};

            fixedInfo.emplace_back(fixedVertices[i], pos);
        }

        std::sort(fixedInfo.begin(), fixedInfo.end(),
                  [](const FixedInfo& fi1, const FixedInfo& fi2) { return fi1.first < fi2.first; });
...
void TimeIntegrator::addConstraints(std::shared_ptr<ConstraintFunctions> constt, ES::ConstRefVecXd clow,
                                    ES::ConstRefVecXd chi) {
    constraints = constt;
...
void TimeIntegrator::enableFiniteDifferenceTest(bool testIntegratorEnergy, bool testImplicitEnergy,
