...
bool CCDTestVega(int numVertices, const double* pos_t0, const double* pos_t1, int numTriangles,
                 const ES::V3i* triangles, int triA, int triB, CCDData* ccdDataPtr = nullptr);
bool CCDTest3rdParty(int numVertices, const double* pos_t0, const double* pos_t1, int numTriangles,
                     const ES::V3i* triangles, int triA, int triB, CCDData* ccdDataPtr = nullptr);
}  // namespace TriangleCCD
}  // namespace CCDKernel
}  // namespace pgo

bool pgo::CCDKernel::TriangleCCD::CCDTest(int numVertices, const double* pos_t0, const double* pos_t1, int numTriangles,
                                          const ES::V3i* triangles, int triA, int triB, CCD_METHOD m,
                                          CCDData* ccdDataPtr) {
    if (m == CCDM_VEGA) {
        return pgo::CCDKernel::TriangleCCD::CCDTestVega(numVertices, pos_t0, pos_t1, numTriangles, triangles, triA,
                                                        triB, ccdDataPtr);
    } else if (m == CCDM_3RDPARTY) {
        return pgo::CCDKernel::TriangleCCD::CCDTest3rdParty(numVertices, pos_t0, pos_t1, numTriangles, triangles, triA,
                                                            triB, ccdDataPtr);
    } else {
...
bool pgo::CCDKernel::TriangleCCD::computeCCDPlane(int numVertices, const double* pos_t0, const double* pos_t1,
                                                  int numTriangles, const ES::V3i* triangles, const CCDData& ccdData,
                                                  bool doSign, double planePoint[3], double planeNormal[3],
                                                  double& signA, double& signB) {
    constexpr double eps = 1e-10;

    const double* vtxA0[3] = {pos_t0 + triangles[ccdData.triA][0] * 3, pos_t0 + triangles[ccdData.triA][1] * 3,
                              pos_t0 + triangles[ccdData.triA][2] * 3};

    const double* vtxA1[3] = {pos_t1 + triangles[ccdData.triA][0] * 3, pos_t1 + triangles[ccdData.triA][1] * 3,
                              pos_t1 + triangles[ccdData.triA][2] * 3};

    const double* vtxB0[3] = {pos_t0 + triangles[ccdData.triB][0] * 3, pos_t0 + triangles[ccdData.triB][1] * 3,
                              pos_t0 + triangles[ccdData.triB][2] * 3};

    const double* vtxB1[3] = {pos_t1 + triangles[ccdData.triB][0] * 3, pos_t1 + triangles[ccdData.triB][1] * 3,
    ...
    signB = 1;
        ...
        for (size_t ai = 0; ai < numAxes; ai++) {
            ...
            double projA[3] = {triA0_t.dot(axes[ai]), triA1_t.dot(axes[ai]), triA2_t.dot(axes[ai])};

            double projB[3] = {triB0_t.dot(axes[ai]), triB1_t.dot(axes[ai]), triB2_t.dot(axes[ai])};
...
bool pgo::CCDKernel::TriangleCCD::computeDCDPlane(int, const double* pos_t0, int, const ES::V3i* triangles,
    ...
    if (ccdData.ccdCase == CCDC_VTXA_TRIB) {
        ...
        ES::V3d contactPoint =
            vtxB[0] * ccdData.weights[0] + vtxB[1] * ccdData.weights[1] + vtxB[2] * ccdData.weights[2];
        (ES::Mp<ES::V3d>(planePoint)) = contactPoint;
    } else if (ccdData.ccdCase == CCDC_VTXB_TRIA) {
        (ES::Mp<ES::V3d>(planeNormal)) = nA;
        ES::V3d contactPoint =
            vtxA[0] * ccdData.weights[0] + vtxA[1] * ccdData.weights[1] + vtxA[2] * ccdData.weights[2];
        (ES::Mp<ES::V3d>(planePoint)) = contactPoint;
    }
...
void pgo::CCDKernel::TriangleCCD::computeSign(int, const double* pos_t0, const double* pos_t1, int,
                                              const ES::V3i* triangles, const CCDData& ccdData,
                                              const double planePoint[3], const double planeNormal[3], double& signA,
                                              double& signB) {
    const double* vtxA0[3] = {pos_t0 + triangles[ccdData.triA][0] * 3, pos_t0 + triangles[ccdData.triA][1] * 3,
                              pos_t0 + triangles[ccdData.triA][2] * 3};
...
void pgo::CCDKernel::TriangleCCD::dumpCCD(int, const double* pos_t0, const double* pos_t1, int,
                                          const ES::V3i* triangles, const CCDData& ccdData,
                                          std::vector<ES::V3d>& outVertices, std::vector<ES::V3i>& outTriangles) {
    const double* vtxA0[3] = {pos_t0 + triangles[ccdData.triA][0] * 3, pos_t0 + triangles[ccdData.triA][1] * 3,
                              pos_t0 + triangles[ccdData.triA][2] * 3};
...
bool pgo::CCDKernel::TriangleCCD::CCDTest3rdParty(int, const double* pos_t0, const double* pos_t1, int,
                                                  const ES::V3i* triangles, int triA, int triB, CCDData* ccdDataPtr) {
    CCDData ccdData;
    ccdData.triA    = triA;
    ccdData.triB    = triB;
    ccdData.t       = 1000;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (SafeCCDWrapper::VertexTriangleCCD(vtx0, vtx1, vtxB0[0], vtxB1[0], vtxB0[1], vtxB1[1], vtxB0[2], vtxB1[2],
                                              thist, &thisb2, &thisb3) &&
            ExactCCDWrapper::VertexTriangleCCD(vtx0, vtx1, vtxB0[0], vtxB1[0], vtxB0[1], vtxB1[1], vtxB0[2],
                                               vtxB1[2])) {
            if (thist < ccdData.t) {
                ccdData.t = thist;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        double thist, thisb2, thisb3;
        if (SafeCCDWrapper::VertexTriangleCCD(vtx0, vtx1, vtxA0[0], vtxA1[0], vtxA0[1], vtxA1[1], vtxA0[2], vtxA1[2],
                                              thist, &thisb2, &thisb3) &&
            ExactCCDWrapper::VertexTriangleCCD(vtx0, vtx1, vtxA0[0], vtxA1[0], vtxA0[1], vtxA1[1], vtxA0[2],
                                               vtxA1[2])) {
            if (thist < ccdData.t) {
                ccdData.t = thist;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (SafeCCDWrapper::EdgeEdgeCCD(vtxA0[v0], vtxA1[v0], vtxA0[v1], vtxA1[v1], vtxB0[v2], vtxB1[v2], vtxB0[v3],
                                            vtxB1[v3], thist, &thisr, &thiss) &&
                ExactCCDWrapper::EdgeEdgeCCD(vtxA0[v0], vtxA1[v0], vtxA0[v1], vtxA1[v1], vtxB0[v2], vtxB1[v2],
                                             vtxB0[v3], vtxB1[v3])) {
                if (thist < ccdData.t) {
                    ccdData.t = thist;
...
bool pgo::CCDKernel::TriangleCCD::CCDTestVega(int, const double* pos_t0, const double* pos_t1, int,
                                              const ES::V3i* triangles, int triA, int triB, CCDData* ccdDataPtr) {
#if 0
  CCDData ccdData;
  ccdData.triA = triA;
...
double pgo::CCDKernel::distanceSegmentSegmentExact(const double P0[3], const double P1[3], const double Q0[3],
                                                   const double Q1[3], double w[2]) {
