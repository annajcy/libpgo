...
public:
    PointPenetrationEnergy(int numPoints, int nAll, const double* constraintCoeffs,
                           const double* constraintTargetPositions, const double* constraintNormals,
                           const std::vector<std::vector<int>>&    barycentricIdx,
                           const std::vector<std::vector<double>>& barycentricWeights, double bcCoeffs, int useN,
                           int checkPenetration, const std::vector<Mesh::TriMeshGeo>* externalMeshes = nullptr,
...
protected:
    ...
    void   frictionPotentialGradient(const EigenSupport::V3d& x, const EigenSupport::V3d& xlast,
                                     const EigenSupport::V3d& n, double eps, double timestep,
                                     EigenSupport::V3d& grad) const;
    void   frictionPotentialHessian(const EigenSupport::V3d& x, const EigenSupport::V3d& xlast,
                                    const EigenSupport::V3d& n, double eps, double timestep,
                                    EigenSupport::M3d& hess) const;
