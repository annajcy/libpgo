...
PointTrianglePairCouplingEnergyWithCollision::PointTrianglePairCouplingEnergyWithCollision(
    int numPairs_, int numObjects_, const std::array<int, 4>* const objectIDs_,
    const std::array<int, 4>* const pointTrianglePairs_, const int* const triangleIDs_,
    const int* const objectDOFOffsets_, const TriMeshRef* const surfaceMeshes_,
    const Mesh::TriMeshNeighbor* const surfaceMeshNeighbors_, const std::vector<int>* const vertexIDToSampleIDs_,
    const ES::SpMatD* const sampleEmbeddingWeights_, const std::vector<double>* const sampleWeights_)
    : numPairs(numPairs_),
      numObjects(numObjects_),
      objectIDs(objectIDs_),
      pointTrianglePairs(pointTrianglePairs_),
      triangleIDs(triangleIDs_),
      objectDOFOffsets(objectDOFOffsets_),
      surfaceMeshesRest(surfaceMeshes_),
    ...
    // tbb::enumerable_thread_specific<std::vector<ES::TripletD, tbb::cache_aligned_allocator<ES::TripletD>>>
    // entriesTLS;
...
std::tuple<bool, int> PointTrianglePairCouplingEnergyWithCollision::checkContact(ES::ConstRefVecXd x, int pi,
                                                                                 ES::V12d& xlocal,
    ...
    if (checkNeighboringTriangles) {
        ...
        for (int trii = 0; trii < (int)neighboringTriangles[pi].size(); trii++) {
            ...
            ES::V3d pt, w;
            double  dist =
                getSquaredDistanceToTriangle(asVec3d(xlocal.data()), trip[0], trip[1], trip[2], feature, pt, w);
...
double PointTrianglePairCouplingEnergyWithCollision::frictionPotential(const ES::V12d& x, const ES::V12d& xlast,
                                                                       const ES::V3d& n, const ES::V3d& w, double eps,
...
void PointTrianglePairCouplingEnergyWithCollision::frictionPotentialGradient(const ES::V12d& x, const ES::V12d& xlast,
                                                                             const ES::V3d& n, const ES::V3d& w,
                                                                             double eps, double timestep,
                                                                             ES::V12d& grad) const {
...
double PointTrianglePairCouplingEnergyWithCollision::func(ES::ConstRefVecXd x) const {
    ...
    auto localEnergy = [&](int pi) {
        ...
        if (frictionCoeff > 0 && frictionContactForceMag > 0) {
            ...
            double frictionEnergy =
                frictionPotential(xlocal, xlast, normals[pi], barycentricWeights[pi], eps, timestep);
    ...
    // for (int pi = 0; pi < numPairs; pi++) {
    tbb::parallel_for(0, numPairs, [&](int pi) { localEnergy(pi); });
    // }
...
void PointTrianglePairCouplingEnergyWithCollision::hessian(ES::ConstRefVecXd x, ES::SpMatD& hess) const {
                    ...
                    for (ES::SpMatD::InnerIterator it_j(sampleEmbeddingWeights[mj], vidj * 3); it_j; ++it_j) {
                        ...
                        for (int ii = 0; ii < 3; ii++) {
                            for (int jj = 0; jj < 3; jj++) {
                                // std::ptrdiff_t offset = ES::FindEntryOffset(hess, objectDOFOffsets[mi] + actual_vidi
                                // * 3 + ii, objectDOFOffsets[mj] + actual_vidj * 3 + jj); std::ptrdiff_t offset1 =
                                // ES::FindEntryOffset(hessTemplate, objectDOFOffsets[mi] + actual_vidi * 3 + ii,
                                // objectDOFOffsets[mj] + actual_vidj * 3 + jj);

                                auto itt = entryMap.find(std::make_pair(objectDOFOffsets[mi] + actual_vidi * 3 + ii,
                                                                        objectDOFOffsets[mj] + actual_vidj * 3 + jj));
                                ...
                                if (offset < 0) {
                                    ...
                                    std::cout << pi << ',' << (objectDOFOffsets[mi] + actual_vidi * 3 + ii) << ','
                                              << (objectDOFOffsets[mj] + actual_vidj * 3 + jj) << std::endl;
                                    abort();
                                } else {
    ...
    // for (int pi = 0; pi < numPairs; pi++) {
    tbb::parallel_for(0, numPairs, [&](int pi) { localHessian(pi); });

    // std::cerr << std::endl;
}

PointTrianglePairCouplingEnergyWithCollisionBuffer* PointTrianglePairCouplingEnergyWithCollision::allocateBuffer()
    const {
    PointTrianglePairCouplingEnergyWithCollisionBuffer* b = new PointTrianglePairCouplingEnergyWithCollisionBuffer;

    // b->entryLock = std::vector<tbb::spin_mutex>(std::max(hessTemplate.nonZeros(),
    // (ES::IDX)objectDOFOffsets[numObjects]));
...
void PointTrianglePairCouplingEnergyWithCollision::freeBuffer(
    PointTrianglePairCouplingEnergyWithCollisionBuffer* buf) const {
    delete buf;
}
...
ES::V3d PointTrianglePairCouplingEnergyWithCollision::computeLastPosition(ES::ConstRefVecXd x, int objID,
                                                                          int vid) const {
    ES::V3d p;
    p.setZero();
