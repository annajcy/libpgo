...
int AnimationLoader::load(const char* filename) {
    const FileService::ConfigPathResolver configPathResolver(filename);

    ConfigFileJSON jconfig;
    if (jconfig.open(configPathResolver.filePath().c_str()) == false) {
        return 1;
    }
    ...
    try {
        for (auto& jmesh : jconfig_root["meshes"]) {
            ...
            aseq.drivingMeshFilename = configPathResolver.resolve(jmesh.at("driving-mesh").get<std::string>());
            aseq.displayMeshFilename = configPathResolver.resolve(jmesh.value("display-mesh", ""));
            aseq.sequenceName        = configPathResolver.resolve(jmesh.at("sequence").get<std::string>());
    ...
    for (auto& aseq : seqs) {
        ...
        if (aseq.scaleString.length()) {
            int ret = std::sscanf(aseq.scaleString.c_str(), "%lf,%lf,%lf", &embeddedScale, &embeddingScale,
                                  &displacementScale);
            PGO_ALOG(ret == 3);
        }
        ...
        if (saveCache == 0 || std::filesystem::exists(uCacheFilename) == false) {
            if (aseq.sequenceType == "objmesh") {
                ...
                bool                 hasLoadedFrame      = false;
                int                  skippedLeadingFrame = 0;
                for (int i = aseq.sequenceRange[0]; i < aseq.sequenceRange[1]; i++) {
                    std::string      frameFilename = fmt::format(fmt::runtime(aseq.sequenceName), i);
                    ...
                    if (!std::filesystem::exists(frameFilename)) {
                        ...
                        SPDLOG_LOGGER_WARN(Logging::lgr(), "Frame mesh file is missing after sequence start: {}",
                                           frameFilename);
                        continue;
                    }

                    if (frameMesh.load(frameFilename) != true) {
                        SPDLOG_LOGGER_WARN(Logging::lgr(), "Failed to load frame mesh contents: {}",
                                           frameFilename);
                        continue;
                    }

                    if (!hasLoadedFrame && skippedLeadingFrame > 0) {
                        SPDLOG_LOGGER_WARN(
                            Logging::lgr(),
                            "Skipped {} missing leading frame(s) for sequence '{}' in range [{}, {}). "
                            "First available frame: {}",
                            skippedLeadingFrame, aseq.name, aseq.sequenceRange[0], aseq.sequenceRange[1],
                            frameFilename);
                    }
                    hasLoadedFrame = true;
                ...
                if (disp.empty()) {
                    SPDLOG_LOGGER_ERROR(
                        Logging::lgr(),
                        "No animation frames could be loaded for sequence '{}' in range [{}, {}). "
                        "Check the configured frame pattern '{}'.",
                        aseq.name, aseq.sequenceRange[0], aseq.sequenceRange[1], aseq.sequenceName);
                    return 1;
                }
            ...
            // if (foundTemp == std::string::npos) {
            //   std::filesystem::path uAllPath = std::filesystem::path(outputDir) /
            //   std::filesystem::path(fmt::format("{}-uAll.u", m.name)); WriteMatrixToDisk(uAllPath.string().c_str(),
            //   (int)m.objMesh->getNumVertices() * 3, (int)m.displacements.size(), uAll.data());
            // }
        } else {
            // std::vector<double> uAll;
            // int nr, nc;
