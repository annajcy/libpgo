...
void pgo::AnimationIO::dumpABC(const char* filename, const char* name, const std::vector<float>& positions,
                               const std::vector<std::vector<float>>& displacements,
                               const std::vector<std::vector<int>>& triangles, const std::vector<float>* uv) {
    OArchive archive(
    ...
    OPolyMeshSchema::Sample mesh_samp(V3fArraySample((const V3f*)positions.data(), positions.size() / 3),
