...
bool allOf(InputIt first, InputIt last, const T& value) {
    auto l = [&](const typename std::iterator_traits<InputIt>::value_type& it) { return it == value; };
...
std::size_t binarySearchGetIndex(const RamdomAccessRange& sortedVector, const T& val, Compare comp) {
    ...
    return ((it != sortedVector.end() && !(comp(val, *it))) ? std::distance(sortedVector.begin(), it)
                                                            : sortedVector.size());
}
...
// It functions like std::unique, which operates on a sorted range and moves duplicated elements to the back of the
// range, and finally returns the iterator pointing to the end of the deduplicated elements. However, reduceDuplicates
// not only moves duplicate elements to the back, but also calls a reduce operator on those duplicated elements. reduce
// is a binary function: void reduce(T & entryA, T & entryB), reduces data in entryA and entryB and store the result
// into entryA e.g. input buffer is [0, 1, 2.1, 2.2, 3, 4.1, 4.2, 4.3], and we only compare with the integer part of
// each number (e.g. 2.1 == 2.2) and reduce two elements by adding their fractional parts together (e.g. (2.1, 2.2)
// => 2.3) then the result is a buffer: [0, 1, 2.3, 3, 4.6, x, x, x], the function returns the iterator on the first x
