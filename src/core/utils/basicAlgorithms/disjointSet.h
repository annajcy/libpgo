...
/*
  ...
  2) Find: Determine which set a particular element is in. Also useful for determining if two elements are in the same
  set. 3) Union: Combine or merge two sets into a single set.
...
public:
    ...
    // returns the representative of the set that x belongs to (path compression is implemented in this function as
    // well)
