...
//   hasVtx_[i] == false for i in [3,5] causes the kinematics to skip the
//   opposite-normal contribution for that edge.
//   Missing-neighbor restX_ entries are not read.

class KoiterShellKinematics : public ShellKinematics
{
public:
  ...
  KoiterShellKinematics(const double restX[18], const bool hasVtx[6]);
