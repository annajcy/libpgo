#include "hexTricubicHermiteBasis.h"
...
namespace
{
// Corner parametric bits — must match HexTrilinearBasis / kVertexAlpha/Beta/Gamma so the 8 mesh
...
void HexTricubicHermiteBasis::N(double xi, double eta, double zeta, double N_out[]) const
{
  for (int c = 0; c < 8; c++) {
    for (int m = 0; m < 8; m++) {
...
void HexTricubicHermiteBasis::dN_dxi(double xi, double eta, double zeta, double dN_out[]) const
{
...
void HexTricubicHermiteBasis::nodeCoords(int node, double xi[3]) const
{
