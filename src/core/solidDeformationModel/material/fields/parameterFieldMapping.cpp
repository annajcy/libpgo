  ...
  std::span<double> materialValues)
{
  if (localDofValues.size() != static_cast<std::size_t>(numChannels))
    throw std::invalid_argument("IdentityParameterFieldMapping local DOF count mismatch.");
  if (materialValues.size() != static_cast<std::size_t>(numChannels))
    throw std::invalid_argument("IdentityParameterFieldMapping material channel count mismatch.");
}
}  // namespace

IdentityParameterFieldMapping::IdentityParameterFieldMapping(int numChannels):
  numChannels_(numChannels)
{
  if (numChannels < 0)
    throw std::invalid_argument("IdentityParameterFieldMapping requires a non-negative channel count.");
}

void IdentityParameterFieldMapping::evaluate(
  int, int,
  std::span<const double> localDofValues,
...
void IdentityParameterFieldMapping::evaluateJacobian(
  int, int,
  std::span<const double> localDofValues,
  double *output) const
{
  ...
  if (numChannels_ > 0 && output == nullptr)
    throw std::invalid_argument("IdentityParameterFieldMapping requires a Jacobian output buffer.");
  if (numChannels_ > 0)
    std::fill(
      output,
...
void IdentityParameterFieldMapping::evaluateHessians(
  int, int,
  std::span<const double> localDofValues,
  double *output) const
{
  ...
  if (count > 0 && output == nullptr)
    throw std::invalid_argument("IdentityParameterFieldMapping requires a Hessian output buffer.");
  if (count > 0)
