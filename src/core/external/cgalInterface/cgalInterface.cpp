...
struct User_input_is_constrained_edge_map_for_SurfaceMesh {
    ...
    User_input_is_constrained_edge_map_for_SurfaceMesh() : surfaceMesh(nullptr), userSelectedEdges(nullptr) {}

    User_input_is_constrained_edge_map_for_SurfaceMesh(const SurfaceMesh&                                   sm,
                                                       const std::vector<typename SurfaceMesh::Edge_index>& usedge)
        : surfaceMesh(&sm), userSelectedEdges(&usedge) {}
};
}  // namespace pgo::CGALInterface

pgo::Mesh::TriMeshGeo pgo::CGALInterface::isotropicRemeshing(const Mesh::TriMeshGeo& mesh, double targetEdgeLength,
                                                             int numIter, double angleInDegree,
    ...
    if (smFaceSubset.size()) {
        CGAL::Polygon_mesh_processing::isotropic_remeshing(
            smFaceSubset, targetEdgeLength, surfaceMesh,
            CGAL::Polygon_mesh_processing::parameters::number_of_iterations(numIter)
                .protect_constraints(true)
                .edge_is_constrained_map(eifMap));
    } else {
        CGAL::Polygon_mesh_processing::isotropic_remeshing(
            CGAL::faces(surfaceMesh), targetEdgeLength, surfaceMesh,
            CGAL::Polygon_mesh_processing::parameters::number_of_iterations(numIter)
                .protect_constraints(true)
                .edge_is_constrained_map(eifMap));
    }
...
pgo::Mesh::TriMeshGeo pgo::CGALInterface::refineMesh(const Mesh::TriMeshGeo& mesh, double density) {
    ...
    CGAL::Polygon_mesh_processing::refine(surfaceMesh, CGAL::faces(surfaceMesh), std::back_inserter(new_facets),
...
pgo::Mesh::TriMeshGeo pgo::CGALInterface::refineSharpRegionOnMesh(const Mesh::TriMeshGeo& mesh, double density,
    ...
    CGAL::Polygon_mesh_processing::refine(surfaceMesh, selectedFaces, std::back_inserter(new_facets),
                                          std::back_inserter(new_vertices),
                                          CGAL::parameters::density_control_factor(density));
...
pgo::Mesh::TriMeshGeo pgo::CGALInterface::smoothMesh(const Mesh::TriMeshGeo& mesh, int numIter, double angleInDegree,
    ...
    if (constrainedEdges) {
        for (SM::Edge_iterator eitt = surfaceMesh.edges_begin(); eitt != surfaceMesh.edges_end(); eitt++) {
            ...
            bool f1 =
                std::binary_search(constrainedEdges->begin(), constrainedEdges->end(), std::pair<int, int>(vi0, vi1));
            bool f2 =
                std::binary_search(constrainedEdges->begin(), constrainedEdges->end(), std::pair<int, int>(vi1, vi0));
...
bool pgo::CGALInterface::corefineAndComputeUnion(const Mesh::TriMeshGeo& mesh1, const Mesh::TriMeshGeo& mesh2,
                                                 Mesh::TriMeshGeo& unionMesh) {
    using K                 = KernelExact;
    using SM                = CGAL::Surface_mesh<K::Point_3>;
    using vertex_descriptor = boost::graph_traits<SM>::vertex_descriptor;
    namespace PMP           = CGAL::Polygon_mesh_processing;
...
bool pgo::CGALInterface::corefineAndComputeIntersection(const Mesh::TriMeshGeo& mesh1, const Mesh::TriMeshGeo& mesh2,
                                                        Mesh::TriMeshGeo& intersectionMesh) {
    using K                 = KernelExact;
    using SM                = CGAL::Surface_mesh<K::Point_3>;
    using vertex_descriptor = boost::graph_traits<SM>::vertex_descriptor;
    namespace PMP           = CGAL::Polygon_mesh_processing;
...
pgo::Mesh::TriMeshGeo pgo::CGALInterface::clipMesh(const Mesh::TriMeshGeo& meshIn, const Vec3d& planeN,
                                                   const Vec3d& planeP) {
    // using K = KernelInexact;
    using K                 = KernelExact;
    using SM                = CGAL::Surface_mesh<K::Point_3>;
...
void pgo::CGALInterface::corefineOnly(Mesh::TriMeshGeo& mesh1, Mesh::TriMeshGeo& mesh2, bool noModify1, bool noModify2,
...
public:
    SimpleMask3(Poly& pmesh) : pmesh(pmesh), vpm(get(CGAL::vertex_point, pmesh)) {}
    ...
    void vertex_node(vertex_descriptor vd, Point& pt) { pt = boost::get(vpm, vd); }
...
pgo::Mesh::TriMeshGeo pgo::CGALInterface::triangulate(const std::vector<Vec3d>&            vtx,
                                                      const std::vector<std::vector<int>>& faces) {
    Polyhedron<KernelInexact> P;
    PolyhedronBuilderNonTriangle<KernelInexact, std::vector<Vec3d>::const_iterator,
                                 std::vector<std::vector<int>>::const_iterator>
        builder(vtx.begin(), faces.begin(), (int)vtx.size(), (int)faces.size());
...
bool pgo::CGALInterface::isSelfIntersected(const Mesh::TriMeshGeo& meshIn) {
    ...
    bool intersecting = CGAL::Polygon_mesh_processing::does_self_intersect<CGAL::Parallel_if_available_tag>(
        P, CGAL::parameters::vertex_point_map(CGAL::get(CGAL::vertex_point, P)));
...
pgo::Mesh::TriMeshGeo pgo::CGALInterface::simplifyMeshGH(const Mesh::TriMeshGeo& meshIn, const std::string& method,
                                                         double edgeStoppingRatio) {
