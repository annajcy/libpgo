...
Mesh::TriMeshGeo isotropicRemeshing(const Mesh::TriMeshGeo& mesh, double targetEdgeLength, int numIter,
                                    double angleInDegree, std::vector<int>* constrainedVertices = nullptr,
                                    std::vector<std::pair<int, int>>* constrainedEdges = nullptr,
                                    std::vector<int>*                 faceSubset       = nullptr);
Mesh::TriMeshGeo smoothMesh(const Mesh::TriMeshGeo& mesh, int numIter, double angleInDegree,
                            std::vector<int>*                 constrainedVertices = nullptr,
                            std::vector<std::pair<int, int>>* constrainedEdges    = nullptr);
...
bool corefineAndComputeIntersection(const Mesh::TriMeshGeo& mesh1, const Mesh::TriMeshGeo& mesh2,
                                    Mesh::TriMeshGeo& intersectionMesh);
void corefineOnly(Mesh::TriMeshGeo& mesh1, Mesh::TriMeshGeo& mesh2, bool noModify1 = false, bool noModify2 = false,
                  double edgeLengthThreshold = 1e-5);
