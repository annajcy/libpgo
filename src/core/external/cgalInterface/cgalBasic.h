...
public:
    PolyhedronBuilder(const Mesh::TriMeshGeo& mesh) : meshIn(mesh) {}
...
void triangleMesh2SurfaceMesh(const Mesh::TriMeshGeo& mesh, SurfaceMesh& surfaceMesh, PropertyMap* indexMap) {
    ...
    for (const auto& pos : mesh.positions()) {
        typename SurfaceMesh::Vertex_index vi =
            surfaceMesh.add_vertex(typename SurfaceMesh::Point(pos[0], pos[1], pos[2]));
    ...
    for (const auto& tri : mesh.triangles()) {
        [[maybe_unused]] typename SurfaceMesh::Face_index fi =
            surfaceMesh.add_face(vertexIndices[tri[0]], vertexIndices[tri[1]], vertexIndices[tri[2]]);
    }

    if (indexMap) {
        *indexMap = surfaceMesh
                        .template add_property_map<typename boost::graph_traits<SurfaceMesh>::vertex_descriptor, int>(
                            "v:idx", int(-1))
                        .first;
...
void polyhedron2TriangleMesh(const Polyhedron<Kernel>& P, Mesh::TriMeshGeo& mesh) {
    ...
    for (typename Polyhedron<Kernel>::Vertex_const_iterator itt = P.vertices_begin(); itt != P.vertices_end(); ++itt) {
        ...
        mesh.addPos(Vec3d{CGAL::to_double(pt[0]), CGAL::to_double(pt[1]), CGAL::to_double(pt[2])});
