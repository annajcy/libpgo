...
struct MyVisitor : CGAL::Surface_mesh_simplification::Edge_collapse_visitor_base<Polyhedron<Kernel>> {
    ...
    void OnSelected(const Profile&, boost::optional<typename Kernel::FT> cost, std::size_t initial,
                    std::size_t current) {
        std::cerr << *cost << '\n' << std::flush;
    }
    ...
    void OnNonCollapsable(const Profile&) { std::cout << "Link\n" << std::flush; }
...
void removeSmallPieces(Polyhedron<Kernel>& p, double edgeLengthThreshold) {
    ...
    while (1) {
        int r = SMS::edge_collapse(
            p, CGAL::Surface_mesh_simplification::Edge_length_stop_predicate<typename Kernel::FT>(edgeLengthThreshold),
            CGAL::parameters::get_cost(SMS::Edge_length_cost<Poly>()).get_placement(SMS::Midpoint_placement<Poly>()));
        ...
        for (auto it : p.facet_handles()) {
            ...
            for (int j = 0; j < 3; j++) {
                ...
                if constexpr (0) {
                    ...
                    // Poly pp1;
                    // pp1.make_triangle(h1->vertex()->point(), h1->next()->vertex()->point(),
                    // h1->next()->next()->vertex()->point()); std::ofstream("aa.off") << pp1;
