...
int pgo::TetgenInterface::computeVoronoiDiagram(const std::vector<EigenSupport::V3d>& points, VoronoiDiagram& vd) {
    ...
    std::cout << double(std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count()) * 1e-6 << "s."
              << std::endl;
...
int pgo::TetgenInterface::computeTetMesh(const Mesh::TriMeshGeo& mesh, const std::string& switcher,
                                         EigenSupport::MXd& vtx, EigenSupport::MXi& tet) {
    // assign vertices;
