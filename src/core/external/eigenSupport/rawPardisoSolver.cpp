...
PardisoSolverEigen::~PardisoSolverEigen() {
    ...
    pardiso(pt.data(), &maxfct, &mnum, &mtype, &phase, &n, &ddum, ia.data(), ja.data(), &idum, &nrhs, iparm.data(),
            &msglvl, &ddum, &ddum, &error, dparm.data());
}
...
int PardisoSolverEigen::stage1(const SpMatD& A) {
    ...
    phase = 11;
    pardiso(pt.data(), &maxfct, &mnum, &mtype, &phase, &n, entries.data(), ia.data(), ja.data(), &idum, &nrhs,
