...
void tensorProduct(Eigen::MatrixBase<DerivedC>& r, const Eigen::MatrixBase<DerivedA>& v1,
...
void   createWeightMatrix(int numRows, int numCols, int numVertices, int numWeightsPerVertex, const int* vertexIndices,
                          const int* interpolationVertexIndices, const double* interpolationWeights, int inflate,
                          SpMatD& A);
SpMatD createWeightMatrix(int numRows, int numCols, int numVertices, int numWeightsPerVertex, const int* vertexIndices,
                          const int* interpolationVertexIndices, const double* interpolationWeights, int inflate);
...
void small2Big(const SpMatD& Asmall, const SpMatD& Abig, const std::vector<int>& dofsRow,
               const std::vector<int>& dofsCol, SpMatI& mapping);

void big2Small(const SpMatD& Abig, const SpMatD& Asmall, int rowStart, int colStart, SpMatI& mapping,
               int allowDifferentTopo = 0);
void big2Small(const SpMatD& Abig, const SpMatD& Asmall, const std::vector<int>& dofsRow,
               const std::vector<int>& dofsCol, SpMatI& mapping, int allowDifferentTopo = 0);
...
void removeRowsCols(const SpMatD& Abig, const std::vector<int>& removedRowDofs, const std::vector<int>& removedColDofs,
                    SpMatD& mat);

void selectRowsCols(const SpMatD& Abig, const std::vector<int>& selectedDofs, SpMatD& mat);
void selectRowsCols(const SpMatD& Abig, const std::vector<int>& selectedRowDofs,
...
void removeRows(int dofAll, const std::vector<int>& removedDofs, std::vector<int>& big2smallMapping,
                std::vector<int>& small2bigMapping);

void addSmallToBig(double coeff, const SpMatD& Asmall, SpMatD& Abig, double beta, const SpMatI& mapping,
                   int parallel = 0);
void addBigToSmall(double coeff, const SpMatD& Abig, SpMatD& Asmall, double beta, const SpMatI& mapping,
                   int parallel = 0);
...
void EigenSupport::mergeSparseMatrix(SpMatD& Adst, T&&... Ms) {
    static_assert(
        std::is_same<std::integer_sequence<bool, true, std::is_same<std::decay_t<T>, SpMatD>::value...>,
...
template <typename DerivedA, typename DerivedB, typename DerivedC>
void EigenSupport::tensorProduct(Eigen::MatrixBase<DerivedC>& ret, const Eigen::MatrixBase<DerivedA>& v1,
                                 const Eigen::MatrixBase<DerivedB>& v2) {
    if (v1.rows() != v2.rows() || v1.cols() != 1 || v2.cols() != 1 || ret.rows() != v1.rows() ||
        ret.cols() != ret.rows())
...
int EigenSupport::readMatrix(const char* filename, Eigen::PlainObjectBase<Derived>& mat, int overwrite) {
    ...
    if (overwrite) {
        ...
        if (nRows != int(mat.rows()) || nCols != int(mat.cols())) {
            std::cerr << "#rows or #cols mismatched. File is (" << nRows << ',' << nCols << ") while input is ("
