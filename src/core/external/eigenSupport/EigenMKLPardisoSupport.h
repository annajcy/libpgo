...
public:
    ...
    enum class MatrixType : int { REAL_STRUCTURAL_SYM = 1, REAL_SPD = 2, REAL_SYM_INDEFINITE = -2, REAL_UNSYM = 11 };

    // Matrix re-ordering is specified as follows:
    // = 0   minimum degree ordering
    // = 2   nested dissection algorithm from the METIS package
    // = 3   parallel (OpenMP) version of nested dissection; it can decrease the computation time on multi-core
    // computers, especially when the constructor takes a long time
    ...
    EigenMKLPardisoSupport(const SpMatD& A, MatrixType mtype = MatrixType::REAL_SYM_INDEFINITE,
                           ReorderingType rtype = ReorderingType::NESTED_DISSECTION, int directIterative = 0,
                           int msgLevel = 0, int maxNumRefinement = 0, int transposeMatrix = 0, int solverMode = 0,
                           int inputMatrixIsUpper = 0);
