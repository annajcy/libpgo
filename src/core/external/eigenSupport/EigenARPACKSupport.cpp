...
extern "C" {
void dsaupd_(int* IDO, char* BMAT, int* N, char* WHICH, int* NEV, double* TOL, double* RESID, int* NCV, double* V,
...
int EigenSupport::solveGenEigShInv(const SpMatD& A, const SpMatD& B, int numEigenvalues, VXd& eigenvalues,
                                   MXd& eigenvectors, double sigma, int verbose) {
    ...
    do {
        dsaupd_(&IDO, &BMAT, &N, WHICH, &NEV, &TOL, RESID.data(), &NCV, V.data(), &LDV, IPARAM, IPNTR, WORKD.data(),
    ...
    int     LDZ   = N;
    double  SIGMA = sigma;
    dseupd_(&RVEC, &HOWMNY, SELECT.data(), D, Z, &LDZ, &SIGMA, &BMAT, &N, WHICH, &NEV, &TOL, RESID.data(), &NCV,
