...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::createWeightMatrix(int numRows, int numCols, int numVertices,
                                                                int numWeightsPerVertex, const int* vtxIdx,
                                                                const int*    interpolationVertexIndices,
                                                                const double* interpolationWeights, int inflate,
                                                                SpMatD& A) {
    ...
    if (inflate) {
        ...
        for (int vi = 0; vi < numVertices; vi++) {
            for (int vj = 0; vj < numWeightsPerVertex; vj++) {
                entries.emplace_back(vertexIndices[vi], interpolationVertexIndices[vi * numWeightsPerVertex + vj],
                                     interpolationWeights[vi * numWeightsPerVertex + vj]);
                // std::cout << entries.size() << ',' << vertexIndices[vi] << ',' << interpolationVertexIndices[vi *
                // numWeightsPerVertex + vj] << ',' << interpolationWeights[vi * numWeightsPerVertex + vj] << '\n';
            }
        }
    }
...
EIGEN_SUPPORT_INLINE pgo::EigenSupport::SpMatD pgo::EigenSupport::createWeightMatrix(
    int numRows, int numCols, int numVertices, int numWeightsPerVertex, const int* vtxIdx,
    const int* interpolationVertexIndices, const double* interpolationWeights, int inflate) {
    SpMatD W;
...
EIGEN_SUPPORT_INLINE pgo::EigenSupport::SparseMatrixMKL pgo::EigenSupport::createSparseMatrixMKL(SpMatD& A) {
    ...
    mkl_sparse_d_create_csr(&sm.mklHandle, SPARSE_INDEX_BASE_ZERO, (int)A.rows(), (int)A.cols(),
                            (int*)A.outerIndexPtr(), (int*)A.outerIndexPtr() + 1, (int*)A.innerIndexPtr(),
                            (double*)A.valuePtr());
...
EIGEN_SUPPORT_INLINE sparse_matrix_t fromEigenMatrix(const SpMatD& A) {
    ...
    sparse_matrix_t AM;
    sparse_status_t ret;
    ret = mkl_sparse_d_create_csr(&AM, SPARSE_INDEX_BASE_ZERO, (int)A.rows(), (int)A.cols(), (int*)A.outerIndexPtr(),
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::mv(const SpMatD& A, ConstRefVecXd v, RefVecXd b, double alpha, double beta,
                                                int transpose) {
#if defined(MKL_INSPECTOR_EXECUTOR)
    sparse_matrix_t AM = fromEigenMatrix(A);
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::mv(SparseMatrixMKL A, ConstRefVecXd v, RefVecXd b, double alpha,
                                                double beta, int transpose) {
    sparse_operation_t opt = SPARSE_OPERATION_NON_TRANSPOSE;
    if (transpose)
        opt = SPARSE_OPERATION_TRANSPOSE;
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::mm(const SpMatD& A, ConstRefMatXd B, RefMatXd C, int transpose) {
    ...
    mkl_sparse_d_mm(opt, 1.0, AM, desc, SPARSE_LAYOUT_COLUMN_MAJOR, B.data(), (int)B.cols(), (int)B.rows(), 0.0,
                    C.data(), (int)C.rows());
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::mm(const SpMatD& A, ConstRefMatXd B, RefMatXd C, double alpha, double beta,
                                                int transpose) {
#if defined(MKL_INSPECTOR_EXECUTOR)
    sparse_matrix_t AM = fromEigenMatrix(A);
    ...
    mkl_sparse_d_mm(opt, alpha, AM, desc, SPARSE_LAYOUT_COLUMN_MAJOR, B.data(), (int)B.cols(), (int)B.rows(), beta,
                    C.data(), (int)C.rows());
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::mm(SparseMatrixMKL A, ConstRefMatXd B, RefMatXd C, int transpose) {
    ...
    mkl_sparse_d_mm(opt, 1.0, A.mklHandle, desc, SPARSE_LAYOUT_COLUMN_MAJOR, B.data(), (int)B.cols(), (int)B.rows(),
                    0.0, C.data(), (int)C.rows());
}

EIGEN_SUPPORT_INLINE void pgo::EigenSupport::mm(SparseMatrixMKL A, ConstRefMatXd B, RefMatXd C, double alpha,
                                                double beta, int transpose) {
    sparse_operation_t opt = SPARSE_OPERATION_NON_TRANSPOSE;
    if (transpose)
        opt = SPARSE_OPERATION_TRANSPOSE;
    ...
    mkl_sparse_d_mm(opt, alpha, A.mklHandle, desc, SPARSE_LAYOUT_COLUMN_MAJOR, B.data(), (int)B.cols(), (int)B.rows(),
...
#if defined(PGO_HAS_MKL)
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::mm(SparseMatrixMKL A, SparseMatrixMKL B, SparseMatrixMKL C,
                                                int transpose) {
    sparse_operation_t opt = SPARSE_OPERATION_NON_TRANSPOSE;
    if (transpose)
        opt = SPARSE_OPERATION_TRANSPOSE;
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::aba(const SpMatD& A, const double* W, SpMatD& C, int transpose) {
    ...
    sparse_matrix_t BM;
    ret = mkl_sparse_d_create_csr(&BM, SPARSE_INDEX_BASE_ZERO, (int)diagSize, (int)diagSize, rowStart.data(),
                                  rowStart.data() + 1, rowStart.data(), (double*)W);
    if (ret != SPARSE_STATUS_SUCCESS) {
        std::cout << "Encounter sparse matrix creation Error: " << ret << std::endl;
        abort();
    }
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::transposeTransfer(const SpMatD& A, SpMatD& AT, const SpMatI& mapping,
                                                               int parallel) {
    Eigen::Index nnz = AT.nonZeros();
    if (parallel) {
        tbb::parallel_for((Eigen::Index)0, nnz,
                          [&](Eigen::Index i) {
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::small2Big(const SpMatD& Asmall, const SpMatD& Abig,
                                                       const std::vector<int>& dofs, SpMatI& mapping) {
    // std::vector<TripletI> entries;
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::small2Big(const SpMatD& Asmall, const SpMatD& Abig,
                                                       const std::vector<int>& dofsRow, const std::vector<int>& dofsCol,
                                                       SpMatI& mapping) {
    std::vector<TripletI> entries;
    entries.reserve(Asmall.nonZeros());
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::small2Big(const SpMatD& Asmall, const SpMatD& Abig, int rowStart,
                                                       int colStart, SpMatI& mapping) {
    std::vector<TripletI> entries;
    entries.reserve(Asmall.nonZeros());
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::big2Small(const SpMatD& Abig, const SpMatD& Asmall, int rowStart,
                                                       int colStart, SpMatI& mapping, int allowDifferentTopo) {
    std::vector<TripletI> entries;
    ...
    for (Eigen::Index outeri = 0; outeri < Abig.outerSize(); outeri++) {
        for (SpMatD::InnerIterator it(Abig, outeri); it; ++it) {
            ...
            if (row >= 0 && col >= 0 && row < Asmall.rows() && col < Asmall.cols()) {
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::big2Small(const SpMatD& Abig, const SpMatD& Asmall,
                                                       const std::vector<int>& dofsRow, const std::vector<int>& dofsCol,
                                                       SpMatI& mapping, int allowDifferentTopo) {
    std::vector<TripletI> entries;
    entries.reserve(Abig.nonZeros());
    ...
    for (Eigen::Index outeri = 0; outeri < Abig.outerSize(); outeri++) {
        for (SpMatD::InnerIterator it(Abig, outeri); it; ++it) {
            ...
            if (rowSmall >= 0 && colSmall >= 0 && rowSmall < Asmall.rows() && colSmall < Asmall.cols()) {
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::removeRowsCols(const SpMatD& Abig, const SpMatD& Asmall,
                                                            const std::vector<int>& removedDofs, SpMatI& mapping) {
    ...
    for (Eigen::Index outeri = 0; outeri < Abig.outerSize(); outeri++) {
        for (SpMatD::InnerIterator it(Abig, outeri); it; ++it) {
            ...
            if (row >= 0 && col >= 0) {
                ...
                entries.emplace_back((SpMatI::StorageIndex)it.row(), (SpMatI::StorageIndex)it.col(), -1);
            }
        }
    }
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::removeRowsCols(const SpMatD& Abig, const std::vector<int>& removedDofs,
                                                            SpMatD& mat) {
    removeRowsCols(Abig, removedDofs, removedDofs, mat);
}

EIGEN_SUPPORT_INLINE void pgo::EigenSupport::removeRowsCols(const SpMatD& Abig, const std::vector<int>& removedRowDofs,
                                                            const std::vector<int>& removedColDofs, SpMatD& mat) {
    std::vector<int> dofMappingsRow(Abig.rows());
    std::vector<int> dofMappingsCol(Abig.cols());
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::selectRowsCols(const SpMatD& Abig, const std::vector<int>& selectedDofs,
                                                            SpMatD& mat) {
    selectRowsCols(Abig, selectedDofs, selectedDofs, mat);
}

EIGEN_SUPPORT_INLINE void pgo::EigenSupport::selectRowsCols(const SpMatD& Abig, const std::vector<int>& selectedRowDofs,
                                                            const std::vector<int>& selectedColDofs, SpMatD& mat) {
    std::vector<int> dofMappingsRow(Abig.rows());
    std::vector<int> dofMappingsCol(Abig.cols());
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::addSmallToBig(double coeff, const SpMatD& Asmall, SpMatD& Abig,
                                                           double beta, const SpMatI& mapping, int parallel) {
    Eigen::Index nnz = Asmall.nonZeros();
    if (parallel) {
        tbb::parallel_for((Eigen::Index)0, nnz,
                          [&](Eigen::Index i) {
                              Eigen::Index newOffset = mapping.valuePtr()[i];
                              Abig.valuePtr()[newOffset] =
                                  Abig.valuePtr()[newOffset] * beta + coeff * Asmall.valuePtr()[i];
                          },
                          tbb::static_partitioner());
    } else {
        for (Eigen::Index i = 0; i < nnz; i++) {
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::addBigToSmall(double coeff, const SpMatD& Abig, SpMatD& Asmall,
                                                           double beta, const SpMatI& mapping, int parallel) {
    ...
    if (parallel) {
        tbb::parallel_for((Eigen::Index)0, nnz,
                          [&](Eigen::Index i) {
                              Eigen::Index newOffset = mapping.valuePtr()[i];
                              if (newOffset >= 0) {
                                  Asmall.valuePtr()[newOffset] =
                                      Asmall.valuePtr()[newOffset] * beta + coeff * Abig.valuePtr()[i];
                              }
                          },
                          tbb::static_partitioner());
    } else {
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::transferSmallToBig(const SpMatD& Asmall, SpMatD& Abig,
                                                                const SpMatI& mapping, int parallel) {
    Eigen::Index nnz = Asmall.nonZeros();
    if (parallel) {
        tbb::parallel_for((Eigen::Index)0, nnz,
                          [&](Eigen::Index i) {
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::transferSmallToBig(ConstRefVecXd Asmall, RefVecXd Abig,
                                                                const std::vector<int>& mapping, int parallel) {
    if (parallel) {
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::transferBigToSmall(const SpMatD& Abig, SpMatD& Asmall,
                                                                const SpMatI& mapping, int parallel) {
    Eigen::Index nnz = Abig.nonZeros();
    if (parallel) {
        tbb::parallel_for((Eigen::Index)0, nnz,
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::transferBigToSmall(ConstRefVecXd Abig, RefVecXd Asmall,
                                                                const std::vector<int>& mapping, int parallel) {
    if (parallel) {
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::removeRows(const SpMatD& Abig, const SpMatD& Asmall,
                                                        const std::vector<int>& removedDofs, SpMatI& mapping) {
    std::vector<int> dofMappingsRow(Abig.rows());
    for (Eigen::Index i = 0, j = 0; i < Abig.rows(); i++) {
        if (std::binary_search(removedDofs.begin(), removedDofs.end(), (int)i) == false) {
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::removeRows(int dofAll, const std::vector<int>& removedDofs,
                                                        std::vector<int>& big2smallMapping,
                                                        std::vector<int>& small2bigMapping) {
    int inc = 0;
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::rigidFit(const M3Xd& p0, const M3Xd& p1, M3d& R, V3d& t,
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::symbolicMm(const SpMatD& A, const SpMatD& B, SpMatD& C,
                                                        SymbolicMmData** dat, int transpose) {
#if defined(MKL_INSPECTOR_EXECUTOR)
    ...
    sparse_status_t ret =
        mkl_sparse_sp2m(opt, desc, AM, SPARSE_OPERATION_NON_TRANSPOSE, desc, BM, SPARSE_STAGE_FULL_MULT, &CM);
...
EIGEN_SUPPORT_INLINE void pgo::EigenSupport::mm(const SpMatD& A, const SpMatD& B, const SymbolicMmData* dat, SpMatD& C,
                                                int transpose) {
