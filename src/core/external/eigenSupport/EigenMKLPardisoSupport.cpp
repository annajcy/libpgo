...
EigenMKLPardisoSupport::EigenMKLPardisoSupport(const SpMatD& Ain, MatrixType mt, ReorderingType rt, int di, int ml,
                                               int mr, int tm, int sm, int inputMatrixIsUpper)
    : mtype(mt),
      rtype(rt),
      directIterative(di),
      msgLvl(ml),
      maxNumRefinementSteps(mr),
      transposeMatrix(tm),
      solverMode(sm) {
    ...
    if (inputMatrixIsUpper == 0 && mtype != EigenMKLPardisoSupport::MatrixType::REAL_STRUCTURAL_SYM &&
...
EigenMKLPardisoSupport::~EigenMKLPardisoSupport() {
    ...
    pardiso(pointers.data(), &maxfct, &mnum, &mi_mtype, &phase, &n, nullptr, nullptr, nullptr, perm.data(), &nrhs,
...
void EigenMKLPardisoSupport::setParam() {
    ...
    if (directIterative) {
        if (mtype == MatrixType::REAL_SPD)
            iparm[3] = 62;  // matrix is symmetric positive-definite; use CGS iteration for symmetric positive-definite
                            // matrices
        else
    ...
    iparm[7] =
        maxNumRefinementSteps;  // Max numbers of iterative refinement steps (used during the solving stage). Value of 0
                                // (default) means: The solver automatically performs two steps of iterative refinement
    ...
    iparm[18] = 0;  // Report number of floating point operations (in 10^6 floating point operations) that are necessary
                    // to factor the matrix A. Disabled.
    iparm[19] = 0;  // Output: Report CG/CGS diagnostics.
    iparm[20] = 1;  // Pivoting for symmetric indefinite matrices: Apply 1x1 and 2x2 Bunch-Kaufman pivoting during the
                    // factorization process.
    iparm[21] = 0;  // Output: Inertia: number of positive eigenvalues.
    iparm[22] = 0;  // Output: Inertia: number of negative eigenvalues.

    iparm[23] = 0;  // Parallel factorization control. Use default. 10: Intel oneAPI Math Kernel Library PARDISO uses an
                    // improved two-level factorization algorithm for nonsymmetric matrices.
    iparm[24] =
        0;  // Parallel forward/backward solve control. Intel MKL PARDISO uses a parallel algorithm for the solve step.
...
int EigenMKLPardisoSupport::analyze(const SpMatD& Ain) {
    ...
    pardiso(pointers.data(), &maxfct, &mnum, &mi_mtype, &phase, &n, Aptr->valuePtr(), Aptr->outerIndexPtr(),
            Aptr->innerIndexPtr(), perm.data(), &nrhs, iparm.data(), &msgLvl, nullptr, nullptr, &error);
...
int EigenMKLPardisoSupport::solve(const SpMatD& Ain, double* x, double* rhs, int nrhs_) {
    ...
    pardiso(pointers.data(), &maxfct, &mnum, &mi_mtype, &phase, &n, Aptr->valuePtr(), Aptr->outerIndexPtr(),
            Aptr->innerIndexPtr(), perm.data(), &nrhs, iparm.data(), &msgLvl, rhs, x, &error);
...
std::map<int, std::string> EigenMKLPardisoSupport::errorMessages = {
    ...
    {-4,
     "Zero pivot, numerical factorization or iterative refinement problem.If the error appears during the solution "
     "phase, try to change the pivoting perturbation(iparm[9]) and also increase the number of iterative refinement "
     "steps.If it does not help, consider changing the scaling, matchingand pivoting options(iparm[10], iparm[12], "
     "iparm[20])"},
    ...
    {-15,
     "Internal error which can appear for iparm[23] = 10 and iparm[12] = 1. Try switch matching off(set iparm[12] = 0 "
     "and rerun.)"},
};