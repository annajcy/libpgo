...
pgo::Mesh::TriMeshGeo loadTriangleMesh(const std::string &inputMesh)
{
  ...
  if (toLower(inputPath.extension().string()) != ".obj")
    throw std::runtime_error("cubic_mesher only supports .obj input");
...
std::vector<GridIndex> collectOccupiedVoxels(
  const pgo::Mesh::TriMeshGeo &mesh, const VoxelGrid &grid, OccupancyMode occupancyMode)
{
  ...
  for (int i = 0; i < grid.nx; ++i) {
    for (int j = 0; j < grid.ny; ++j) {
      for (int k = 0; k < grid.nz; ++k) {
        ...
        if (!occupiedVoxel && occupancyMode == OccupancyMode::Conservative) {
  ...
  const TriangleMeshVoxelizerOptions &options)
{
  ...
  const std::vector<GridIndex> occupiedVoxels = collectOccupiedVoxels(mesh, grid, options.occupancy);
