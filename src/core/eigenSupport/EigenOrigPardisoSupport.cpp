  ...
  solverMode(sm), numThreads(nt)
{
  ...
  if (error != 0) {
    std::string msg;
    if (error == -10)
      msg = "No PARDISO license file found";
    else if (error == -11)
      msg = "PARDISO license is expired";
    else if (error == -12)
      msg = "Wrong username or hostname for PARDISO license";
    else
      msg = "pardisoinit error: " + std::to_string(error);
    throw std::runtime_error("[PARDISO] " + msg);
  }
  else {
    if (msgLvl)
