...
std::shared_ptr<PyGridField> PyImplicitField::sampleToGrid(const PyGridSpec &spec) const
{
  nb::gil_scoped_release release;
  return std::make_shared<PyGridField>(
    std::make_shared<ImplicitSurface::GridField>(field_->sampleToGrid(spec.core())));
}
