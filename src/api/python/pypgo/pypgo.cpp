...
void pypgo_init(py::module& m) {
    ...
    m.def("create_tetmeshgeo", [](pyArrayFloat vertices, pyArrayInt elements) -> TetMeshGeo {
...
#endif
            std::cerr << "Wrong tet type:" << tetInfo.ndim << ',' << tetInfo.format << ','
                      << py::format_descriptor<int64_t>::format() << std::endl;
            return TetMeshGeo();
        }

        Eigen::VectorXd vertexPosDouble =
            Eigen::Map<const Eigen::VectorXf>((float*)vtxInfo.ptr, vtxInfo.size).cast<double>();
        Eigen::VectorXi tets = Eigen::Map<const Eigen::VectorXi>((int*)tetInfo.ptr, tetInfo.size);

        pgoTetMeshGeoStructHandle tetmesh = pgo_create_tetmeshgeo(
            (int)vertexPosDouble.size() / 3, vertexPosDouble.data(), (int)tets.size() / 4, tets.data());
        return TetMeshGeo(tetmesh);
    });

    m.def("create_tetmeshgeo_from_file", [](const std::string& filename) -> TetMeshGeo {
        pgoTetMeshGeoStructHandle tetmesh = pgo_create_tetmeshgeo_from_file(const_cast<char*>(filename.c_str()));
        return TetMeshGeo(tetmesh);
    });

    m.def("tetmeshgeo_get_num_vertices", [](TetMeshGeo m) -> int { return pgo_tetmeshgeo_get_num_vertices(m.handle); });

    m.def("tetmeshgeo_get_num_tets", [](TetMeshGeo m) -> int { return pgo_tetmeshgeo_get_num_tets(m.handle); });
    ...
    m.def("destroy_tetmeshgeo", [](TetMeshGeo tetmesh) { pgo_destroy_tetmeshgeo(tetmesh.handle); });

    m.def("create_tetmesh",
          [](pyArrayFloat vertices, pyArrayInt elements, float E, float nu, float density) -> TetMesh {
              py::buffer_info vtxInfo = vertices.request();
              py::buffer_info tetInfo = elements.request();
              ...
              pgoTetMeshStructHandle tetmesh =
                  pgo_create_tetmesh((int)vertexPosDouble.size() / 3, vertexPosDouble.data(), (int)tets.size() / 4,
                                     tets.data(), (double)E, (double)nu, (double)density);
    ...
    m.def("update_tetmesh_vertices", [](TetMesh& tetmesh, pyArrayFloat vtxNew) -> TetMesh {
        ...
        if (vtxInfo.ndim != (py::ssize_t)2 || vtxInfo.shape[1] != 3 ||
            vtxInfo.format != py::format_descriptor<float>::format()) {
            std::cerr << "Wrong vertex type:" << vtxInfo.ndim << ',' << vtxInfo.format << std::endl;
            return tetmesh;
        }
        ...
        Eigen::VectorXd vtxNewDouble =
            Eigen::Map<const Eigen::VectorXf>((float*)vtxInfo.ptr, vtxInfo.shape[0] * 3).cast<double>();
    ...
    m.def("create_element_laplacian_matrix",
          [](TetMeshGeo tetmesh, int face_neighbor, int n_repreat, int scale) -> SparseMatrix {
              pgoSparseMatrixStructHandle mat =
                  pgo_create_tet_laplacian_matrix(tetmesh.handle, face_neighbor, n_repreat, scale);
              return SparseMatrix(mat);
          });

    m.def("create_tet_biharmonic_gradient_matrix",
          [](TetMeshGeo tetmesh, int face_neighbor, int scale) -> SparseMatrix {
              pgoSparseMatrixStructHandle mat =
                  pgo_create_tet_biharmonic_gradient_matrix(tetmesh.handle, face_neighbor, scale);
              return SparseMatrix(mat);
          });

    m.def("conjugate_mv", [](SparseMatrix m, pyArrayFloat v) -> double {
    ...
    m.def("destroy_sparse_matrix", [](SparseMatrix m) { pgo_destroy_sparse_matrix(m.handle); });

    m.def("create_tet_gradient_matrix",
          [](TetMeshGeo m) -> SparseMatrix { return SparseMatrix(pgo_create_tet_gradient_matrix(m.handle)); });
    ...
    m.def("destroy_smooth_rs_energy", [](SmoothRSEnergy energy) { pgo_destroy_smooth_rs_energy(energy.handle); });
    ...
    m.def("create_trimeshgeo", [](pyArrayFloat vertices, pyArrayInt triangles) -> TriMeshGeo {
        py::buffer_info vtxInfo = vertices.request();
...
#endif
            std::cerr << "Wrong tri type:" << triInfo.ndim << ',' << triInfo.format << ','
                      << py::format_descriptor<int64_t>::format() << std::endl;
            return TriMeshGeo();
        }
        ...
        pgoTriMeshGeoStructHandle trimesh = pgo_create_trimeshgeo(
            (int)vertexPosDouble.size() / 3, vertexPosDouble.data(), (int)tris.size() / 3, tris.data());
        return TriMeshGeo(trimesh);
    });

    m.def("trimeshgeo_get_num_vertices", [](TriMeshGeo m) -> int { return pgo_trimeshgeo_get_num_vertices(m.handle); });

    m.def("trimeshgeo_get_num_triangles",
          [](TriMeshGeo m) -> int { return pgo_trimeshgeo_get_num_triangles(m.handle); });
    ...
    m.def("destroy_trimeshgeo", [](TriMeshGeo m) { pgo_destroy_trimeshgeo(m.handle); });
    ...
    m.def("mesh_isotropic_remeshing",
          [](TriMeshGeo m, double targetEdgeLength, int nIter, double angleThreshold) -> TriMeshGeo {
    ...
    m.def("trimesh_closest_distances", [](TriMeshGeo m, pyArrayFloat queryPt) -> py::array_t<float> {
        ...
        Eigen::VectorXd queryPtDouble =
            Eigen::Map<const Eigen::VectorXf>((float*)quertPtInfo.ptr, quertPtInfo.size).cast<double>();
        Eigen::VectorXd queryDist(quertPtInfo.size / 3);
        pgo_trimesh_closest_distances(m.handle, (int)quertPtInfo.size / 3, queryPtDouble.data(), queryDist.data(),
                                      nullptr);
    ...
    m.def("tetmesh_barycentric_weights",
          [](TetMeshGeo m, pyArrayFloat queryPt) -> std::tuple<py::array_t<float>, py::array_t<int>> {
              py::buffer_info quertPtInfo = queryPt.request();
              if (quertPtInfo.ndim != (py::ssize_t)1 || quertPtInfo.format != py::format_descriptor<float>::format()) {
                  std::cerr << "Wrong vertex type:" << quertPtInfo.ndim << ',' << quertPtInfo.format << std::endl;
              ...
              pgo_tetmesh_barycentric_weights(m.handle, (int)quertPtInfo.size / 3, queryPtDouble.data(), queryW.data(),
                                              (int*)iinfo.ptr);
    ...
    m.def(
        "run_sim_from_config",
        [](const std::string& filename, py::object deterministic) -> int {
            if (deterministic.is_none()) {
                return pgo_run_sim_from_config(filename.c_str());
            }

            return pgo_run_sim_from_config_with_deterministic(filename.c_str(), py::cast<bool>(deterministic) ? 1 : 0);
        },
        py::arg("filename"), py::arg("deterministic") = py::none(),
        "Run simulation from config file. Pass deterministic=True/False to override config field.");

    m.def("convert_animation_to_abc", [](const std::string& configFilename, const std::string& outputFolder) -> int {
        return pgo_convert_animation_to_abc(configFilename.c_str(), outputFolder.c_str());
    });

    m.def("debug", []() { std::cout << "Test!!!" << std::endl; });
