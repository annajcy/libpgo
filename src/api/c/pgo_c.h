...
LIBPGO_C_EXPORT pgoTetMeshStructHandle pgo_create_tetmesh(int nv, double* vertices, int ntet, int* tets, double E,
...
LIBPGO_C_EXPORT pgoSparseMatrixStructHandle pgo_create_tet_laplacian_matrix(pgoTetMeshGeoStructHandle tetmesh,
                                                                            int faceNeighbor, int repeat, int scale);
...
LIBPGO_C_EXPORT pgoSparseMatrixStructHandle pgo_create_tet_biharmonic_gradient_matrix(pgoTetMeshGeoStructHandle tetmesh,
                                                                                      int faceNeighbor, int scale);

LIBPGO_C_EXPORT pgoSmoothRSEnergyStructHandle pgo_create_smooth_rs_energy(pgoTetMeshGeoStructHandle tetmesh,
                                                                          double coeffR, double coeffS);
...
LIBPGO_C_EXPORT void    pgo_smooth_rs_energy_hess(pgoSmoothRSEnergyStructHandle energy, double* x, int* rows, int* cols,
                                                  double* values);
...
LIBPGO_C_EXPORT pgoTriMeshGeoStructHandle pgo_mesh_isotropic_remeshing(pgoTriMeshGeoStructHandle trimesh,
                                                                       double targetEdgeLength, int nIter,
                                                                       double angleThreshold);

LIBPGO_C_EXPORT void pgo_trimesh_closest_distances(pgoTriMeshGeoStructHandle trimesh, int n, double* queryPos,
                                                   double* queryDistance, int* queryTri);
LIBPGO_C_EXPORT void pgo_tetmesh_barycentric_weights(pgoTetMeshGeoStructHandle tetmesh, int n, double* queryPos,
                                                     double* queryW, int* queryEle);

LIBPGO_C_EXPORT int pgo_run_sim_from_config(const char* configFileName);
LIBPGO_C_EXPORT int pgo_run_sim_from_config_with_deterministic(const char* configFileName, int deterministic);
