...
### Basis — `formulations/basis/`

Shape functions $N_a$ and their reference gradients $\partial N_a/\partial\boldsymbol\xi$
(`Basis`, `formulations/basis/basis.h`). The three concrete bases differ in *order* and
*continuity*:

- **`TetP1Basis`** — 4 nodes, linear. $\mathbf F$ is **constant** over the tet (one material
  point suffices). The classic, robust workhorse.
- **`HexTrilinearBasis`** — 8 nodes, trilinear tensor product on $[0,1]^3$. $\mathbf F$
  varies across the hex.
- **`HexTricubicHermiteBasis`** — 64 scalar functions = 8 corners × 8 **Hermite modes**
  ...
  basis for high-order/smooth simulation.
...
- **`TetP1DefaultQuadrature`** — 1 point at the centroid ($w=\tfrac16$); exact because the
...
- **`HexTricubicHermiteDofLayout`** — one vertex carries **24** DOFs (8 Hermite modes × 3
  ...
  (`formulations/formulation.h:49-61,176-177`).
...
`TricubicHermiteFormulation` overrides all three with Hermite-aware versions
(`formulations/formulation.h:90-97,179-186`).

### Concrete formulations

| Python | Basis | Quadrature | DOF layout | Local DOFs | `.name` |
|---|---|---|---|---|---|
| `TetP1` | linear tet (4) | 1-pt centroid | `Vertex3` | 12 | `tet_p1` |
| `LinearCubic` | trilinear hex (8) | $2^3$ Gauss | `Vertex3` | 24 | `hex_trilinear` |
| `TricubicHermite` | tricubic Hermite (64) | $4^3$ Gauss | `HexTricubicHermite` (24/vtx) | 192 | `hex_tricubic_hermite` |
| `KoiterShell` | Koiter shell | — | `Vertex3` | 18 | `shell_koiter` |

(`formulations/formulation.cpp:96,111,370,440`.) `TetP1`/`LinearCubic`/`TricubicHermite`
are volumetric; `KoiterShell` is a shell — pair it with [`KoiterStVK`](elastic.md).

## Quick reference

```python
from pypgo.fem import TetP1, TricubicHermite

formulation = TetP1()
formulation.name                       # "tet_p1"
